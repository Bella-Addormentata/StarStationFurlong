...
- **Oriented transit — the silent-180 fix (#102):** the pose helpers `physicalDoorPose` / `physicalDoorPoseOrNull` / `physicalPortPose` and the projection helper `projectionPoseForDoor` now take the room's door layout as an explicit parameter (`DoorRecordsMap`), so a caller reasoning about the ARRIVAL room's doors gets the arrival answer instead of whatever the module-global happened to hold. `world.completeAdapterArrival` captures the arrival snapshot at entry and threads it through `resolveArrivalDoor` and the R2 station-bias math, closing the ordering hazard that `applyRoomVisuals`'s `setDoorRecords` runs AFTER the arrival hook. `projectionPoseFromWall`'s chained-connector branch now returns `rotY: 0` on an unknown far wall (matching the straight-gangway branch) — a guessed rotation is a silent R2 violation, per the owner's ruling. First-person yaw in `zoom.ts` is seeded from the camera rig on the 2→1 handoff (unless a seat's `faceAngle` is the authority for that entry), so a dive-and-return after a rotated transit preserves the on-screen "forward". Vestibule proximity fade adopts a three-threshold hysteresis (in 1.5 m / hold 3.0 m / out 4.0 m) so a small isometric jitter around a detent no longer flickers opacity. Reproduction harnesses land as `src/orientedTransit.test.ts` (H1 arrival-heading continuity under both legacy layouts, H2 mixed-layout silent-180 / T1 projection zero, H3 mirrorSegments rotY cancellation). **Audit round 1 remediation (2026-08-21):** three follow-on fixes with pure-helper extractions so the composed paths are regression-guarded: (i) the T5 `seatFaceOverride` flag no longer arms when a sit-down fires already at level 1 (arming there stranded the flag past the eventual dive and silently defeated its rig-inherit yaw seed) — pure predicate `shouldPinSeatFaceOverride(currentLevel, faceAngle)` moved to `src/zoomSeatPolicy.ts` and called inline from `requestFirstPerson`; (ii) the arrival hook's R2 composition (`selectArrivalDoorLayout` → `computeArrivalStationBias`) is now exercised end-to-end from a bound `Y.Doc` (harness H2b — catches the re-wire class the primitive harnesses could not); (iii) the vestibule proximity fade's target math is continuous at every threshold — pre-fix, at `dist = FADE_HOLD` the flat hold branch returned 1.0 and the fall-through ramp returned ~0.4 (a ~0.6 jump the per-frame chase-lerp then papered over on slow walks and revealed on fast ones); the fix has each side's ramp span only its half of the hysteresis band, and lives in `src/vestibuleFade.ts` under `computeVestibuleFadeTarget` with boundary continuity assertions (harness H5). All 125 vitest tests pass; `tsc --noEmit` clean. **Honest limits.** Furniture placements that assume the retired cardinal wall convention (world.ts:2205 casino chandelier positions among them) still key on the old x/z layout — the fix moves geometry math, not authored art; a follow-up will retire those literals once furniture layout follows the same DoorRecordsMap seam.
