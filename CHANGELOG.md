...
- **Card-wager REFUND-MINT close + honest-limits sharpening (#45, part of):** `refundCardWager` in [`src/casinoDoc.ts`](prototypes/0.29.0-core-loop-demo/src/casinoDoc.ts) was the ONE escrow-consuming path that credited a record's SELF-DECLARED `amount` without checking it against the table config. A hostile peer can raw-write a well-formed `held` escrow record (Yjs has no write ACL) carrying `amount = MAX_CARD_WAGER_BUY_IN` (1,000,000) under a player id they control — no matching pay-in debit ever happened — then let an HONEST owner's cancel / stray-record sweep CREDIT it, minting up to a million chips from nothing and shattering the module's `escrowed = paid_out + refunded + still_held` conservation invariant (the honest owner is the laundering vector; the attacker never touches a real balance directly). The refund now runs the SAME conformance gate `activateCardWager` and `settleCardWager` already apply before they move escrow chips — `isCardWagerRecord` ∧ `amount === config.buyIn` ∧ `ownerId === config.ownerId` ∧ `kind === config.kind` ∧ `state === 'held'` ∧ `playerId === key` — and returns only the OWNER-declared `config.buyIn` (never the record's self-declared figure). A non-conforming record is DELETED by the owner WITHOUT credit (debris cleanup that can never mint) and REFUSED outright for a non-owner self-caller. **Honest residual (unsigned-chip v1, now documented in-code and here):** a record planted at EXACTLY `config.buyIn` still returns one buy-in on a conforming refund — under an attacker-chosen puppet id (an owner sweep credits the puppet) OR under an HONEST payer's own key, since `payCardWager`'s crash-retry idempotency returns `true` WITHOUT debiting when a conforming record already occupies the key, so that payer's own pre-BEGIN self-refund hands them a buy-in they never paid — but either way it is exactly one buy-in, no more power than the raw `bal:` write the same no-ACL peer can already make this phase (chips are unsigned doc state; the derived cage ledger's `houseNet` surfaces the anomaly), and G4 signed-registry chips close it for real. The fix strips refund's AMPLIFICATION of that baseline into an attacker-sized, honest-owner-laundered mint. The same pass sharpens the two-player-confirm-gate wording to match the unsigned-doc reality: result-acks are UNSIGNED Yjs writes on the `cw-ack:` own-key lane, so a peer with raw doc-write capability — the SAME capability assumed in the forged-terminal-state threat — can also forge the counterparty's ack. The gate is therefore a hard stop against a UI-BOUND lying payer (who cannot write another peer's key) and, against a raw-writer, converts a SILENT wrong-payout into a VISIBLE DISPUTED / withheld-consent state that the honest client contests by own-key LWW re-ack (a liveness race, not a cryptographic proof); full finality is the chia-gaming signed-referee layer the module-header contract maps piece-by-piece. Tests grew from 293 to 297 (`cardWager.test.ts` 69 → 73): an inflated 1,000,000-chip hostile plant swept by the owner credits nothing and conserves the real 200-chip supply; a wrong-owner plant and a mis-bound (field `playerId` ≠ key) conforming-amount plant each delete without credit; a self-caller cannot sweep a non-conforming record. `npx tsc --noEmit` clean; `npx vitest run` 297/297. No version bump; node binaries unchanged.
- **Card-wager TWO-PLAYER CONFIRM GATE + wager-config hardening (#45, part of):** the wager settle no longer trusts the shared PokerState alone. Threat closed: any peer can write the poker doc's whole-value LWW game state, so a hostile player could plant a forged terminal state ("I hold all the chips") and steer an HONEST owner's auto-settle into paying the pot to the wrong seat. Settlement is now gated on BOTH payers countersigning the SAME result — new `CardWagerAck` shape + `cw-ack:<tableId>:<playerId>` own-key lane in [`src/games/cardWager.ts`](prototypes/0.29.0-core-loop-demo/src/games/cardWager.ts), with `ackCardWagerResult` in [`src/casinoDoc.ts`](prototypes/0.29.0-core-loop-demo/src/casinoDoc.ts) (payer-only vote; the named winner must be a current payer or `'split'`; re-acking is whole-value LWW on the own key — the dispute-resolution path), shape-guarded `readCardWagerAck`, and `readAgreedCardWagerResult` (null unless the escrow set is the exact heads-up pair and both acks are present, key-bound — the payload `playerId` must match the key it sits under, kind-bound to the config, FRESH — `matchStartedAt` must echo `config.startedAt`, so a replayed ack from a previous match on the same table is dead — and identical). `settleCardWager` refuses unless that agreed result exists AND equals the owner's `winnerId` argument (the owner can no longer unilaterally pick a winner), and deletes both acks inside the same settle transact; `activateCardWager` sweeps stale ack keys at BEGIN; `clearCardWagerKeys` sweeps them on both the owner and no-config recovery paths. The confirmed value in the UI is ENGINE-DERIVED only — new pure `deriveHeadsUpWinner` over the two terminal stacks (one live stack → that seat; both live → `'split'`; missing id / both zero → null, never a payout) — no free-form winner picking, so a griefing payer cannot "confirm" a wrong result into a wedge; a payer who never confirms cannot hostage the pot either, because the owner's new match-over CANCEL — REFUND BOTH valve refunds both buy-ins at any point. The match-over WAGER panel shows the derived RESULT, per-seat CONFIRMED/UNCONFIRMED, a DISPUTED warning when the two acks disagree, and a per-payer CONFIRM RESULT button (✔ YOU CONFIRMED once your fresh ack matches the current derivation). NOT defended, documented in-code: a malicious OWNER — the owner is the doc's money author by design; arbitrating the owner themselves is the chia-gaming referee's job on the later slice. Config hardening in the same pass: `stampCardWagerConfig` grew a LIVE-MATCH LOCK (no re-stamp of any shape while `startedAt !== null` — a successful one would reset `startedAt` to null and reopen the self-refund window mid-match) and an ESCROW-FREEZE (buy-in/kind cannot drift while any escrow record is held; an identical re-stamp stays legal for crash retry), and poker got a real buy-in FLOOR — `POKER_MIN_WAGER_BUY_IN = 20` via per-kind `minCardWagerBuyIn(kind)`, because `beginPoker` clamps `startingStack` to at least 2×bigBlind (= 20 with the fixed 10-chip big blind), so a smaller buy-in would seat stacks LARGER than the escrowed pot; enforced at stamp time, in the enable-wager UI (input `min` + refusal), and defensively in `beginPokerMatch` — which now also refuses a kind-mismatched (e.g. `'war'`) config planted on the poker felt. `maybeSettleWager` keys its re-entrancy signature on (table, matchStartedAt, agreed result) so a fresh match settles cleanly while re-renders stay quiet. The cancel path was hardened too: `cancelPokerWager` now refunds by scanning the actual escrow records instead of iterating the two seat ids, closing a chips-burn edge where a hostile raw write nulling a seat id made the owner's cancel skip that player's refund while `clearCardWagerKeys` deleted the still-held record. Tests grew from 260 to 293: the 33 new/updated cases cover the `isCardWagerAck` guard branch-by-branch, the `deriveHeadsUpWinner` table, the per-kind floors, write-side ack refusals (spectator, pre-BEGIN, outsider winner, empty actor, bad time), the agreed-result matrix (missing / one-sided / DISPUTED + LWW re-ack resolution / stale freshness echo / mis-bound key / kind mismatch / forged third escrow record), the settle gate (refused until both confirm; the owner cannot pay out a result the payers did not agree on; acks deleted on settle), both sweep paths, and all three re-stamp guards — and every pre-existing settle test now runs through the confirm gate. No version bump; node binaries unchanged.
- **Card-felt SECURE WAGERING (#45, part of):** heads-up poker on the game table now supports an optional WAGER MODE — both players ESCROW a chip BUY-IN into the casino doc before dealing, and the winner takes the POT through ONE single-writer settle by the wager owner. Plain-language throughout (WAGER / BUY-IN / POT — never token/channel jargon). New pure module [`src/games/cardWager.ts`](prototypes/0.29.0-core-loop-demo/src/games/cardWager.ts) owns the two record shapes (`CardWagerConfig`, `CardWagerRecord`), key layout (`cw-cfg:<tableId>`, `cw-escrow:<tableId>:<playerId>`), buy-in bounds (1..1,000,000 chips), the conservation invariant helper (`isCardWagerConservationBalanced`: escrowed_total = paid_out + refunded + still_held), and a full CHIA-GAMING COMPATIBILITY CONTRACT in the module header — the escrow shape mirrors a two-party state channel so a later Chialisp-referee swap is a single-layer upgrade (config + escrow records → stake coin; owner settle → referee puzzle; Yjs LWW → signed off-chain state; abandonment refund → unilateral channel close). Escrow-side transacted operations added to [`src/casinoDoc.ts`](prototypes/0.29.0-core-loop-demo/src/casinoDoc.ts): `stampCardWagerConfig` (owner single-writer for `cw-cfg`), `payCardWager` (payer single-writer for their own `cw-escrow` key, debit + record write in one transact, crash-retry idempotent), `activateCardWager` (owner stamps `startedAt` after both PAID — locks self-refund; heads-up-exactly enforcement rejects a hostile-peer THIRD escrow), `settleCardWager` (owner atomic transact: credit winner from pot + delete both records + delete config; split-tie path returns each buy-in; refuses on anything other than exactly two well-formed records — a hostile peer that plants a well-formed extra record without paying cannot inflate the pot), `refundCardWager` (self pre-BEGIN, owner-abandonment any time), `clearCardWagerKeys` (owner-only teardown). Poker device UI at the table's card face grew a WAGER panel — owner enables with a buy-in input, per-seat PAID/PENDING indicator, POT display, PAY BUY-IN button, CANCEL WAGER (owner or payer), auto-settle on match-over derived from final stacks; free-play mode (no config) stays bit-identical to today. Chips flow only through the escrow path so the cage ledger's issued/outstanding/cashed math is unaffected. HOSTILE-PEER posture locked by tests: non-owner cannot settle/activate; non-participant cannot pay for someone else or refund another; malformed records read as absent shape-first; self-refund refused after BEGIN; second settle refuses (no double-credit); a well-formed hostile third escrow record blocks BOTH settle paths so the owner must sweep before payout. New test file [`src/games/cardWager.test.ts`](prototypes/0.29.0-core-loop-demo/src/games/cardWager.test.ts): 36 tests covering shape guards (every field's malformed branch), the conservation helper, end-to-end pay/refund/settle round-trip (winner takes pot, split-tie, war-kind end-to-end), owner-abandonment refund after BEGIN, hostile-peer refusals across every entry point, drain safety (a refused settle mutates nothing), and concurrent divergence (two Y.Docs each pay their own player, merge via `encodeStateAsUpdate`/`applyUpdate`, converge to identical map contents). Vitest coverage grew from 224 to 260 tests. **Honest limits:** the referee is still the OWNER — no Chialisp puzzle, no on-chain payout; hole cards are still visible in the doc (hidden-info commit-reveal blocks on the same later slice); war UI is not wired for wager mode this pass (the casinoDoc functions already accept `kind: 'war'` and are tested end-to-end; the war panel wiring is a follow-up). No version bump; node binaries unchanged.
- **Card games on the game table's felt face (#45, part of):** the table's flippable card side now hosts three games alongside the board's chess/checkers. **WAR** (two-player automatic draw-and-compare with recursive war-within-war on ties, and a clean under-supply gameover), **KLONDIKE SOLITAIRE** (single-player: 7-column tableau + stock/waste + four suit foundations, alternating-colour descending runs on the tableau, same-suit ascending Ace-through-King on the foundations, click a source card then a destination pile to move), and **HEADS-UP NO-LIMIT TEXAS HOLD'EM** (two-player: chosen as the canonical clean-standard heads-up variant — the button posts SB and acts first preflop; the big blind acts first postflop; check/call/bet/raise/fold with min-raise enforcement; best-5-of-7 evaluation across nine hand categories with correct tiebreaks). All engines are pure functional TypeScript under [`src/games/`](prototypes/0.29.0-core-loop-demo/src/games/) sharing a deterministic 52-card core ([`cards.ts`](prototypes/0.29.0-core-loop-demo/src/games/cards.ts), splitmix32 PRNG + Fisher-Yates), each with a shape-guard for the doc lane. State syncs through the existing games Y.Map via the same shape-guarded whole-value LWW seam as chess/checkers — the `TableGame` union grew tagged `war`/`solitaire`/`poker` variants beside the tagged `chess` and the legacy tag-less `checkers`. A single VS BOT plays the deterministic opponent when only one seat is claimed. Vitest coverage grew from 97 to 194 tests (cards encoding + RNG uniformity + deal fairness; war round resolution incl. ties, nested war-in-war, and under-supply; solitaire legality helpers, move application, and win detection; poker hand ranking table across all nine categories with ordering + tiebreaks, betting state machine, showdown, visibility, and determinism; round-1 audit regressions on the poker doc trust boundary — `PokerShowdown` shape-guard closes the last unchecked field in `isPokerState` so a hostile peer can't slip an invalid winner seat into the render path, the bot's postflop strength heuristic now evaluates the real 5..7-card set via `evaluateBest` instead of duplicating community cards through `evaluate7` (the old padding fed 3-of-same-card samples to `evaluate5` and inflated high-card boards to trips), and `readVisiblePokerState` mucks a folder's hole cards at showdown/complete instead of revealing them to spectators; round-2 audit regressions on live-game deadlocks — `chooseBotAction` now routes `raise` (not `bet`) when `canCheck` fires with `currentBet !== 0`, so the BB seat's canonical preflop-after-limp path never emits an action the state machine will silently reject and re-loop forever; `dealHand` no longer sets `toAct: 'button'` when the button seat went all-in from posting the small blind (`currentBet` is normalized to `max(sb, bb)`, `toAct` passes to the non-all-in seat, and both-all-in-from-blinds runs to showdown in one write); `advanceAfterAction` closes the street when the opponent seat is all-in or folded so a bot pump cannot land the hand in a "playing" state with no legal actor; `pokerForfeit` in `devices.ts` was folded into a pure `pokerForfeit(state, seat)` transition in `games/poker.ts` that awards the live pot to the opponent first — the old inline forfeit only transferred residual stack and left the pot's chips orphaned in the terminal state; round‑3 audit remediations (2026‑08‑22, PR 132) — **KLONDIKE**: turn‑three draw is now the classic default with a per‑seat DRAW ONE / DRAW THREE toggle in the waiting lobby (rejected after the deal to preserve the stock/waste draw count for the hand), a bounded UNDO history (128 snapshots, snapshot‑before‑transition; each snapshot elides its own history to keep doc shape bounded), and a shape‑guard that rejects `drawMode ∉ {1, 3}` / non‑array `undoHistory` / over‑cap history / nested history‑of‑history — plus the stock‑recycle bug where a full waste was re‑stocked via `waste.reverse()` (which broke determinism: the JUST‑DRAWN top of waste became the next card drawn again) is fixed to `waste.slice()`; **POKER (blinds)**: `dealHand` posts the unmatched‑blind refund — when the shorter‑post player is all‑in from posting and the other posted more, the excess returns to the over‑poster's stack (pre‑fix, stacks 3/1000 with 5/10 blinds produced pot=13 and let a 3‑chip winner walk with 10 chips of BB money BB never contested); **POKER (aggression)**: `applyPokerAction` refuses a `raise` to any amount below `currentBet` (pre‑fix, a short‑stack "raise" to less than the standing bet was accepted as an all‑in raise AND lowered `currentBet`, corrupting the state machine) and refuses aggression into an already‑all‑in / folded opponent (no one to raise AT); `legalActions` gates `bet`/`raise` on `opponentCanRespond && canIncrease` (the actor's maxBet must EXCEED `currentBet`, else CALL is their only aggression); **POKER (trust boundary)**: `isPokerState` now validates per‑street invariants when `status === 'playing'` — both hole cards populated, per‑street community length exact (preflop 0, flop 3, turn 4, river 5), deck holds enough cards to open the remaining streets (preflop ≥ 8, flop ≥ 4, turn ≥ 2), and `street === 'showdown' | 'complete'` is rejected under `'playing'` — so a hostile peer cannot plant a state that drives `dealCommunity` to shift `undefined` through the evaluator; **POKER (mucking)**: `pokerForfeit` now marks the forfeiter `folded = true` so `readVisiblePokerState`'s showdown‑visible branch mucks their hole cards to spectators (pre‑fix a rage‑forfeit revealed both hands, unlike a normal fold); **DEVICES (UI truth)**: `pokerSeatLabel` and the poker bot pump now derive the bot's seat from the `null`‑id convention (mirrors chess/checkers) — pre‑fix both hardcoded `bigBlind`, so after `dealHand`'s hand‑#2 rotation (`state.handsPlayed > 0` swaps button/BB) the human's client stopped pumping the bot AND the UI mislabeled the seats; `showForfeit` on the poker card face is now `'playing'`‑only (pre‑fix `'hand-over'` also enabled FORFEIT, letting a between‑hands rage‑forfeit hop the "start next hand vs. leave" decision point); the card‑felt menu blurb now labels the fairness caveat explicitly — each client shuffles independently from a deterministic seed written into the doc, so a peer who observes the SEED before posting can pre‑compute the whole hand order (trusted‑node only in v1; competitive/wagered play blocks on the same commit‑reveal work as hidden‑hand poker). Tests grew from 194 to 224 (36 solitaire, 62 poker; +12 solitaire covering turn‑3 draw, undo across foundation flip, undo bounded to MAX cap, undo not self‑snapshotting, recycle order preservation, shape‑guard rejects for drawMode/undoHistory shape; +18 poker covering refund SB‑short/BB‑short/canonical, raise‑below‑currentBet, raise‑into‑all‑in, legalActions gating, per‑street deck / community / hole‑card guards, terminal‑state leniency, forfeit mucking / self‑visibility). **v1 scope note (see the felt banner):** hole cards live in the shared doc so spectators can technically read them — a proper commit-reveal or private-map lane will come with the chia-gaming wagering integration (that piece stays out here — it needs the #68 Registry hooks). No chip escrow, no wagers. No version bump; node binaries unchanged.
