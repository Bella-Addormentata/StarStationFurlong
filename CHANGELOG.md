...
- **🚪🕳️ Doors punch actual holes in the octagon hull (#92).** The octagon hull builder now takes a door cut list the same way it already takes windows, and every door records its own hole in the room's shell. New `DoorSurface` type (`wall-neg`/`wall-pos`/`cap-neg`/`cap-pos`) covers all four vertical hull faces — the two extruded SIDE-WALL strips and, newly cut-capable, the two octagonal END CAPS (previously plain fan-triangulated quads). A door punches a floor-flush rectangular NOTCH into the wall's bottom outline (`DOOR_OPENING_WIDTH` × new `DOOR_OPENING_HEIGHT` = 2.0 × 3.0 m, single-source shared with the docking frame's `OPEN_H`) rather than a `Shape.holes` interior cut — a hole would touch the outline at the floor and break triangulation. Cuts merge on overlap, take the max height, clip to the wall on overflow, and REPLACE (not duplicate) the bottom corner when a notch reaches the wall's end. `reconcileDoorPlacements` now rebuilds the hull so a dragged door carries its opening with it (mirroring `reconcileWindowLayout`); the exterior shell view sees the same cuts, and `subscribeDoorLayout` fires `refreshExteriorView` so a free-door add/move/remove on the current room recuts the exterior shell live at zoom 3 (parity with the existing window-layout wiring). The exterior end-cap merge expands the wall-band `ShapeGeometry` via its `index` before appending the gable fans, so cap doors render as a well-formed rectangular hole instead of a garbled triangle spray (the interior barrel keeps its indexed cap mesh unchanged). Pure geometry primitives (`notchedRectOutline`, `clampedDoorCuts`, `doorSurfaceForWall`, `bucketDoorCutsFromRecords`) are exported and pinned by a 38-test vitest suite over CCW winding, sign parity, merge behaviour, corner-touching, room-resize invariants, end-to-end BufferGeometry assertions on both `buildOctagonHull` and `buildOctagonShell` with cap + side-wall cuts, and hostile-record clamp/drop behaviour at the collection boundary. **Defence in depth at the collection boundary (PR #122 review):** the collector `bucketDoorCutsFromRecords` now clamps each opening to its own hull surface's extent at collection time (side walls to `±longHalf`, end caps to `±narrowHalf`, `wallHeight` from `computeOctagonProfile`) so a hostile or malformed door record (peer-writable — `readAllDoorLayout` reads across the doc trust boundary and `isDoorLayoutRecord` only guarantees shape/finiteness; #130 widens this domain) cannot feed a cut that runs off the end of a strip or end cap into the hull builder. The mesh builders' own `clampedDoorCuts` still runs, so the clamp is idempotent and belt-and-braces. **Honest limits:** hull door cuts apply to the CURRENT room only — the exterior shell's neighbour outlines stay plain (matches how neighbour windows work), and doors on non-authoritative unseeded rooms cut the four cardinal defaults (same fallback as `reconcileDoorLayout`).
