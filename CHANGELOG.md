...
- **🏛️ Ventures V4 — the trading floor is a room now (#68):** Ventures gained a diegetic MARKET — a wall-mounted `TRADING FLOOR` panel any shareholder can spawn (dev menu, wall-mount) that opens to a signed OFFER board, a live PRICE CHART, and the settlement TAPE for whichever venture's office you're standing in. Any player signs a SELL or BUY slip, any player accepts one, and a deterministic reconcile applies the trade through the SAME office-authoritative `transferShares` lane that V1/V2 use — no second money authority. Conflict-safety is solved the treasury-doc way (per-taker acceptance slots + a pure `pickCanonicalAccept` reducer: earliest `acceptedAt`, tie-break lex-smallest taker pub); two peers accepting the same offer converge on exactly one winner and shares are conserved across CRDT fork/merge — proved by a 24-test conservation suite (`src/tradingFloorDoc.test.ts`, all green; full 121/121 vitest suite green) that forks two Y.Docs, acts on both, merges both ways, and asserts totals equal `CHARTER_TOTAL_SHARES` in every path. Records are shaped so V3's Registry anchor drops in later with zero UX change. Plain-language rule is honored end-to-end: every string on the screen says OFFER, SHARES, VENTURE, REGISTRY (never NFT / CAT / mint / token / on-chain). **Honest limits:** v1 settles only GIFT offers (priceMojo === 0) — priced offers post to the board and appear on the chart but refuse settlement pending the V3 Registry payment rail; the reconcile is best-effort per session (no persistent tape yet beyond the Y.Doc); manager / share-weighted voting (V5) isn't in this slice.
- **🛡️ Ventures V4 audit hardening (#68 follow-up):** Two audit findings against V4's reconciliation lane closed, plus one misleading docstring corrected. **(1) Backdated-cancel bypass fixed** — `verifyFloorCancel` only shape-checked `Number.isFinite(cancelledAt)`, so a maker could sign a cancel with an arbitrarily small `cancelledAt` and use reconcile's `cancel.cancelledAt <= winner.acceptedAt` comparison to nullify every legitimate accept ever posted against the offer. Both `reconcileTradingFloor` and `listOpenOffers` now apply the new `isCancelInWindow(cancel, offer)` guard which requires `cancel.cancelledAt >= offer.createdAt`; the signature can still verify but a semantically backdated cancel is refused. **(2) Premature reconcile / LWW-decides-winner fixed** — the UI button-handler ran `reconcileTradingFloor()` SYNCHRONOUSLY after `acceptFloorOffer`, BEFORE any Yjs merge. Each peer locally settled from only its own accept and wrote a tape; the earlier `readTapeFor` shortcircuit then prevented any post-merge canonical re-pick, so LWW — not `pickCanonicalAccept` — decided the settled winner. Fix: the shortcircuit is dropped, reconcile splits into a first-time-settlement path and a POST-MERGE CORRECTIVE path (moves shares from the wrong recipient to the canonical one and rewrites the tape; both peers compute bytes-identical corrections → LWW converges); the UI wires accepts and cancels to a new `scheduleReconcileTradingFloor()` merge-quiescence debouncer (400 ms trailing edge) instead of the synchronous call; and `main.ts` binds `subscribeTradingFloor(() => scheduleReconcileTradingFloor())` at the T0 seam so every merge triggers a post-merge reconcile on every peer. **(3) Docstring correction** — the module docstring's claim that "the subscribe hook does this" for reconcile was misleading (no subscribe hook actually called reconcile pre-fix); replaced with the true current wiring. Regression coverage lives in `src/tradingFloorDoc.audit68.regression.test.ts` — finding 1 asserts the trade settles even after a backdated cancel, and finding 2 runs 20 trials with alternating clientId ordering and asserts the canonical (earliest-acceptedAt) winner wins EVERY trial regardless of which peer LWW would have favoured. Full 123/123 vitest suite green; type-check clean. **Honest limits:** the corrective transfer only works while the wrong recipient still holds the shares — if they've been spent onward in the interim, reconcile leaves the stale tape and returns the offer in `refusedOfferIds` (a console.warn is emitted). Advisory-clock timestamps still drift across peers; the fix relies on `pickCanonicalAccept` being deterministic under merged state, not on clock accuracy.
- **🏷️ Ventures V4 label fix — no chain jargon on the player surface (#68 / PR #128 reviewer note 2026‑08‑21):** The trading-floor UI's price cell said `${priceMojo.toLocaleString()} M/share` and its POST-slip input placeholder said `price (mojo/share)`. "mojo" is not on the explicit banned list (singleton / CAT / vault / checkpoint coin) but it is the same species — chain jargon on a player surface that had already promised the plain-language rule. Both labels are now `credits/share` (SSF's existing demo-ledger unit per phone-apps-breakdown §3.1). The internal `priceMojo` field NAME, the integer arithmetic, the `Number.isInteger` guards, and every wire/CBOR shape are unchanged — the field will still map one-to-one to Chia mojo when the V3 Registry rail lands. Tests reference the internal field, not the display strings, so all 131/131 vitest cases continue to pass with no changes to test expectations.
- **🛡️ Ventures V4 audit re-round (#68 round 2):** A max-scrutiny re-audit against the round-1 hardening surfaced two BLOCKER share-theft paths that the round-1 corrective + backdated-cancel fixes exposed, plus one NOTE about an untested export. All three closed. **(1) BLOCKER — share theft via unsigned hostile `FloorTape` planting:** `verifyFloorTape` only shape-checks the record and `FloorTape` carries no `sig`/`writerPub` field, so any peer with Yjs write access could plant a byte-valid tape naming ANY pub as `takerPub`. The round-1 corrective path in `reconcileTradingFloor` then read `existing.takerPub` and moved shares from the attacker's chosen victim to the canonical winner (probe: Alice=40/Bob=30/Charlie=20; Alice legit-SELLs 5; Bob accepts; attacker plants tape with `takerPub=PUB_CHARLIE`; reconcile robs Charlie of 5 shares). Fix: `reconcileTradingFloor` now applies a new `isExistingTapeAuthentic(tape, offer, validAccepts)` gate BEFORE the corrective path — every payload field must match the offer (ventureId, kind, shares, priceMojo, makerPub), `tape.takerPub` must name a real in-window signed FloorAccept, and `tape.settledAt` must equal that accept's `acceptedAt`. A fabricated tape refuses the offer instead of trusting the plant. **(2) BLOCKER — retroactive share theft via backdated `FloorAccept`:** `verifyFloorAccept` symmetrically lacked the `acceptedAt >= offer.createdAt` window check that round-1 added for cancels; because `pickCanonicalAccept` picks the earliest acceptedAt, an attacker who signs an accept with `acceptedAt=0` becomes canonical for ANY offer and the corrective path moves shares from the legit winner to them (probe: Bob legit-accepts at t=1050, trade settles; Eve then signs accept with `acceptedAt=0`, becomes canonical, corrective moves shares Bob→Eve). Fix: `reconcileTradingFloor` and `hasCanonicalAcceptBefore` both filter accepts through `isAcceptInWindow(accept, offer)` (which requires `acceptedAt ∈ [offer.createdAt, offer.expiresAt)`) BEFORE `pickCanonicalAccept` sees them — out-of-window accepts are treated as absent, so they cannot become canonical. **(3) NOTE — `cancelScheduledReconcile` export was live but untested:** the module exposed `scheduleReconcileTradingFloor` and `cancelScheduledReconcile` for UI + test callers but neither had direct coverage; four new fake-timer probes now exercise the debouncer (burst collapses to one trailing reconcile, `cancelScheduledReconcile` prevents a pending timer from firing, is a safe no-op when nothing is scheduled, and the T0 `subscribeTradingFloor → scheduleReconcileTradingFloor` observer wiring is exercised end-to-end). Regression coverage extended in `src/tradingFloorDoc.audit68.regression.test.ts` with eight new tests (two BLOCKER probes plus tampered-payload defense-in-depth for finding 1, forward-dated upper-bound probe for finding 2, four debouncer probes for finding 3). Full **131/131** vitest suite green; type-check clean. **Honest limits:** the fabricated-tape gate is a REFUSE (not a rewrite) — Bob's legitimate settlement is BLOCKED while a fabricated tape stands, so an attacker can DoS a specific offer's settlement by persistently re-planting a byte-valid tape; the attacker still CANNOT move shares off any account (theft is prevented, only settlement is delayed). Because `FloorTape` remains unsigned, the fabricated tape also blocks GC of the offer's map slot (long-dead cleanup uses `readTapeFor` to detect settlement). A follow-up refactor to add `writerPub` + `sig` fields to `FloorTape` (writer must be a current shareholder) is tracked in TODO.md — it would let a fabrication fail signature verification instead of just failing authenticity, and would allow the offer to settle around the plant.
