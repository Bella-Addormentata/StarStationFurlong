...
- **CRDT-backed stand claims at multi-player tables (#76):** the walk-up router now consults a shared `stands` Y.Map keyed by `StandSlot.id` before it picks a spot, so two peers converging on the same table converge on *different* slots — Yjs's per-key LWW picks exactly one occupant on every replica, and the loser's next walk-up sees the claim and rounds to the next open stand. Reserved slots (roulette wheel-head / craps stickman) stay off-limits for regular walk-ups; owner-equivalent players (`canEditRoom().ok`) get them as a last-resort fallback so the house isn't stranded when every civilian slot is taken, and an owner walking BACK onto a reserved slot they already hold resumes on it in the mine-active tier (issue directive: "someone that has spin privileges, to stand in a reserved spot"). Live holders heartbeat their claim every 5 s of real time; a crashed peer's claim ages out after a 15 s TTL and any client's periodic reap deletes it (delete-only, LWW-safe against a racing heartbeat), so a dropped tab self-heals its slot without a coordinator. Round-2 audit hardened the ABORT paths: `_cancelDeviceApproach` in `player.ts` now fires the wrapped `onRelease` uniformly for every abort site (floor-click, seat/door-click, WASD manual, FINE-stuck, obstacles-changed, non-ENGAGED releaseDevice, item-removed / item-moved, `enterFromDoor`, `beamTo`, ~12 sites total) so the claim clears immediately instead of leaking behind a healthy heartbeat, and `navigateToDevice`'s same-device branch now adopts the fresh `DeviceTarget` (not just hooks) so a re-tap that lands on a different slot keeps the newest wrapped closure alive. Round-3 audit hardened the RESUME path: `tickStandHeartbeat` now consults `canPlayerClaim` against the current doc value BEFORE writing, so a tab that thaws past the TTL (Chrome background-tab freeze, phone lock, network drop) yields the slot to the peer who legitimately took it during the outage instead of clobbering their claim via a plain LWW write — and `pickFreeStand`'s local tier re-sort now gates its mine-tier promotion on `isClaimActive` so a stale-mine leftover from a forced reload no longer beats a nearer open civilian slot (aligning the world-side tier assignment with `pickStandForWalkup`'s pure-engine rule). Round-4 audit bounded the clock-skew tolerance in `isClaimActive`: the pre-fix code accepted *any* future timestamp, so a peer writing `at = Date.now() + 1e12` would read as live forever — closing BOTH reclaim paths at once (`canPlayerClaim` refused every other player and `findExpiredClaims` skipped the entry), leaving the slot dead for the life of the doc on every replica. A new `STAND_CLAIM_MAX_SKEW_MS = STAND_CLAIM_TTL_MS` ceiling treats `age < -MAX_SKEW` as *not* live while keeping the documented few-seconds-fast peer tolerance the comment intends. Round-5 (PR #126 review — "a claim is unsigned, so any peer can write any slot key"): claims are now **signed**. `StandClaim` is `{ pub, at, sig }` where `pub` is the holder's Ed25519 identity (`keypair.ts`, #124) and `sig` covers `canonicalEncode({ domain: 'ssf-stand-claim:v1', roomId, slotId, claim: { pub, at } })` — the `doorPolicy.ts` (#129) pattern — so the room and the slot KEY are inside the signed bytes: a valid claim cannot be lifted to another slot or another room, and a record naming a key the writer does not hold never verifies. `standsDoc.ts` verifies every record on read over the map key it was found under and the bound room, refuses unsigned or legacy `{ playerId }` records outright (the map is new in this PR — nothing to migrate), fails closed when no verifier is bound (one console warning per bind), self-checks every write with the same verifier so a record only its writer could see is never posted (`claimStand` returns `null` and the walk-up proceeds unclaimed), and memoises verification per distinct `(slot, pub, at, sig)` — an Ed25519 verify is ~3 ms in pure JS and the reap re-reads the whole map every 3 s, so without the memo a few dozen claims would stall the frame loop; the memo is pruned to the live map on every full read and reset per bind. `main.ts` injects `roomId`, `verifyIdentity`, `getIdentityPub` and `signIdentity` at the T0 seam; `world.ts` compares claims against `localStandPub()` (the pub the signer writes under — never a separate lookup), hoists one verified snapshot per walk-up instead of re-reading per slot inside the tier sort, and drops tracking when a renewal cannot be signed. New pure engine (`src/standClaims.ts`) + Yjs binding (`src/standsDoc.ts`) with 87 vitest cases — `standClaims.test.ts` (pure engine, 42) and `standsDoc.test.ts` (binding + real Ed25519 keys, 45) — covering shape-guard hostility (incl. the signed-shape rules: non-empty `sig`, safe-integer `at`, legacy shape refused), signature-bytes determinism and sensitivity to room / slot / pub / at, TTL/heartbeat semantics, the walk-up tier order (mine incl. mine-reserved → open civilian → reserved-if-owner), cross-doc convergence when two peers race the same slot, the release-ownership guard that keeps a stale wrap from dropping another peer's legitimate claim, the signed-claim layer (wrong-signer forgery, cross-slot and cross-room lifts, tampered `at` or `sig`, unsigned and legacy shapes, planted `Y.Map`s, a throwing verifier, and a lone-surrogate slot key / `pub` that passes the shape guard but throws inside the encoder all read as "no claim" without propagating; fail-closed with no verifier; self-checked writes incl. signer/verifier disagreement; the verifier memo running once per distinct record and re-arming on change and rebind; two-replica convergence with valid signatures on both sides), the online-pub provider used by the reap sweep, stand-switch cleanup so an aborted or replaced walk-up cannot leak an unreapable claim, the round-2 aborted-APPROACH releases (immediate clear, idempotent re-fire, re-tap onto a fresh slot, and race-safe non-drop of another peer's claim), and the round-3 heartbeat-ownership guard against a frozen-tab resume + the tier-alignment for stale-mine claims. Bound at the T0 seam in `main.ts` beside `bindGamesDoc`/`bindCasinoDoc`; the reap sweep reads the room's `players` map through an injected accessor (`getOnlinePubs` supplied at bind time — the entries' `keyB64` identity pubs, since that is what a signed claim carries) rather than the `__ssfDoc` debug window handle. Honest limits: proximity to remote avatars is still a backup filter so a peer that never posts a claim (a legacy build, or one whose claims we cannot verify) can't get walked through; signing stops forgery, not denial — any peer can still delete or garbage-overwrite a valid claim (the holder's 5 s heartbeat rewrites it; closing this needs signed CRDT ops at the transport layer); replay of a holder's own past claim is bounded by the 15 s TTL + skew window; any keypair may claim any slot (a signature proves the writer holds the key, not that they are at the table — the proximity filter is the backstop); and the periodic reap sweep is largely a safety net today — the players Y.Map is append-only (no `players.delete` call site), so a departed peer's presence entry sticks around on other replicas and the load-bearing recovery is `canPlayerClaim`'s stale-clause in the walk-up path, not the reap.
