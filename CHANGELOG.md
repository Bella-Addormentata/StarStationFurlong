...
- **#77 final-audit hardening — charge hysteresis latch + validated step editing (part of #77):** Two behavioural defects surfaced by the branch's max-scrutiny audit, both fixed. (1) The low-charge override released the moment the percent climbed back over the owner's threshold, so a resumed bot drained under it within a second and walked straight back — ping-ponging at the pad edge forever. The latch is now the extracted, pure `nextLowChargeLatch` (`src/robotCharge.ts`): arm on the low edge, HOLD until the bot is docked AND the displayed percent reads 100 — release keys off the ROUNDED display value so the console's "✅ FULL" badge and the bot's resume agree on the same frame (a raw-100 release would strand a visibly-full bot for up to ~9 s at the slowest legal charge rate). 9 regression cases pin the state table plus a tracker-driven replay of the exact pre-fix release frame. (2) The custom-script per-step editor wrote the rebuilt script RAW — no `isRobotStep` gate, unlike the sibling wheel-pacing and charge-envelope editors — so one out-of-envelope keystroke (coord past ±30, empty SAY text, WAIT past 60 s) committed an invalid step and `isRobotConfig` then refused the WHOLE config on every client: routine, script, pacing and charge envelope all silently reverting to defaults over a typo. Every per-step edit now routes through the new pure `patchScriptStep` (`src/robotScript.ts`) — per-kind field whitelist (no smuggled keys), blank-numeric refusal (`Number('') === 0` would silently zero a cleared coordinate), full `isRobotStep` validation, refuse-and-snap-back on rejection — and the step inputs carry native min/max/maxlength hints mirroring the envelope; 13 new cases. Full suite runs 204/204 (robotScript 59, robotCharge 36, robotDoc 12, treasuryTypes 67, treasuryDoc 30); `tsc --noEmit` clean.
- **Robot CHARGING beat is now engine-pure + owner-visible + script-composable (part of #77):** The dock's robot now has a real battery. A new dependency-free `src/robotCharge.ts` (no THREE, no Y.js, no DOM, no wall clock) carries the model — a `RobotChargeTracker` that discharges/charges at owner-tuneable rates, clamps dt to DT_CEILING so a tab-freeze can't slew the battery in one tick, and derives a per-frame `ChargeReading` (integer percent + charging/full/low flags). Determinism follows `RobotScriptScheduler`: same dt+docked sequence ⇒ same reading on every client. A bounds-guarded `ChargeParams` (discharge/charge seconds + low threshold) rides on `RobotConfig`; `isChargeParams` refuses anything outside its envelope on the doc trust boundary, and the tracker falls back to `DEFAULT_CHARGE_PARAMS` on rejection so a hostile peer can't strand the battery. Scripts compose: a payload-free `{ kind: 'dock' }` step reaches the render binding through a new `gotoDock` SchedulerAction — the target dock is resolved at render time, so a hostile owner can't teleport the bot outside the bounded goto envelope; `DOCK_STEP_NO_DOCK_SECS` keeps the loop unfrozen in a dockless room. Multi-dock claim uses a new pure helper `pickDockForRobot` (assigned dock honoured, else nearest, id-lex tie-break) so every client picks the same dock for each robot. Runtime: `poolWaiter.ts` advances the tracker each frame with a physically-docked flag, adds a low-charge override that forces the robot home and speaks a "recharging" line on cooldown, and honours a scripted DOCK step; `world.reconcileRobots` routes each robot's dock binding through `pickDockForRobot`, and `applyRobotRoutines` threads `chargeParams` to each bot. The dock console (`createRobotDockUI`) now shows a CHARGE pill (percent + charging/full/low badge) that refreshes per frame without a full re-render, and an owner-only rate-params editor with envelope hints. New tests: `src/robotCharge.test.ts` (27 cases — validators, discharge/charge determinism, dt-clamp, low-trigger, dock claim), `src/robotDoc.test.ts` (12 shape-guard cases including the chargeParams branch), plus dock-step coverage added to `src/robotScript.test.ts`. Full suite ran 182/182 at that slice (current totals live in the audit-hardening entry above); `tsc --noEmit` clean.
- **Robot scripting substrate is now engine-pure + owner-tuneable wheel pacing (part of #77):** The owner-programmed robot's step loop moves out of the render layer into a dependency-free `src/robotScript.ts` (no THREE, no Y.js, no DOM, no wall clock) — a deterministic `RobotScriptScheduler` with clamped dt, arrival + hard-timeout on `goto`, edge-triggered `say`, and JSON-fingerprint dedup so a mid-loop re-apply doesn't restart the cursor. Strict shape+bounds validators (`isRobotStep`, `parseRobotScript`, `isWheelTiming`) refuse coord/text/wait values outside their envelope on the doc trust boundary, so a hostile peer's payload can at worst waste cycles inside caps this module sets — no eval anywhere. `poolWaiter.ts` now delegates the custom-routine tick to the scheduler and only renders; `robotDoc.ts` re-uses the stricter step guard. A new bounds-validated `wheelTiming` (betting/closing/show seconds) threads from `RobotConfig` through `tickAutoCroupier` to the roulette table's phase deadlines, edited from the robot dock's console under the 'croupier' routine; `world.updateCroupier` picks the first dock's (id-sorted) timing so every client agrees on the pace. New vitest lane `src/robotScript.test.ts` — 38 cases; the full suite ran 135/135 at that slice; `tsc --noEmit` clean. **Honest limits:** the fitness-coach routine slice remains PR #97's; the drink-service and croupier duty behaviours themselves already existed pre-branch — this change makes the scripting substrate and roulette pacing owner-programmable, not those routines from scratch.
...
- **Cross-PR follow-up — unified T1 floor-occupancy claim (#77 / #97 / #126):** The reviewer's PR #125 cross-PR note (2026-08-21) flagged that #97's fitness-coach routine lazily treats "open floor nearest room centre" as a class stage while #126 adds CRDT-backed stand claims for table slots — two independent notions of "this floor is taken" that can double-book (a coach can hold a stage that a player's `pickStandForWalkup` believes is free). Neither PR needs to solve it alone, but SOMEBODY has to own the resolution: when both land, land a T1 **occupancy claim record** — one CRDT map keyed by floor cell / footprint id, whole-value LWW writes, a bounded TTL so an abandoned coach class doesn't wedge a stage forever, single-writer-per-key discipline (the claim's owner extends it, everyone else backs off) — and route both the coach's stage acquisition and `pickStandForWalkup`'s slot pick through it. The scheduler in this branch's `robotScript.ts` is off-chain by construction and does not need any of this today; the item is recorded here so the merge order that lands both routines cannot leave the double-book unresolved.
