...
- **Door policy D3.2 — revoked-grant replay defence (#67):** Closed the fail‑DANGEROUS revocation shape that dorkmo reproduced on PR #129 (2026‑08‑21). Before this pass `removeDoorGrant` did a bare `grantsMap.delete`, and signed grant bytes carried no sequence/expiry — a hostile peer that retained the pre-revocation grant bytes could re-`set` the same map key with the owner's original still-valid signature, and `hasDoorGrant` returned true, restoring privileges the owner actively withdrew. The fix follows the treasury's greatest-sequence-wins precedent (sovereign‑treasury‑serverless‑plan.md §5): (1) grants and revocation tombstones carry a monotonic `seq` inside their signed envelope (`doorGrantSignatureBytes` includes `seq` when the record has one, so a hostile peer cannot bump seq without owning the owner's key); (2) revocation becomes an owner‑signed **tombstone** record (`doorGrantTombstoneSignatureBytes`, domain tag `ssf-door-grant-tombstone:v1`) written to the SAME `${doorId}|${pub}` slot as the grant it withdraws — never a bare Yjs delete; (3) read rule: greatest verified `(seq, tombstone-tie-break)` wins per slot, tombstone at seq S defeats any grant with seq ≤ S; (4) each reader keeps an IN-MEMORY watermark of the highest verified observation per slot, so a subsequently-replayed lower-seq grant is refused even after a hostile peer CRDT-overwrites the slot. **Migration (documented, explicit):** legacy pre-D3 unsigned grants AND pre-D3.2 signed-seqless grants remain readable — treated as `seq = -1` by the read rule, so any seq-carrying record always outranks them; a hostile downgrade attack (strip seq to plant a legacy shape) is refused by the watermark. New signed grants require a non-negative integer `seq` (a fractional or negative seq is refused up front so a hostile peer cannot force a legacy-degrading verify path). **Trust boundary — honest residuals:** `seq` NARROWS the replay window; it does not close it. A peer that never observed the tombstone (fresh join after a hostile tombstone-delete + replay; a peer whose browser restarted and lost its in-memory watermark) will still accept a replayed grant — the tombstone key itself is deletable by any peer with map-write access. Closing this fully requires signed CRDT operations (durability C6) or a chain-anchored tombstone log (chia-authority-architecture), both out of scope for D3.2. `removeDoorGrant` in signed mode refuses a non-owner caller (would plant an unsigned tombstone that verify-on-read drops but that still CRDT-clobbers the grant); a legacy binding (no verifier / no signer) keeps the pre-D3 bare-delete semantics. Vitest suite grew to 58 doorPolicy tests (17 new D3.2 cases): reviewer's exact repro (now passing), tombstone-beats-replay at equal seq, higher-seq-regrant-after-tombstone, cross-door / cross-room / cross-pub non-interference, legacy-grant + signed-tombstone interaction, signed-grant-vs-legacy-downgrade forgery resistance, fake-seq/negative-seq/non-integer-seq refusal, tombstone-with-invalid-signature drop, non-owner-in-signed-mode refusal, legacy-binding preserves bare-delete, and the DOCUMENTED RESIDUAL. Full suite: 155/155 pass; `npx tsc --noEmit` clean. Additive fields only — no runtime API surface change to `docking.ts` or `main.ts` beyond the existing `bindDoorPolicy` seam. No version bump.
- **Door policy D3 — signatures under owner authority (#67):** doorPolicy, doorGrants and doorRequests records now carry optional ed25519 signatures via the same primitives the treasury cache uses (domain-tagged SSF deterministic CBOR bytes, `verifyIdentity` verifier, `signIdentity` signer). Each signature is scoped to `{roomId, doorId}` — cross-room and cross-door replays refuse. Owner-authored records sign on write when the local player IS the current room owner (live-read via `roomInfo.owner → players.get(id).keyB64`); guests self-sign their own request records. Verify-on-read drops non-verifying, mis-scoped, or tampered records so a hostile peer cannot forge an owner-authored policy or an owner-issued grant. **Fail-safe fallback per the issue spec:** a record with NEITHER signature field is honored as legacy (pre-D3 stations keep working, no schema bump). A record with only ONE of the two fields is refused (defence against accidental partial writes). **Map-key discipline (grants/requests):** every grant/request is keyed at `${doorId}|${pub}` in its map; the map key is the authoritative slot identity, and reads verify signatures against the KEY's doorId (not the record's carried `v.doorId`). A record whose carried `doorId`/`pub` does not match its slot is dropped — this closes a strong cross-door/cross-pub replay where a hostile peer with map-write access could copy an owner-signed grant UNMODIFIED to a foreign key (leaving `v.doorId` intact so the original signature still verified against the record's bytes) and thereby grant themselves construction rights on a door the owner never approved. `readDoorGrants`/`readDoorRequests` now iterate `entries()` and drop mismatched slots for legacy records too, killing the duplicate-listing side-effect in the same fix. **Trust boundary honesty:** D3 prevents forgery of owner-authored content but does NOT prevent a hostile peer that can write the doc from OVERWRITING a valid signed record with garbage (Yjs LWW rule) — verify-on-read then falls back to defaults rather than to the owner's previous policy. Signed CRDT operations that would close this durability hole are C6 (out of scope). Additive fields only; no runtime API surface change to `docking.ts` or `main.ts` beyond the extra `bindDoorPolicy` options passed at the T0 seam. Vitest suite added — `src/doorPolicy.test.ts`, 41 tests covering signed round-trip, hostile forgery, tampered payload, partial-sig refusal, cross-room replay, cross-door replay (including the strong UNMODIFIED-lift attacks against `hasDoorGrant`/`hasDoorRequest`), cross-pub replay, legacy-record lift refusal, unparseable-map-key drops, legacy fallback, owner rotation, grant/request parity, and D1/D1b pure-logic gating (shape guards, enum coercion, doorId scoping). All 138 tests pass; no version bump.
