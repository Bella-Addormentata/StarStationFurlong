...
- **Door pairing D3 — round-5 audit-remediation, signed far-side lateral placement (#67):** `farLateral` (the far-side lateral placement offset, ±32 m) was the last raw number riding into the pairing signature envelope — both `doorsDoc.ts` sig-bytes builders (`doorPairingSignatureBytes` owner, `doorPairingGuestSignatureBytes` guest) forwarded `orNull(p.farLateral)`, the raw value. `sanitizeDoorGeometry` accepts ANY finite `|farLateral| ≤ 32` including fractional, so a conforming fractional offset (e.g. a `0.5 m` nudge off a wall) reached `treasuryTypes.canonicalEncode`, which refuses non-safe-integer numbers (`only safe integers are encodable (no floats)`) — the same fail-DANGEROUS class as the round-4 segment defect. On the owner's ACCEPTED-handshake re-sign the throw was swallowed by `world.ts`'s `try { ... } catch (e) { console.error(...); }`, the map slot stayed empty, and every reader silently saw the door as unpaired — a griefable owner-write DoS reachable by any peer that plants a fractional-`farLateral` legacy record the owner then re-signs. Fix: new `farLateralForSig` helper accepts EXACTLY the window `sanitizeDoorGeometry` keeps — finite AND `|farLateral| ≤ 32` — quantizing in-range values ×1000 to a fixed-point MILLIMETRE safe integer (`|32000|` sits far inside `MAX_SAFE_INTEGER`) and folding everything else (non-finite `NaN`/`±Infinity`, or out-of-range) to `null` rather than clamping or crashing the encoder; both builders emit `farLateralMm` through the one helper so signer and verifier round through byte-identical output. Module-header PREVENTED note now names `farLateral` (millimetres, alongside stretch). Trust boundary: a lateral shift above the 1 mm quantization floor invalidates the signature; sub-millimetre tweaks slip through but are visually indistinguishable at vestibule scale. Because the sign helper mirrors the sanitizer's DROP (not a clamp) at the `|v| > 32` edge, a signed boundary offset (exactly ±32 m) cannot be laundered to a larger out-of-range value that keeps the signature valid while the reader silently drops the field to "centred" — such a tamper changes the rebuilt bytes, so verify refuses the record (fail-safe to unpaired) exactly as it does for any in-range shift. Regression tests (`src/doorsDoc.test.ts`, 5 new attack-labeled tests): fractional `farLateral: 1.5` round-trips; a `1.5 → 2.5` tamper on a planted record refuses; a hostile `NaN` does not crash the encoder and folds to `undefined` on read; a keyless LEGACY `farLateral` plant does not throw the owner re-sign; a signed ±32 boundary offset tampered to an out-of-range `33` is refused (clamp-laundering closed). Full suite 208/208 pass. `npx tsc --noEmit` clean. No API surface change.
- **Door pairing D3 — round-4 audit-remediation, RING preset segments (#67):** `doorsDoc.ts`'s `segmentForSig` forwarded raw fractional `bendDeg` / `stretch` values to `treasuryTypes.canonicalEncode`, which refuses non-safe-integer numbers (`only safe integers are encodable (no floats)`). Every RING-preset pairing (`{kind:'flex',bendDeg:22.5,stretch:0}` x2 with a solid ext bay bank — one of the two shipped octagon-connector presets) threw on the sig-bytes build inside signed binding. The two production `writeDoorPairing` call sites (`world.ts`'s ACCEPTED-handshake owner path and `main.ts`'s cross-room mirror) wrapped the write in a `try { ... } catch (e) { console.error(...); }`, so the throw was swallowed, the map slot stayed empty, and every reader silently saw the door as unpaired — no UI feedback, no gossip, no docked ship. Same fail-DANGEROUS class as the D3.2 unsigned-tombstone-in-signed-binding pattern PR #129 closed. Fix: `segmentForSig` runs the same clamp helpers the sanitizer already uses (`clampFlexBendFine`, `clampFlexStretch`, `clampExtBays`, `clampExtStretch`) with a `Number.isFinite` guard on every raw input, then quantizes flex bend to deci-degrees and both stretches to millimeters (bays already integer via `Math.round` inside `clampExtBays`) — every value lands well inside the safe-integer range across the whole clamp window, both the signer and the verifier round through the same helper for byte-identical output, and hostile non-finite floats (`NaN`, `±Infinity`) fold to the safe clamp default rather than crashing the encoder. Trust boundary updated: segment tampering above the 0.1° / 1 mm quantization floor invalidates the signature; sub-quantization tweaks slip through but are visually indistinguishable at the vestibule scale (0.1° at ~2.4 m gangway ≈ 4 mm arc). Regression tests (`src/doorsDoc.test.ts`, 6 new attack-labeled tests): shipped RING preset round-trips; shipped SPOKE preset round-trips (integer-only boundary); flex-bend 22.5°→30° tamper refuses; ext `bays` 4→8 tamper refuses; hostile `NaN`/`Infinity` segments do not crash the encoder and verify because both sides canonicalize to the same safe default; guest-signed transient with the jetbridge-solver preset (continuous 40.1°, 0.001 m stretch) round-trips. Full suite 203/203 pass. `npx tsc --noEmit` clean. No API surface change.
- **Door pairing D3 completion — signed pairing/tombstone records (#67):** `doorsDoc.ts` extends the D3 signed-record discipline (already shipped for `doorPolicy`, `doorGrants`, `doorRequests`, and grant-tombstones) to the last remaining door-authority records: PAIRINGS and their TOMBSTONES. New domain tags — `ssf-door-pairing:v1` (owner-signed non-transient pairings), `ssf-door-pairing-guest:v1` (guest-signed transient berths), `ssf-door-tombstone:v1` (owner-signed permanent undocks + owner-authored detaches), `ssf-door-tombstone-guest:v1` (guest-authored detaches) — reuse the exact `treasuryTypes.canonicalEncode` primitive already in production. Each side signs its own writes: the room owner signs non-transient pairings and permanent-undock tombstones with the identity key; the LOCAL player self-signs guest-side transient berths (guest identity keypair) and their either-side detach tombstones (guestPub rides INSIDE the signature envelope so a hostile peer cannot swap the guest identity on a captured record). Reads verify against the map key's `doorId` (map-key discipline — cross-door replay refuses even for records copied UNMODIFIED to a foreign slot) and drop signature-forgery/mis-scoped/tampered records; legacy pre-D3 pairings (no sig fields) remain accepted as fail-safe fallback so pre-D3 rooms keep working across the transition. **New `detachTransientBerth(doorId)` API** replaces the pre-D3 bare-`deleteDoorPairing` from docking.ts's "detach-berth" action — that bare delete was the pairing analogue of PR #129's revoked-grant replay defect: any peer that retained the pre-detach paired bytes could re-`set` the map slot after the detach and restore the berth on every reader. `detachTransientBerth` writes a signed tombstone at the slot (owner-signed if local == room owner, guest-signed by the local player otherwise), and the seq inside the signed envelope + the reader's in-memory `${roomId}|${doorId}` watermark defeats the replay for peers that observed the tombstone. **Fail-loud writes (D3.2 pattern):** `writeDoorPairing`, `writeDoorTombstone`, and `detachTransientBerth` THROW in signed binding when signing was required and failed (canonical-encode overflow past `MAX_SAFE_INTEGER`, or the injected signer erroring/returning null) — surfaced at every call site (`world.ts`'s pairing publisher, `main.ts`'s cross-room mirror, `docking.ts`'s detach-berth / undock-module actions) so the caller can log rather than leave the map in a poisoned state. **PR #129 audit-lessons applied up front:** `Number.isSafeInteger`-grade shape guards for `seq`; legacy pairings MUST have no `seq` field (a "no sig but seq present" record is refused up front to defeat watermark-inflation attacks); UNSIGNED tombstones in signed binding are refused (a hostile peer cannot revoke a signed pairing via an unsigned tombstone); records with BOTH owner AND guest sig fields are refused as malformed. **Guest-only-transient enforcement:** a guest signature over a non-transient pairing shape is treated as forgery — guests may only author transient berths (adapter policy is the owner's consent to that write class). **Documented residual — cross-room mirror-write:** `main.ts`'s post-arrival lazy-mirror path writes into the ARRIVAL room's doc from a TRAVELER who is not the arrival-room owner; the traveler cannot authoritatively sign for that room, so mirror records land UNSIGNED as fail-safe legacy fallback (a hostile could equally have written the same shape today; a future slice can carry a departure-owner attestation to close it). **Trust boundary — honest residuals (mirroring the D3.2 boundary):** verify-on-read prevents forgery but NOT hostile map-overwrite (Yjs LWW rule — the slot falls back to unpaired, not to the previous authoritative pairing); `seq` narrows detach-then-replay but does not close it for peers that never observed the tombstone or lost their in-memory watermark to a browser restart; closing these fully requires signed CRDT ops (durability C6) or a chain-anchored tombstone log, both out of scope for D3. **Vitest suite:** `src/doorsDoc.test.ts`, 33 attack-labeled tests (owner-signed forgery/cross-room/cross-door/tampered/partial; guest-signed transient-only + identity-swap + dual-authorship; seq+watermark detach-then-replay + unsigned-revocation + monotonicity + legacy-with-seq refusal; fail-loud on null-signer/throwing-signer; map-key discipline; free `d:` id acceptance). Full suite 197/197 pass. `npx tsc --noEmit` clean. Additive fields only — no runtime API surface change beyond the new `bindDoorsDoc(doc, opts?)` seam (opts omitted = legacy binding = pre-#67 behavior) and the new `detachTransientBerth` export. No version bump.
- **Door policy D3 + D3.2 — signed enforcement and revoked-grant replay defence (#67):** doorPolicy, doorGrants, doorRequests, and (new in D3.2) revocation tombstones now carry ed25519 signatures via the same primitives the treasury cache uses (domain-tagged SSF deterministic CBOR bytes from `treasuryTypes.canonicalEncode`, `verifyIdentity` verifier, `signIdentity` signer). Signatures scope to `{roomId, doorId}` — cross-room and cross-door replays refuse. Owner-authored records sign on write when the local player IS the current room owner (live-read via `roomInfo.owner → players.get(id).keyB64`); guests self-sign their own request records; verify-on-read drops non-verifying, mis-scoped, or tampered records. **Fail-safe fallback per the issue spec:** a record with NEITHER signature field is honored as pre-D3 legacy (existing stations keep working, no schema bump); a record with only one of the two fields is refused. **Map-key discipline (grants/requests):** each record is keyed at `${doorId}|${pub}`; the map key is the authoritative slot identity, and reads verify signatures against the KEY's doorId — a record copied UNMODIFIED to a foreign key fails verify. `readDoorGrants`/`readDoorRequests` iterate `entries()` and drop mismatched slots for legacy records too, killing a duplicate-listing side-effect in the same fix. **D3.2 revoked-grant replay defence** closes the fail-DANGEROUS revocation shape dorkmo reproduced on PR #129 (2026-08-21): `removeDoorGrant` no longer bare-deletes, and signed grant bytes now carry a `seq` — a hostile peer that retained the pre-revocation bytes can no longer re-`set` the same slot with the owner's original still-valid signature. Following the treasury's greatest-sequence-wins precedent (sovereign-treasury-serverless-plan.md §5): (1) grants and tombstones carry a monotonic `seq` inside their signed envelope (`doorGrantSignatureBytes` includes `seq` when the record has one, so a hostile peer cannot bump seq without owning the owner's key); (2) revocation becomes an owner-signed **tombstone** record (`doorGrantTombstoneSignatureBytes`, domain tag `ssf-door-grant-tombstone:v1`) written to the SAME `${doorId}|${pub}` slot as the grant it withdraws — never a bare Yjs delete; (3) read rule: greatest verified `(seq, tombstone-tie-break)` wins per slot, tombstone at seq S defeats any grant with seq ≤ S, a higher-seq regrant restores rights; (4) each reader keeps an IN-MEMORY watermark of the highest verified observation per slot, so a subsequently-replayed lower-seq grant is refused even after a hostile peer CRDT-overwrites the slot. **PR #129 audit hardening (pass 2):** the reviewer surfaced three defects on the D3.2 land, all now closed with regression tests locked. **BLOCKER-1:** `isValidSignedTombstone`'s `!hasPub && !hasSig ⇒ return true` short-circuit ran BEFORE the `!verifier` check, so in signed binding a hostile peer could plant an unsigned tombstone `{tombstone:true, doorId, pub, revokedAt, seq: 999}` at any slot and every reader treated it as authoritative revocation AND bumped its watermark — forcing every subsequent owner regrant to burn seq up to (and past) `Number.MAX_SAFE_INTEGER`. Post-fix, unsigned tombstones are refused when a verifier is wired; legacy binding still accepts the shape (there is no signature layer to protect there anyway). **BLOCKER-2:** `writeDoorGrant`/`removeDoorGrant` swallowed signer throws (canonical-encoding overflow at seq > `MAX_SAFE_INTEGER`, or a broken injected signer) with `try/catch ⇒ sig = null` and silently wrote an UNSIGNED record — the caller and every reader then diverged (the owner UI showed "revoked"; every peer still verified the grant). Post-fix, both functions THROW in signed binding so the owner UI can surface the failure instead of leaving a poisoned or split-brain slot. **MAJOR:** legacy-grant validation read `v.seq` at face value, contradicting the module header's `seq = -1` promise — a hostile unsigned record with a huge seq inflated the reader watermark and pushed subsequent owner writes into oversized (or unsafe-integer-crashing) territory. Post-fix, legacy grants MUST have no seq field; the "no sig but seq present" shape is malformed and refused, and `isTombstoneShape` uses `Number.isSafeInteger` (up from `Number.isInteger`) so `MAX_SAFE_INTEGER + 1` cannot slip through the shape gate. **Migration (documented, explicit):** legacy pre-D3 unsigned grants AND pre-D3.2 signed-seqless grants remain readable — treated as `seq = -1` by the read rule, so any seq-carrying record always outranks them; a hostile downgrade attack (strip seq to plant a legacy shape) is refused by the watermark. New signed grants require a non-negative SAFE integer `seq`. **Trust boundary — honest residuals:** verify-on-read prevents forgery but does NOT prevent a hostile peer with map-write access from OVERWRITING a valid signed record with garbage (Yjs LWW rule) — verify-on-read then falls back to defaults rather than to the owner's previous policy. `seq` NARROWS the replay window; it does not close it. A peer that never observed the tombstone (fresh join after a hostile tombstone-delete + replay; a peer whose browser restarted and lost its in-memory watermark) will still accept a replayed grant — the tombstone key itself is deletable by any peer with map-write access. Closing these fully requires signed CRDT operations (durability C6) or a chain-anchored tombstone log (chia-authority-architecture), both out of scope for D3/D3.2. `removeDoorGrant` in signed mode also refuses a non-owner caller (would plant an unsigned tombstone that verify-on-read drops but that still CRDT-clobbers the grant); a legacy binding (no verifier / no signer) keeps the pre-D3 bare-delete semantics. Vitest suite: `src/doorPolicy.test.ts`, 67 tests (41 D3 + 17 D3.2 + 9 PR #129 audit regressions — reviewer's exact repro, unsigned-tombstone-refused-in-signed-mode + legacy-binding-still-accepts-shape, writeDoorGrant/removeDoorGrant throw on null-signer / throwing-signer / MAX_SAFE_INTEGER seq overflow, legacy-with-seq refused + truly-legacy still reads, tombstone-at-MAX_SAFE_INTEGER-plus-one refused up front). `npx tsc --noEmit` clean. Additive fields only — no runtime API surface change to `docking.ts` or `main.ts` beyond the existing `bindDoorPolicy` seam. No version bump.
