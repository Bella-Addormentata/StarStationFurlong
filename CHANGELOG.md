...
- **Docking pane type honesty for the "any door can pair" design (#101 follow-up):** the private cardinal predicate that used to shadow `doorLayout`'s hoisted `isCardinalDoorId` retired — docking.ts now imports the one canonical copy, so every narrowing site sees the same guard the compiler already proves elsewhere. `completePairing`, `receiveInboundPairingRequest`, `animateBlinkingIndicator`, `discardUntouchedPrefill` and `editChainSegment` widened from cardinal-only signatures to `string` — a lie the runtime already bypassed via the pane's stringly-keyed `activeDoorId` stash, and one that would have re-fused free `d:` doors out of the "any door can pair" feature (94df5c6) had it stayed. Every `(pane as any).activeDoorId` read is now typed via `{ activeDoorId?: string }`; the earlier duplicate `activeDoor` helper hoisted to the top of `setupPanelListeners` so INITIATE, ACCEPT and REJECT stop bypassing the compiler with per-callsite `any` casts. Stale comments claiming "cardinal-only" pose or a "simplified free-door panel" refreshed to describe the unified pane. New `src/doorLayout.test.ts` (26 tests) pins `isCardinalDoorId` strictness (no case fold, no substring, no wall-label impostors), the `physicalDoorPose` fallback that keeps the #91 `d:` crash unreachable, `poseFromWall`'s four-wall geometry, and the #91 grid invariants. **Honest limits.** The unreachable INITIATE/ACCEPT crash path issue #101 called out remains a crash-hardening exercise, not a live-bug fix — I could not reproduce it either. The DOM handlers themselves are not test-covered (the codebase has no pane harness); their honesty is now enforced through the type system, and the guarded downstream helpers (`projectionPoseForDoor` → `physicalDoorPose`) already degrade safely on a free id.
