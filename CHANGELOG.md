...
- **Recovery-key backup nudge — prompt BEFORE the identity accrues value (#79 R1, PR #124 review 💡).** The Ed25519 seed in `localStorage` IS the identity, and the Contacts app could already reveal it for backup — but nothing ever *prompted* a player to do so. A new pure decision engine (`src/recoveryNudge.ts`) now tracks, per identity, the FIRST moment the identity accrues something a lost device would take with it — a deed that provably resolves to our pub (`syncDeedsLedgerFromCurrentRoom` through the new `currentRoomDeedClaim` resolver — the legacy `Local-Clone` leg the deeds ledger accepts is excluded, since every guest in such a room passes it), a door build-rights grant (the `subscribeDoorPolicy` listener), a co-host seat (the `subscribeRoomRoles` listener) or venture shares (`syncVentureLedgerFromCurrentRoom`) — and whether the recovery key has ever been *in the player's hands* on this install (revealed in Contacts, or pasted in a successful Contacts / title-screen restore). Value with no key in hand arms a top-centre in-room banner (`src/hud.ts` → `#hud-recovery-nudge`, gated by the same `body.in-room` / `pre-entry` / `exterior-active` classes as the SpacePhone tip, z-index under the phone) with **BACK UP NOW** (raises the phone on Contacts with the Backup / restore panel open and the reveal button focused — it never reveals the key itself), **LATER** (24 h snooze) and **DON'T ASK AGAIN** (permanent for that identity); a one-time phone-system log line marks the moment; and the home-screen identity row carries a standing amber `⚠ NO BACKUP` cue (a fact, not a nag — DON'T ASK AGAIN never silences it). Markers are pub-scoped, so a restore never inherits the previous identity's snooze or "in hand" state, and the seams sit on the existing sync points plus a T0 sweep of the grants / co-host maps at every join (`noteIdentityHoldingsInCurrentRoom`: on a session's first join the bind-time notify runs before those subscribers exist, and a Tier A cache-restored doc never re-observes what it already holds), so holdings that pre-date the nudge are noticed at the next join. Chips are deliberately NOT a trigger: they are playerId-keyed, so a backup would not carry them. **Recovery-key discipline unchanged:** the nudge never reads, logs, or moves the key — it stamps timestamps only, in a separate `ssf-recovery-nudge` record, best-effort behind a write-through cache so a privacy-mode or full storage still yields one nudge per session (never a re-log loop). **Honest limits:** "shown" ≠ "safely stored" (unknowable here — the copy says the key was shown, never saved); markers are per install, so the same identity on a second device is nudged there too; an existing install whose value pre-dates the nudge sees it once at its next join; a lapsed snooze re-shows at the next join or holding change, not at the exact instant; the first-value stamp stays once set even if that holding is later given away. New vitest coverage: `src/recoveryNudge.test.ts` (40, hermetic in-memory store) → 221/221 green; tsc clean; no APP_VERSION bump, no new dependencies. Confirming audit (max scrutiny, read-only): two LOW findings — the first-join subscriber-ordering gap and the legacy-leg over-fire — both folded in above; its code-smell note (the phone-raise hook referencing a helper declared later in the closure) folded in too.
- **First-boot atlas seed + tested title-screen boot-order controller (#79 P6).** A brand-new install now renders the WHOLE shared station on its very first boot instead of the previous "single joined module" view. A baked cardinal-shell layout (`STATION_ATLAS_SEED` in `src/station.ts` — hub Furlong Station + four spoke stubs Atrium/Market/Observatory/Lounge, geometry + names only) is folded into the local station-atlas store at boot by a new pure merger (`mergeFirstBootAtlas`) plus a thin localStorage adapter (`applyFirstBootAtlas`); the merge is non-destructive so a returning install's visited entries win, and every seed entry carries a `lastSeen: 0` sentinel that a new `pushAtlasToDoc` guard refuses to publish — so no fabricated geometry ever reaches the shared atlas of the station. The title-screen boot order (owner spec: "NEW PLAYER + LOAD FROM BACKUP before the atlas view; existing user fades straight to last location") is now driven by a new pure controller `decideInitialStage(hasStoredIdentity)` in `src/bootFlow.ts`: first-run yields `idle` (choice shown), returning install yields the terminal `proceed` (curtain arms once dwell + exterior are ready). `main.ts setupClickToEnter` derives `proceedChosen` from the stage kind so the boot-flow logic has ONE source of truth. **Security invariant respected:** the baked seed carries GEOMETRY AND NAMES ONLY — no `targetSeed` credentials ride in `STATION_ATLAS_SEED`, and the pure controller takes a `boolean` (never the identity key itself). New vitest coverage: `src/stationAtlas.test.ts` (14, NEW) + expanded `src/station.test.ts` (14, was 8) + expanded `src/bootFlow.test.ts` (29, was 24) → 181/181 green; tsc clean; no APP_VERSION bump, no new dependencies.
- **Load-from-Backup gets an explicit CONFIRM step + tested boot-title state machine (#79 P5).** The owner-spec three-step restore ("text box to paste key, then confirmation then load atlas") now lives as a pure state machine (`src/bootFlow.ts`): PREVIEW derives the identity fingerprint from the pasted key WITHOUT applying it (new `previewRecoveryKey` in `src/keypair.ts` — same input-validation contract as `importRecoveryKey`, no state mutation), a distinct CONFIRM RESTORE / CANCEL panel shows the fingerprint before the seed replaces the running identity, and only CONFIRM calls `importRecoveryKey`. `bootFlow.ts` also encodes the tested contract for the boot-target priority (`override → last-room → clone-preference → shared-station → per-install-home`, with station-pointing entries deliberately deferred to the station-default rung) that `fetchDefaultBootstrap` + `bootstrapNetworking` implement. Recovery-key discipline unchanged: the pasted key stays in the textarea only, is scrubbed on Confirm and on Cancel, is never logged/transmitted, and only Confirm ever writes it via `importRecoveryKey`. **Honest limits:** the runtime `fetchDefaultBootstrap` still holds its own priority-ordering code; `bootFlow.decideBootTarget` is the tested spec for that behaviour (drift would show up as a test failure) but the two are not wired to a single source — that consolidation is Phase-2 alongside the mesh-published clone-vat lookup. New vitest coverage: `src/bootFlow.test.ts` (24) + expanded `src/keypair.test.ts` (27, was 20) → 156/156 green; tsc clean; no APP_VERSION bump.
- **Fresh installs land in the shared station (#79 P3, dev-only wiring).** The bake-once default station id + room key are now filled in (`src/station.ts` — `station-furlong-alpha` + a random 32-byte key), and the boot flow prefers them over the per-install `home-*` when there's no override and no persisted `ssf-last-room`. A shared-station join is force-classified as a JOIN (never claims owner defaults, avoiding a race with the host's SyncStep2), and neither `ssf-last-room` nor the per-identity clone-vat preference persists the station id (it's a rebuild-from-defaults room like `home-*`). A per-identity clone-vat preference (`ssf-spawn:<pubkey>`, shape-guarded, whole-value LWW) records the last non-home / non-station room and re-targets the next boot at that room; the LOAD-FROM-BACKUP restore path clears `ssf-last-room` so the restored identity boots into its preference or the station default, never the previous identity's leftover resume. **Honest limits:** `STATION_SEED_HINTS` is still empty (no always-on host stood up yet) — installs sharing the station id converge only when they meet a peer directly (LAN mDNS today, DHT/relay once configured); the clone-vat preference is LOCAL-only until a mesh-published preference lands in Phase 2. Recovery key handling is unchanged: it lives only in `localStorage` via `importRecoveryKey`, is never logged, transmitted, or stored anywhere new, and is scrubbed from the paste box the moment it's applied. New vitest coverage: `src/station.test.ts` (8) + `src/keypair.test.ts` (20) → 125/125 green; tsc clean; no APP_VERSION bump.
