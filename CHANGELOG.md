...
- **🖼️🩹 Walls migration completeness pass — legacy freestanding walls now READ-REPAIR onto the hull (Part of #80 S6).** The `df26789` "wallpaper editor + retire freestanding walls" commit removed `brick-wall` / `window-wall` from `FURNITURE_DEFS`, so `isFurnitureRecord` now silently drops those records on read — a room-doc from a pre-migration client would render as bare hull colour on the side walls the owner had painted with segments. This slice closes that gap the way #80 asks: read-repair AT READ TIME, no doc rewrite. A new pure module `src/wallpaperDerivation.ts` scans RAW furniture values (via new `readRawFurnitureValues()` on `furnitureDoc.ts`) for retired wall kinds, maps each to the hull surface it covered (side wall on the narrow axis, modeled on the old `updateSideWallCoverage` proximity rule with the margin deliberately widened — 1 m strict → 1.5 m inclusive — so a legacy segment nudged slightly off flush still repairs), and hands `world.collectWallpaper()` an implicit `HullWallpapers` map that the world merges UNDER the explicit `wallpaperLayoutDoc` — so an owner who has since painted a different covering wins, but an untouched legacy room STILL shows its brick. Both retired kinds map to the existing `brick` wallpaper preset (same 0x8a4a3a base colour) — honest limit: a `window-wall`'s GLAZING is not derived (the module writes no window records), so only the brick frame survives until the owner re-adds panes with the octagon's window editor. Shape-guarded at the peer trust boundary (`isLegacyWallRecord` — non-object / unknown kind / non-finite coords fail SAFE with no derivation, never a crash or fabricated covering). Furniture-doc changes now trigger a hull rebuild only when the DERIVED covering set actually changes (fingerprint compare) — ordinary furniture moves cost nothing. Verified iso/first-person rendering rules already ship in main (`updateFacing`: near walls hidden + far walls opaque in iso; walls near-solid + roof translucent in first person). Covered by 37 new vitest cases in `src/wallpaperDerivation.test.ts` — 30 pure-function (shape guard, surface mapping edge thresholds, narrow-axis-z rooms, hostile-input safe-fail, merge precedence) + 7 integration (`bindFurnitureDoc` + Y.Doc round-trip proving `readAllFurniture` drops legacy records while `readRawFurnitureValues` surfaces them; preset-reachability + hull-surface-key sanity so the migration cannot silently paint the roof/basement or unwind the door/window pipelines). 166/166 tests pass; `tsc --noEmit` clean.
- **🛑📐🏊🕳️ Pool basin lives in the REAL basement now (Part of #80).** The pool floor opening and its basin no longer render into imaginary space below the floor: the basin bottom sits ON the octagon basement floor (y=−basementDepth) and long-axis basin walls close off the extruded basement "trench" past the water. The narrow-axis sides use the hull's own 45° chamfer as a naturally sloped basin wall, so no duplicate wall clashes with the chamfer. The pool water outline (poolHoleOutline / poolHoleRect / poolHoleCells) is now CLAMPED to ±halfAtWater on the room's narrow axis so the floor hole never crosses the chamfer at surface level — a safety belt for small rooms. Both `pool-1` (lazy) and `pool-2` (classic) templates share the same basin discipline via one new `placeOctagonPoolBasin` helper + one `poolBasementBoundsFor(rot)` bounds derivation, so "block depths near the 45-degree chamfer edges" has one definition. **Follow-up remediation (pass 1, 2026‑08‑21):** the long-axis basin walls now span the FULL water extent on the narrow axis (previously clipped to `halfAtFloor`, which left the "wing" regions `|narrow| ∈ [halfAtFloor, halfAtWater]` visually open past `z=±waterZHalf` into the extruded wedge cavity — the audit's finding-1 leak); the plate-thickness comment now honestly describes the plate STRADDLING `y=basementY` rather than sitting entirely above it (bottom face 0.02 below, top face 0.06 above — Z-fight still avoided because the top face is what a swimmer sees); the pool builders pass the ITEM's rot into `poolBasementBoundsFor` (matches `poolHoleRect`/`…Outline`) so a future rotated pool stays coherent; and the basin geometry is now derived by a new pure `computePoolBasinGeometry(fp, bounds)` helper so the renderer's slab dimensions/positions are unit-testable without a THREE.js runtime. Covered by 32 vitest cases (21 originals + 11 new for the geometry helper) in `src/furniturePoolBasin.test.ts`.
