...
- **Phone contacts BLOCK + group chats (issue #20 slice — client-side only):** the CONTACTS app grows a red ✕ BLOCK button on every contact row (with a `confirm()` prompt), a `🚫 BLOCKED` section that lists everyone you've muted and offers ↺ UNBLOCK, and a bounded per-install block list persisted at `localStorage['ssf-blocked-pubs']`. Blocked identities are hidden from the CLONES SEEN roster (with a "N hidden by your block list." footer note), their chat and world overhead bubbles are filtered out, and the log surfaces a "N messages hidden" system line whenever the filter fires. The CHAT app grows a horizontal thread picker above the log — 🌐 ROOM by default plus one 👥 chip per group you're a member of, and a `+ NEW GROUP` chip opens a friend-picker + optional title. Group messages ride the existing room `chat` array tagged with `groupId`; thread records live in a new `groupChats` Yjs map, keyed by a deterministic sha512-of-sorted-members id, shape-guarded on read (id-must-match-members hijack check, creator-must-be-a-member, AND `isValidThreadEntry` requires the map key to equal `value.id` so a hostile peer's mismatched-key entry cannot produce a chip whose activation silently misroutes sends to the ROOM lane), and bounded by `MAX_GROUP_CHATS = 64` with `GROUP_CHAT_CAP = 200` per-thread messages. The room lane keeps its parallel `ROOM_CHAT_CAP = 200` — trimmed by `excessRoomIndices` on room-only messages so a burst of room activity cannot evict older group history from index 0 (the pre-fix `sharedChat.length - 200` sitting on the WHOLE array had that gap). Every new chat message now stamps `authorPub` (canonical identity pub) alongside the legacy `authorId` so the filter matches without re-resolving through the players map. Honest limits: this is **local mute + private-by-UI**, not enforcement — blocked bytes still arrive over the mesh, every room member replicates every group's messages, and the blocked party is not notified. A signed on-doc block op and an E2E-sealed group thread remain later TrustAndSafety / RoomLog work (see `brainstorming/phone-apps-breakdown.md` §3.2 and §5). Two new vitest modules cover the pure filtering/threading/cap logic (`src/blockList.test.ts` — 19 tests; `src/groupChat.test.ts` — 49 tests; 68 tests total across the slice). Frontend-only; no node behavior, no version bump.
