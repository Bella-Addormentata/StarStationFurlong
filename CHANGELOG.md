...
- **BANK send/receive — in-world chips between two players (issue #20 slice — client-side only, game-first):** the BANK app grows a `SEND CHIPS` button inside the room's CHIPS section and a `TRANSFERS` audit block (Received / Sent) beneath it. Recipient list is the current room's players map minus yourself and minus anyone on your local block list — legacy players without a `keyB64` are hidden from the picker (a signed transfer needs a pub for both parties). Two-step confirmation (first click on `SEND` arms, second click on `CONFIRM SEND` writes) mirrors the deed-handover pattern; any change to picker/amount re-arms. Refusals speak plain-language: `"Not enough chips — you have N, tried to send M."`, `"Pick a recipient from the list."`, `"Enter a whole number of chips greater than zero."`, and (on a race with a concurrent bet burning the balance between arm and send, or the room's capacity cap) `"Send refused — either your chips just moved, or you've hit the per-room send limit. Try again in a moment."`. A new module `src/casinoTransfers.ts` is the pure engine (no Yjs, no `@noble` import — verifier injected): domain-tagged deterministic sha512 id (`ssf-chip-xfer-id:v1`), shape guard (`isChipTransfer` — kind + version + every field type/range, self-transfer rejected, id must recompute from the candidate fields), entry guard (`isValidChipTransferEntry` — chains the value guard AND requires KEY === value.id so a hostile peer's mismatched-key entry cannot smuggle a phantom row past the audit log), Ed25519 signature over domain-tagged canonical bytes (`ssf-chip-xfer:v1`), deterministic full-ledger replay (`partitionValidTransfers` — sorts by (ts asc, id asc) and replays every signed transfer against a per-player running balance seeded on first touch at `max(0, bought − cashed)`; each valid transfer debits the sender AND credits the recipient, so received chips become spendable exactly as they already are physically at write time, and a send is refused only when its amount exceeds the sender's running balance or would push the recipient past `Number.MAX_SAFE_INTEGER`. This closes a real write/read divergence: write-time settlement checks the physical `bal:<from_pid>`, so a re-send of received chips succeeds on-doc, but the pre-fix conservative read rule — which did NOT credit received chips — hid that physically-successful transfer from every peer's audit view. Because callers signature-verify every row BEFORE the replay, every credit still traces to a genuinely signed debit and `Σ running balances = Σ first-touch budgets` holds at every step, so no reader can be coordinated into minting; the new accepted set is a strict superset of the old conservative one. A self-transfer is additionally refused INSIDE the replay loop as defense-in-depth — `partitionValidTransfers` is exported, so no-mint must not rest solely on the caller's `isChipTransfer` self-filter (a same-pid alias would otherwise overwrite the debit with the credit and net +amount). Honest limit: the replay SEED is cage issuance (`bought − cashed`), NOT the live `bal:<pid>` — a player's gambling WINNINGS credit `bal:` without bumping `bought:` (`creditChips` is the payout path), so a net-up player's spendable balance exceeds their seed here. A winnings-backed send still SETTLES physically and the BANK CHIPS count — which reads `bal:` directly — stays correct, but this replay refuses that one send, so it is hidden from the TRANSFERS log until the seed catches up. Mint-SAFE (the replay can only HIDE a genuine send, never INVENT one) and PRE-EXISTING (the earlier conservative rule hid it too, and more); fully closing it needs a stable croupier-authored winnings counter, tracked as separate money-model future work), plus block-list partition helpers that return `{visible, hidden}` (incoming from a blocked identity is HIDDEN from the audit UI but the physical `bal:<to_pid>` credit that already landed at write time is still counted — conservation preserved, matching the #20 audit r2 block-follows-rotation sibling rule). Bound to the room casino Y.Map by `casinoDoc.ts` at `xfer:<id>` records — `writeChipTransfer(t, verify)` moves `bal:<from_pid>` DOWN and `bal:<to_pid>` UP inside the SAME `boundDoc.transact` as the record, so `Σ bal:*` is instantly unchanged by any transfer (physical conservation invariant, test-locked). Own-write checks reject over-drain, double-send (deterministic id ⇒ existing key collision), unsafe-integer overflow on the recipient's next balance, a bad own-signature (never plant a row the read seam would drop anyway), and the `MAX_CHIP_TRANSFERS = 512` capacity cap — now counted only over SIGNATURE-VERIFIED rows (a hostile peer cannot mint one), which closes a keyless-flood DoS: the pre-fix count over ALL `xfer:` rows let 512 junk rows planted straight onto the Yjs map (no key needed) lock every honest send in the room. The cap is GLOBAL and deliberately the SAME number the read seam materializes, so honest history can never outgrow the replay window — a per-sender cap would let many senders push the total past that window and silently truncate real transfers out of the running-balance replay (a correctness bug, not just a capacity one). The verify work is attempt-bounded and FAILS OPEN if a junk flood exhausts the budget (refusing there would restore the very DoS this removes); the cap is send hygiene, not a money-safety invariant — Yjs has no write ACL, so the enforceable truth stays the balance transact plus the read-seam sig-verify + replay. The BANK repaints on any casino-map change (`subscribeCasino` wired next to `subscribeVentures` at boot); block-list changes clear any half-armed send state so a just-blocked recipient can't survive as an armed confirmation; room switch clears send state so a stale arm from the previous room can't fire against a coincidentally-matching pid. Registry / Chia settlement stays blocked-external (the current per-room casino doc is the ledger of record for this slice). Test coverage: `src/casinoTransfers.test.ts` (41 tests — id derivation determinism, shape/entry guards accept + reject, sig verification with throwing-verifier defence, deterministic replay under any input order, the full-ledger rule that received chips ARE spendable + an over-drain beyond the running balance still refused without corrupting it + no time-travel across (ts, id) order + the recipient safe-integer bound, per-player running-balance independence, a self-transfer refused when handed straight to `partitionValidTransfers` (mint defense for non-read callers), conservation invariant Σ deltas = 0, filters, partitions, factory round-trip); `src/casinoDocTransfers.test.ts` (14 tests — atomic debit + credit + record, Σ bal:* conservation across mixed transfers, cage counters unchanged, over-drain refusal, insufficient-balance refusal, double-send refusal on deterministic id, malformed-transfer refusal at write time, bad-own-signature refusal, the GLOBAL verified-row capacity cap, junk/forged rows never counting toward it, fail-open under a larger-than-budget junk flood, hostile peer's mismatched-key row dropped on read, well-formed rows survive, end-to-end conservation across buyIn → transfer → cashOut with `houseNet === 0`). 55 new tests + 241/241 total across 8 vitest modules. Frontend-only; no node behavior, no version bump, no new dependencies.
- **Phone contacts BLOCK + group chats (issue #20 slice — client-side only):** the CONTACTS app grows a red ✕ BLOCK button on every contact row (with a `confirm()` prompt), a `🚫 BLOCKED` section that lists everyone you've muted and offers ↺ UNBLOCK, and a bounded per-install block list persisted at `localStorage['ssf-blocked-pubs']`. Blocked identities are hidden from the CLONES SEEN roster (with a "N hidden by your block list." footer note), their chat, DM, and world overhead bubbles are filtered out, and the log surfaces a "N messages hidden" system line whenever the filter fires. The DM overlay drops blocked-author messages (with a matching "🚫 N hidden from a blocked contact" system line) and the CONTACTS row hides its 💬 DM button for a blocked friend — so the BLOCK confirm()'s "chat and DM messages will stop appearing on your screen" promise reads truthfully at every entry point. The CHAT app grows a horizontal thread picker above the log — 🌐 ROOM by default plus one 👥 chip per group you're a member of, and a `+ NEW GROUP` chip opens a friend-picker + optional title. Group messages ride the existing room `chat` array tagged with `groupId`; thread records live in a new `groupChats` Yjs map, keyed by a deterministic sha512-of-sorted-members id, shape-guarded on read (id-must-match-members hijack check, creator-must-be-a-member, AND `isValidThreadEntry` requires the map key to equal `value.id` so a hostile peer's mismatched-key entry cannot produce a chip whose activation silently misroutes sends to the ROOM lane). The observer, send path, and input-placeholder all use `isValidThreadEntry` — router state and chip visibility stay in lock-step, and no downstream `groupsMap.get(activeChatThread)` can miss because a hostile entry slipped through a value-only guard. Bounded by `MAX_GROUP_CHATS = 64` with `GROUP_CHAT_CAP = 200` per-thread messages. The room lane keeps its parallel `ROOM_CHAT_CAP = 200` — trimmed by `excessRoomIndices` on room-only messages so a burst of room activity cannot evict older group history from index 0 (the pre-fix `sharedChat.length - 200` sitting on the WHOLE array had that gap). A GLOBAL `TOTAL_CHAT_CAP = 2000` safety net runs after the per-lane trim (`excessTotalIndices`) — the per-lane trims only touch lanes THIS client recognizes (its own active groupId and the room lane), so a hostile peer that writes messages tagged with distinct fresh groupIds (`junk-1`, `junk-2`, …) escapes both; the global cap catches that class of attack while the generous headroom over per-lane sums keeps honest use from tripping it. On leaveRoom the join-scoped chat-rebuild / thread-chip-refresh closures are nulled — the boot-time subscribers that trigger repaints via them stay silent (a no-op instead of an error-swallowed rebuild against a destroyed Y.Doc) until the next join republishes fresh closures. **Audit r4 fix:** the join binder now republishes BOTH closures symmetrically. Pre-fix, `latestRefreshChatThreadBar` was assigned only once inside a one-shot setup path (setupChatThreadBar, reached from bootstrapNetworking); leaveRoom nulled it, and every subsequent join left it null — so remote group creations, the initial paint at end-of-join, and the CHAT-app open handler routed through `refreshChatThreadBarIfBound` all became silent no-ops after the first room hop, and rejoining a room showed stale chips from the departure room. The sibling `latestRebuildChatLog` already republished per join (it closes over the per-join `sharedChat` handle); `latestRefreshChatThreadBar` refers to a module-scope function so republishing the same reference each join is safe. A source-invariant regression test pins the symmetry contract (`src/mainChatClosureSymmetry.test.ts` — 6 tests, extracts leaveRoom/joinRoomAtEpoch bodies by brace-scan and asserts every `let latest…: (() => void) | null` in main.ts is both nulled in leaveRoom and republished in joinRoomAtEpoch). Every new chat message now stamps `authorPub` (canonical identity pub) alongside the legacy `authorId` so the filter matches without re-resolving through the players map. Honest limits: this is **local mute + private-by-UI**, not enforcement — blocked bytes still arrive over the mesh, every room member replicates every group's messages, and the blocked party is not notified. A signed on-doc block op and an E2E-sealed group thread remain later TrustAndSafety / RoomLog work (see `brainstorming/phone-apps-breakdown.md` §3.2 and §5). Four vitest modules cover the pure filtering/threading/cap logic and the closure-symmetry invariant (`src/blockList.test.ts` — 19 tests; `src/groupChat.test.ts` — 57 tests; `src/directMessages.test.ts` — 7 tests for the DM block filter; `src/mainChatClosureSymmetry.test.ts` — 6 tests for the audit-r4 closure symmetry; 89 tests total across the slice). Frontend-only; no node behavior, no version bump.
...
- **Follow-up — block-follows-rotation (issue #20 slice / recorded per PR #136 reviewer note 2026‑08‑21):** the local block list (`ssf-blocked-pubs`) targets base64url Ed25519 identity pubs today — the durable cross-room stable id. When #124's keyed-identity **rotation** path lands (authority-head `seq` bump), a rotated key mints a NEW pub and an existing block would silently NOT follow it. The fix belongs in the #124 rotation slice: at rotation-observed, walk `ssf-blocked-pubs` and rewrite any pre-image pub whose authority head advanced to the post-image pub (same edge applies to the local contacts roster and DM history keyed by pub). See [phone-apps-breakdown.md §3.2](brainstorming/phone-apps-breakdown.md) for the plan entry.
- **Follow-up — winnings-vs-issuance audit view (issue #20 slice / recorded per PR #136 audit 2026‑08‑30):** the BANK TRANSFERS log is materialized by `partitionValidTransfers`, whose per-player running balance is SEEDED at cage issuance (`bought − cashed`). Gambling **winnings** credit `bal:` without bumping `bought:` (`creditChips`, the payout path), so a net-up player's spendable `bal:` exceeds their seed. Effect: a winnings-backed send SETTLES physically (recipient credited, `Σ bal:*` conserved) and the BANK CHIPS count — read from `bal:` directly — stays correct, but the replay refuses that one send, so it is hidden from the Sent/Received audit rows until the seed catches up. **Mint-safe** (the replay can only HIDE a genuine send, never INVENT one) and **pre-existing** — the earlier conservative rule hid it too, and more; the received-chips fix NARROWED this class, it did not introduce it. The complete fix is a stable, croupier-authored **monotonic winnings counter** (`won:<pid>`) folded into the seed as `won + bought − cashed`. Mint trap for the implementer: `creditChips` serves BOTH payouts AND bet **refunds** (a refund returns chips the player already owned), so `won:` must be bumped ONLY on true house→player payouts — a blanket `won++` on every `creditChips` would count refunds as new issuance and over-credit the seed. Belongs in a dedicated casino money-model slice, not the read-side transfer fix.
