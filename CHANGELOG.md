...
- **🎰 Slot machine — a three-reel cabinet with its own chair ([#109](https://github.com/Bella-Addormentata/StarStationFurlong/issues/109)):** a new furniture item you sit at and play. The payout engine is PURE — `src/games/slots.ts` touches no document and no DOM, so the odds are console-testable and a Chia commit-reveal fairness upgrade drops in around it untouched, the same shape as `games/craps.ts`. Each reel samples a virtual stop without modulo bias and maps it through a symbol strip, so the strip distribution *is* the house edge; the default returns **91.99%** over that distribution, inside the 85–98% a Vegas three-reeler pays. **A room editor can override the odds** (`SlotOddsConfig`, stored in the casino doc) and the machine face shows what they chose — "editor", not "deed holder", because the machine's house controls gate on `canEditRoom()`, which on venture property includes the venture's shareholders. Chips are the craps chips, stakes are deducted before the spin, and the machine runs `idle → spinning → settled → idle` — the owner's client or the croupier bot writes the settled result and every other client animates its reels independently from it, so nobody watches a spin that has not happened yet.
  ...
  **The rest of the hardening, most of it found by review rather than design.** Wagers are reserved, settled or refunded rather than optimistically deducted, and escrow is recoverable rather than strandable. **The odds you saw are the odds you get:** a play request carries a hash of the paytable the player was looking at, and the croupier refuses the round — recording a failure rather than spinning — if that hash disagrees with the machine's current odds; the settled record then keeps a *snapshot* of the paytable and funding used, so a later edit cannot rewrite an accepted wager. Requests are one per player per machine, polled on an interval, and a request that is never accepted expires into a recorded `request-expired` failure instead of wedging the machine. Only the deed holder is *eligible* to drive the auto-croupier — a narrower role than the room editor above, narrowed there by [#142](https://github.com/Bella-Addormentata/StarStationFurlong/issues/142) — and an operator lease records which session is driving a machine. Eligibility is not uniqueness, and this release does not claim it: the player id lives in `localStorage`, so two tabs of the same owner both qualify, and during a partition each side's lease can look valid locally. The debug handle is guarded and its teardown exported.

  **What is not claimed.** The request queue is read by scanning the casino map for matching keys, with no cap and no memory of request ids already cleared, so a peer able to write the room document can enqueue requests or re-insert an old one. The TTL above bounds how long a single request sits at the head of the queue — it does **not** bound the queue: a peer can mint arbitrarily many player ids, each getting its own key, and the croupier only ever expires the first sorted request, so total size, per-poll scan cost and drain time are all unbounded. This is the same unauthenticated-room-document-write boundary the room authority entry above describes rather than a slot-specific hole, and it closes when writes are authorized, not before.
