...
- **In-device maps — desk computer with room-management writes (#33 second slice):** Stage B of `brainstorming/device-maps-plan.md` §2 shipped. New `desk-computer` furniture — a 2×1 floor obstacle carrying a monitor + keyboard on a slab top, `deskTerminal` capability tag, `deskComputer` device kind — placed by default at world `(2, -3.5)` rot 0 between the map-table nook and the sofa cluster; the front-point at `(2, -2)` lands in the open aisle and the west edge is EDGE-FLUSH with the map-table under the same wedge-trap-safe-by-construction rule the bunk bed uses (`assertPlacementClear` covers it). Focusing it opens a **two-column** gold-frame panel: the LEFT column reuses Stage A's `wallComputerMap.ts` projection to paint the room canvas + small-station overview verbatim, and the RIGHT column adds the four **room-management writes** the issue calls for — rename (24-char cap, whitespace-collapsing sanitizer, control chars stripped), one-click **invite mint / copy** (delegates to the same `mintBootstrapLink` the SpacePhone uses, clipboard-with-fallback, `isValidInviteLink` guard rejects malformed results), access mode selector (`public` / `pass` / `keyed`, LWW-normalized on read so unknown legacy values collapse to `pass`, owner-only on write), and a peer roster with join-time ordering, owner tag, and honest "malformed row" labelling instead of dropping suspect entries. Discipline (same as Stage A): the new pure logic lives in `deskComputerManagement.ts` under 30 vitest cases (sanitize, access-mode normalize, roster ordering, invite validity, ownership label); the DOM adapter in `devices.ts createDeskComputerUI` is a thin painter that reads room state through a module-scoped provider seam (`deskComputerProvider.ts`) so `devices.ts` never imports Yjs directly and the seam stays testable without a DOM. No new network traffic — the mint reuses the existing bootstrap path. Honest limits: the ROOM ROLE table today distinguishes owner from everyone else — venture-shareholder tiers arrive with the treasury slice; a `.split().join()` escape ladder is used in place of `String.prototype.replaceAll` so the module stays under the tsconfig lib target; the wall computer's small-station overview (Stage A) is unchanged.
- **In-device maps — wall computer's small-station overview (#33 first slice):** the wall computer's focused UI now paints a SECOND wireframe below the room canvas — the whole known station drawn from the shared atlas, with the current room in the "you are here" gold accent and every gossiped neighbour outlined in cyan at its true dims when known (dashed at the uniform fallback when not). Small stations only: at more than twelve modules the pane defers to the future map-table **STATION page** (Stage C of `brainstorming/device-maps-plan.md`; the map table today ships only its SOL page) and says so, keeping the wall computer readable at inset size. All the projection math is pure — module footprints, equal-aspect viewport fit, and the small-station threshold live in `wallComputerMap.ts` under vitest coverage; the rotY convention matches `atlasLayout` / Three.js so oriented footprints agree with the exterior view, and dims are gated by the same integer envelope `floorPlanDoc` enforces so a hostile/older localStorage entry falls back to the honest dashed outline instead of collapsing or blowing out the fit. The full staged plan for moving every remaining map/overview surface into in-world devices (desk computer, map-table STATION page, holograph table, retirement of the fullscreen zoom overlay) is written up in `brainstorming/device-maps-plan.md`. Honest limits: the overview does not draw connector chains (map table's job), and it shows only what THIS client's atlas has gossiped — a visitor who has only joined one module sees only what that room's doc knows about the rest.
