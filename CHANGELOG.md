...
- **🧬 A 2×2 clone vat the clone squeezes out of: drained before the door opens, sealed before it refills, and a pale clone that regains its colour ([#166](https://github.com/Bella-Addormentata/StarStationFurlong/pull/166), fixes [#165](https://github.com/Bella-Addormentata/StarStationFurlong/issues/165)).** The vat is now a 2×2 tank centred on the NW corner's 2×2 square at (−5, −5): a 0.8 m glass radius, 2.7 m of glass, and a 140° door with a fixed transom above it. The corner cherry tree moves to the west wall. The fox is far bigger than any tank, so during the ceremony it is scaled every frame to an hourglass-shaped hard limit (`vatGauge.ts`), fitted to the rig's measured silhouette. The limit runs from the round tank, through the doorway between the door rails and under the transom, to a cone opening into the room. The tank drains and is held visibly empty before the door spins. The door shuts once the clone's tail is clear, and only then does the tank refill. A fresh clone decants almost-white grey and regains its own colours over 30 s.
  ...
  **Movable vats.** The clone is never released overlapping the vat or other furniture.
  - The walk-out ends at the last free spot on the door path, re-planned every frame.
  - A path cut short eases the clone back to full size where it stands.
  - A blocked door sets it down at the nearest free spot, clear of the whole vat (rear pipes included) by the clone's full reach.
  - With nowhere free, it stays held in the vat and retries.
  - A vat moved mid-ceremony carries the clone with it.
  - A vat removed mid-ceremony releases the clone where it stands, or at the nearest free spot.

  The free-spot search is complete but bounded: quadratic in the obstacle count, and capped at 1,000 obstacles because furniture records are peer-written.

  **Existing rooms.** A one-time migration (`cloneVat2x2V1`) moves an unmoved default vat and the corner tree to their new poses. It runs after sync on every join, not only on the first-claim path, which a normal restart never takes. It writes only for someone who may edit the room.

  **Testing, stated honestly:** `tsc` clean, 600 tests, 40 of them new in `vatGauge.test.ts`. They include a brute-force check of the hard limit along the whole walk-out, and clock-stepped tests of the vat's drain / open / close / refill order. Those caught `onOpen` firing a frame before the door's pose was applied. In a headless browser, with the ceremony stepped frame by frame, every vertex of the real rig was checked against the vat geometry, along with the pallor timings and each review fix's scenario. Not exercised: a real multi-peer room, and the migration against a live pre-existing lobby doc. Supersedes [#168](https://github.com/Bella-Addormentata/StarStationFurlong/pull/168).

- **⚓ A round, two-part docking adapter — DOCK / UNDOCK from the door and the helm ([#164](https://github.com/Bella-Addormentata/StarStationFurlong/pull/164), fixes [#163](https://github.com/Bella-Addormentata/StarStationFurlong/issues/163)).** A door can now wear one half of a round docking adapter, a **port**. A **dock** is two halves mated: this door's port and the mating half the connection brings to the far door. **+DOCK** in the door panel's connection assembly fits the port on the first press and stages the mating half on the second (one ADAPTER part each). **PROVISION NEW MODULE** then mints a new, independent ship or station whose birth door wears the other half; or pick a target and **INITIATE**. Two new controls release a module and re-make the berth it remembers: a **DOCK row** at the top of the pane, and a **docking computer** on the helm (a plain DOCK / UNDOCK button for one port, a clickable **ship atlas** for several). A released module is free to fly away, though flight itself is still SH3. Ports and docks are round in every view: an unpaired port wears a sealed half, and a dock is the two halves mated into a round tunnel. The space-only IDA collar is retired.

  **Both ends of a connection.** Only one room doc is bound at a time, which is why every undock used to be one-sided. DOCK and UNDOCK now tell the far room through a short-lived background session on the local node (`farDoorWrite.ts`). The session waits for real room state (for a room hosted elsewhere, a fresh answer from its live host), makes one compare-and-swap decision on the far door, and waits for the node to acknowledge the write (the new `YjsSync.confirmOwnWrites`). A DOCK also lets concurrent claims on the berth settle before it believes it holds it. Every rule both ends apply lives in the new pure `dockRules.ts`, so the two ends of one connection are judged alike: UNDOCK leaves a tombstone that remembers the berth, a dock never re-fits a removed port, and every stamp is causally after the event it replaces, whatever the clocks say. An unreachable far room is reported, never hidden: the near side stands alone until the walk-through mirror completes the dock or the far side undocks itself.

  **Testing, stated honestly:** `tsc` clean, 455 tests (65 new), build clean. Eleven Copilot review rounds; every finding fixed and pinned by a test. Checked live against a local node in two browser instances (station and ship): UNDOCK from the helm tombstoned both ends with matching berth memory, and DOCK re-paired both with the same stamp. Those live runs predate the review rounds, and the hosted-elsewhere path (two machines over a real bridge) was not exercised. **Deferred, documented:** a claim that reaches a berth after the settle window, or a near-side write racing the far end's claim, needs an authority for the berth (the room host, #67 D3). A door that once had a gangway and never a port is refused a dock until its owner fits one.
...
- **🔌 A movement tick can no longer throw out of the render loop ([#162](https://github.com/Bella-Addormentata/StarStationFurlong/pull/162)).** `sendTick` and the 2-second RTT probe each took their **own writer** on the WebTransport datagram stream per send, and the probe held its lock across an `await`. A `WritableStream` admits one writer at a time, so a tick that landed in that window threw `Cannot create writer when WritableStream is locked` — uncaught, from inside `animate()`, which calls `sendTick` *before* `renderer.render`. The frame was never drawn and the tick was lost; a short session against a live room logged 16 of them. The provider now takes **one** writer when the transport is ready and both senders share it (`write()` calls on a single writer simply queue, in order); `sendTick` stays fire-and-forget, is a silent no-op once disconnected, and no longer reports the rejections of writes it abandoned at disconnect. Found while smoke-testing #158 / #160.

  **`connect()` is tied to its own transport while it was open.** A dial abandoned by `disconnect()` rejects *later* — possibly after a newer `connect()` is up on the same provider — and its failure path used to reset the shared state anyway, leaving the live session reading `offline` with `sendTick` muted. Ownership is now checked on both paths: a dial whose handshake *completes* after it was superseded touches nothing shared (it would otherwise swap in its own dead transport's writer) and rejects, one that *fails* late no longer resets the live session, and the probe loop ends with its own transport instead of surviving a quick reconnect and double-counting on the next one. No wire or protocol change. **Testing, stated honestly:** the new tests drive the real provider against a fake `WebTransport` whose datagram sink can be held shut — exactly the race window — and they fail on the code they guard against, the first with the very `TypeError` from the console. Verified live through the local node: 65 s in a room with the avatar walking, then two room swaps on one provider — ping/pong 33/33 and 3/3 at 0 % loss, zero uncaught errors. The abandoned-dial cases are covered by test only; they were not reproduced in a browser.

## v0.36.1 — 2026-09-19

- **🚪 An open door shows its vestibule — the hull is cut open behind it ([#160](https://github.com/Bella-Addormentata/StarStationFurlong/pull/160), fixes [#159](https://github.com/Bella-Addormentata/StarStationFurlong/issues/159)).** When a door opened, its leaves slid aside in front of an intact wall panel: you still saw hull where the doorway should be. The octagon hull now opens a 2 × 3 m aperture behind a door, flush with the inside of its frame, on whichever face the door stands in — a side wall or an end cap (where most rooms hang their first door).

  **A door is a notch, not a window.** The first attempt routed door apertures through the window path, which cannot express one: a `ShapeGeometry` hole has to sit strictly inside its outline, so the window clamp lifted the 3 m doorway to `[0.05, 3.05]` (a sill across the threshold, a cut above the header), the shared openings list glazed every doorway with a window pane, and holes drawn *on* an end cap's outline could hand Earcut a degenerate polygon. A door stands on the floor, so its aperture is now a **notch in the face's own outline** (`notchedFaceOutline`) — the floor line steps up and over each doorway, overlapping ones merged into a skyline — and one code path serves all four vertical faces. Window holes are untouched; glass is built from windows only; a window that would run into a doorway is dropped with its pane (a transom above one stays). Apertures come from the frames themselves (`ajarDoorFrames`: the door groups that exist, at the pose each is actually hung from), so a cut can only ever sit behind its own frame, and `DOOR_OPENING_HEIGHT` joins `DOOR_OPENING_WIDTH` in `doorLayout.ts` as the one clear-opening contract the frame, the editor's ghost and the hull all read.

  **Cut only while the leaves are not shut.** Closed leaves meet at a deliberate 4 cm seam, and a wall cut behind a *shut* door showed through it as a bright hairline down the middle of every closed door. So the wall opens on the first frame of a slide-open and heals once a slide-shut has landed; the docking system announces every change that can move an aperture — a slide, a frame re-posed or removed, whoever asked — and the hull is settled once per frame, rebuilt only when the apertures really changed. **Testing, stated honestly:** the tests measure the *built* hull — triangle area, an exact grid-coverage scan inside and outside every aperture, the pane count — and the same assertions fail on the pre-review commit for the sill, the glazed doorways and overlapping side-wall doors. Checked live in a paired three-door module: first-person at an open door looks straight down the gangway; a closing door keeps its aperture until the leaves land, then the wall heals with no seam artifact; re-opening cuts again at once. The exterior (zoom 3) shell is not cut — the docking tube covers that spot from outside.

- **🛰️ First-person shows the same station as the zoomed-out view ([#158](https://github.com/Bella-Addormentata/StarStationFurlong/pull/158), fixes [#157](https://github.com/Bella-Addormentata/StarStationFurlong/issues/157)).** Looking out a window could show an older station layout — or none — while pressing `−` showed the right one. The first-person neighbour shells are laid out *around the room you are standing in*, but they were only rebuilt when a door or shared-atlas **event** arrived. On the first join of a launch neither does: the shells are built at boot in the *home* room's frame, the doors doc notifies before the listeners exist, and resuming into a room with a warm local cache syncs nothing new — so they stayed posed around the wrong room while the exterior, rebuilt every time it opens, was right. They are now re-posed the moment the room changes, again after the post-sync atlas harvest (which can rewrite the current room's own edges with no doc event), and after the bundled default-station seed at boot. **Testing, stated honestly:** verified against the live station rather than by unit test (the shells are world/THREE state): resuming into SSF-WELCOME and into a non-home module gives one shell per atlas pose with equal positions and rotations (10 of 10), where the same launch on v0.36.0 builds 0 of 10; a room-to-room walk was already correct and still is.

## v0.36.0 — 2026-09-15

- **🛰️ First runs dock at the default station — the owner's SSF-WELCOME — instead of an empty home module ([#156](https://github.com/Bella-Addormentata/StarStationFurlong/pull/156)).** A brand-new install used to mint a random home room and drop the player in it alone. It now joins the default station's welcome room (`home-faa222a14898`, the welcome module of the eight-room octagon) through the ordinary imported-pass path — the browser dials its own node, the node bridges to the pass's iroh hints, and it is a join, never a claim — so a new player's first sight is the whole station from space and their first room has people in it. The room is **one constant**, `WELCOME_ROOM_LINK` in `src/defaultStation.ts` (a GENERATE PASS link; the file header is the swap procedure, and an empty link ships with no default station), and the station's **layout ships beside it** as `defaultStation.atlas.json` — geometry and names, never seeds — merged into every install's local atlas at boot at gossip tier: it never overwrites first-hand entries, the first real gossip outranks it, it evicts first, and it is never republished into a room doc. Re-export it after rebuilding the station with `copy(window.__ssfMesh.exportDefaultStationAtlas())`.

  **If the station does not answer** (its host is offline) the boot waits up to 30 s for the host's room state — the same signal the transit curtain uses — then falls back to the player's own home once, exactly like a failed resume; either way the station lands in ACCESS → ROOMS, and the ACCESS app gains a one-tap **GO TO SSF-WELCOME** so older installs reach it too. Returning installs are untouched (`?seed=`, the last-room resume and the home boot all take precedence), and the install that hosts the welcome room never treats its own home as a foreign join. The bundled pass names only the owner's node as a hint: to widen the rendezvous, re-mint it from a client attached to a second always-on node, and set SSF-WELCOME's ROOM ACCESS to PUBLIC so the shared atlas carries its live seed. **Testing, stated honestly:** `tsc` clean, 367 tests (17 new: link ↔ atlas agreement, the reciprocal graph, the empty configuration, the bundle validator and the component export, gossip-tier seeding, its never-pushed rule and the zero-stamp replacement), build clean; four Copilot rounds found five real things, all fixed — the join's first atlas harvest recorded the EMPTY replica of a foreign room as first-hand and wiped the bundled welcome geometry (harvests now wait for the host's state to land), an explicit `?seed=` could lose the boot-target race to the default station (the link now registers synchronously and the boot awaits its bridged form), the atlas export dumped every station ever visited (now the welcome room's connected component), the home fallback could rejoin a local fork of the station and *claim* it because `activeBootstrap` still named it (cleared first — the #79 resume fallback had the same hazard), and a zero-stamped repaired record could not replace a bundled entry. Wiped-profile smoke tests against the live node: a first run booted straight into SSF-WELCOME with the roster synced and resumed there on reload, and a `?seed=` deep link took the seed path; the offline-fallback path was not exercised in a browser.

## v0.35.2 — 2026-09-14

- **🚪🧲 Docking: a vestibule to an existing module lands on the RIGHT door, and a door takes one vestibule — never two ([#155](https://github.com/Bella-Addormentata/StarStationFurlong/pull/155)).** Closing the octagon, the final vestibule connected to the wrong side of the first room, leaving two vestibules — to two different rooms — on one door (owner report on v0.35.1). Two defects. *(1)* The CONNECT matcher chose the far door by facing angle alone, among four wall-centre hypotheticals it always offered — including a wall whose real door was already paired: the atlas lists a far room's doors *because* they are paired, and the matcher dropped those instead of letting them block, so after seven ring hops of composed layout the occupied adjacent wall could win the 60° test. *(2)* The arrival trusted the record's `farDoor` by id; for a hypothetical that id is a compass word the far room may hang on another wall, so the traveler arrived at whatever door carried the name, the mirror was skipped because that door was taken, and the ring never closed the other way.

  **What changed.** A new pure `doorMatch.ts` does both jobs: the matcher scores by the **position** of the door face against the chain's end, at the module's true size — adjacent walls are metres apart, so layout error cannot flip the pick the way angle could — and never returns an occupied door (the far room's paired doors, any door another room's record already lands on, and no hypothetical on a wall centre a known door sits near); the arrival prefers the record's **wall** (+ lateral) over its id, tests every back-pointing record for being *this* link (its counterpart geometry describes our departure door; minted `d:` ids; lateral on the far wall) so doubly-linked rooms route each link to its own door, and never picks a door paired to a *different* room while a free door exists. **One vestibule per door is enforced at every seam** — the CONNECT prompt explains a refusal instead of offering CONNECT, INITIATE refuses a door with a live pairing and a far door the atlas knows is taken, an inbound request can never overwrite or delete a live pairing (it carries only an address, so it cannot prove it is the same connection), the FAR picker lists in-use doors disabled, and the arrival refuses them. There is no server to enforce it centrally, so each side refuses what it can see. The far module's true half-extent now rides through the projection and the station layout, and the atlas bounds a peer entry's door set at ingest and repairs oversized stored ones.

  **Already-broken octagons self-heal:** when an arrival lands on a different door than the near record named, the record's far end is corrected the next time you walk back into that room (`🩹 Far-door correction applied` in the console) — a compare-and-swap, kept per room and door, and never onto another connection's door; otherwise UNDOCK and re-CONNECT the closing link. **Testing, stated honestly:** ten Copilot review rounds, every inline finding fixed; `tsc` clean, 350 tests (29 new), build clean, the reworked pane opened live; an eight-room ring was not rebuilt in a browser session — the geometry cases are pinned by the tests in the same frame the exterior renders with. **Deferred, safe-side and documented:** re-initiating a door's *existing* connection can be falsely refused when the far mirror carries a stale compass id (UNDOCK and re-CONNECT), and the module-overlap check still tests uniform 5.9 m footprints for resized modules.

## v0.35.1 — 2026-09-14

- **Release process: a release can be cut from the Actions tab, and the workflow now refuses a tree that is not release-ready ([#154](https://github.com/Bella-Addormentata/StarStationFurlong/pull/154)).** `release.yml` fired only on a `vX.Y.Z` tag push, so cutting a release depended on `refs/tags` being pushable from whatever environment prepared it — the one that prepared v0.35.0 could push branches but 403'd on tags four times, and the tag went out from another machine. `workflow_dispatch` now takes `tag` + `target` (a commit, branch or tag name; blank = the dispatched ref's head): an existing tag is reused exactly as it stands — never moved, which also makes dispatch the way to re-run a failed release — and a missing one is created as an annotated tag, but only after the **release guard** has passed on the tree it will name. Every job checks out the resolved commit by SHA, so dispatching from `main` for an older tag builds the tag's tree, with only the workflow definition coming from `main`.

  **The guard turns the two silent failures in README § "Cutting a release" into loud ones, before anything is written:** `CHANGELOG.md`'s first `## v` heading must be the tag's (otherwise the release body is the *previous* version's notes), and all nine version locations — including both `Cargo.lock` crate entries a grep misses — must agree with it, one `::error::` per mismatch. It is inline in the workflow rather than a script file, because on a dispatch the checked-out tree is the tag's and an older tag would have no script to run. Also: the `vX.Y.Z` format is enforced on the push path too (the trigger glob's `*` admits `v1.2.3-rc1`); a tag whose release is already published fails up front (releases here are immutable); and release runs share one concurrency lane with a real queue (`queue: max` — GitHub's default keeps a single pending run and silently cancels the older one). `actions/checkout`, `github-script` and `setup-node` move to the lowest majors that declare Node 24, in both workflows; the runner's deprecation annotation is gone from every job.

  **Testing, stated honestly.** Not exercised end to end before merge — that needs a real, immutable release. The scripts ran against a mocked octokit through 14 scenarios; the guard body, extracted from the YAML, passes on the v0.35.0 and v0.34.5 trees and flags a single mutated `Cargo.lock` entry or a newer heading; the REST behaviours it leans on were confirmed against this repository; and four Copilot rounds found three real gaps (overlapping runs, the push-path format check, the one-slot default queue), all fixed. After merge, four zero-write dispatches from `main` — a suffixed tag, the published `v0.35.0`, an unresolvable target, and `v0.36.0` at `main` — each failed at the intended step (format check; "already published and immutable" after peeling the annotated tag; "does not resolve"; the guard, listing the heading and all nine locations), with no tag and no draft created, and all four ran to completion rather than cancelling each other, which is the queue doing its job. The first real release exercises guard-pass → create-tag → build.

- **Room authority: the deed check leaves `main.ts` and gets its first unit tests ([#151](https://github.com/Bella-Addormentata/StarStationFurlong/pull/151)) — no behaviour change.** `currentRoomDeedIsMine` is the predicate that has carried authority since [#142](https://github.com/Bella-Addormentata/StarStationFurlong/issues/142) — the deed hand-over, the sole-croupier election, the room's access mode and co-host accept/deny/revoke, everything irreversible — and it had no unit coverage at all, which is why the v0.35.0 entry below could pin those gates only with a source scan. The decision now lives in `roomOwner.ts` as `isDeedHolder`; `main.ts` keeps a same-named wrapper that supplies the live getters, the shape `isRoomOwner` already had.

  **The one design decision, and it is a security contract rather than a style choice:** `DeedContext.ownerKeyB64` is a lookup *function*, not a resolved key. [#141](https://github.com/Bella-Addormentata/StarStationFurlong/issues/141)'s invariant is that the legacy marker must never be *resolved*, not merely never compared — `players` is peer-written and the marker is used as a key into it, so `players['Local-Clone'] = { keyB64: <their pub> }` would hand an attacker the deed to every legacy room the moment anything looked the marker up. Passing an already-resolved key would move that lookup into the caller, where the legacy check cannot guard it, and `roomOwner.ts` would still read as correct. The interface makes the ordering enforceable, and a test pins it: the lookup is not called at all for `'Local-Clone'` or `''`.

  **Eight cases:** both grant paths (the owner id is ours; an owner id whose `players` entry carries our identity key — the returning-owner case, the only reason the lookup exists), the refusals, and the two the extraction exists to make possible — *never resolves the legacy marker*, and *narrower than `isRoomOwner`*: a venture shareholder passes the owner-equivalent gate and must not hold the deed, the #142 split as an executable statement instead of prose. Moving the lookup above the legacy check — the exact shape of the #141 hole — fails the first while every other case still passes, which is precisely how the original bug survived review. The source-scan `describe` no longer claims it cannot vouch for `currentRoomDeedIsMine`; `categorizeRoom`/`roomOwnerInfo`, `resolveOwnerLabel` and the two-step confirm guards remain `main.ts`-only and are named as such. 321 tests on `main` with this and #153 merged; `tsc --noEmit` clean.

- **🤖 Dock robots get a new body — the voxel waiter becomes a humanoid android ([#153](https://github.com/Bella-Addormentata/StarStationFurlong/pull/153)):** every robot a charging dock spawns — the waiter, croupier, coach and custom-script routines all run on the one `PoolWaiter` body — is rebuilt from the monochrome chassis with the black polo and sunglasses visor into a humanoid android: a synthetic face with blue eyes, long medium-brown hair under a white headset, a tapered torso with ab shading dressed in a red bikini, white-plated mechanical limbs with dark joint segments, sneaker-boots on lit soles, and cyan light strips (toe lights among them). Appearance only: the rig is the same hip-pivoted legs with knee-pivoted shins, shoulder-pivoted arms and one bobbing body group, so the patrol, the tray-and-drink serve, the coach's squat sets and every other routine animate unchanged. Still purely local ambience — each client simulates its own robots and nothing here is networked.

- **Housekeeping: private local files are ignored ([#152](https://github.com/Bella-Addormentata/StarStationFurlong/pull/152)):** `.gitignore` now covers `.local/` (identity recovery key and other secrets), `.claude/`, `.vscode/` and `images/`. Nothing in the built app changes.

## v0.35.0 — 2026-09-12

- **⚠️ BREAKING — room authority: the `Local-Clone` wildcard is retired and destructive surfaces move to the deed holder ([#141](https://github.com/Bella-Addormentata/StarStationFurlong/issues/141), [#142](https://github.com/Bella-Addormentata/StarStationFurlong/issues/142), [#143](https://github.com/Bella-Addormentata/StarStationFurlong/issues/143)).** Three ways an unauthenticated peer write turned into room authority, found by reading the code during the [#138](https://github.com/Bella-Addormentata/StarStationFurlong/issues/138) ownership analysis, none of them reachable without the capability already known and documented: **there is no write authorization on room documents at all**, so a modified client writes any Yjs record and every gate named here is client-side.

  **Two player-visible changes, both deliberate.** *(1)* **Legacy rooms are now read-only for everyone, including whoever built them.** `owner === "Local-Clone"` used to grant owner-equivalence — to every peer in the room simultaneously, not just to whoever wrote it — and it is gone from all three authority sites (`isLocalPlayerRoomOwner`, `currentRoomDeedIsMine`, `categorizeRoom`). There is no migration and none is possible: the marker records the *absence* of an owner, so no owner is recoverable from the document, and a first-claim would simply hand the room to whoever raced there. The off-chain ownership root (pass v3 `ownerPubB64`, then the keyed room id) is what gives these rooms a verifiable owner back, and that sequencing is now load-bearing rather than merely desirable.

  **If this is your room, here is what still works.** A legacy room stays *enterable* and stays in your list — it is filed as "visited", which is what it honestly is — and its contents, furniture and doors are untouched. What it loses is authority: editing, docking changes, access mode, co-hosts, and handing over the deed. The app says so in place rather than failing silently ("This module predates room ownership, so no one can edit it"). **To get an editable room, mint a new one** — a room you provision records you as its owner, so everything works normally there. Nothing is lost, but nothing moves across automatically either; a legacy room is frozen where it stands until the ownership root above lands. *(2)* **On a venture property, only the deed holder sets the room's access mode or manages its co-hosts.** Shareholders keep room edits, docking and door policy. `isVentureShareholder` reads the current room's own venture map entry — shape-checked, peer-written, and tied to nothing about this room or its owner — so a fabricated "office" record used to carry the right to lock a room out or unseat its co-hosts. The deed hand-over and the croupier election already sat with the raw deed holder; access mode and co-host accept/deny/revoke now join them.

  **The rest, in brief.** A planted office record is no longer permanent: it can be deregistered from the venture panel behind a two-step confirm, and the repair drops the local ledger entry too, so a forged cap table cannot re-propagate on the next visit. Peer-written `snapshotAt` on a venture link is bounded, so a far-future stamp can no longer pin a cap table against every honest correction.

  **A fourth fix ships alongside these, and is NOT one of them ([#144](https://github.com/Bella-Addormentata/StarStationFurlong/issues/144)).** The station atlas had the same defect *class* — an unauthenticated value driving a monotonic rule — but a different subject: it is about what your own client keeps, not about who may act in a room, and it came out of the saved-map work on [#133](https://github.com/Bella-Addormentata/StarStationFurlong/pull/133) / [#140](https://github.com/Bella-Addormentata/StarStationFurlong/pull/140) rather than the ownership analysis. Grouping it under "room authority" would misdescribe it. The gossip `updatedAt` stamp is now bounded; the atlas repairs and **persists** stamps poisoned before the upgrade, clamps what it publishes so a writer never emits what its own reader would refuse, and evicts on a two-tier rule where a room you actually stood in outranks anything gossiped. Its cap was a capacity bug as much as an attack — one join absorbs a whole station, so 64 entries is reached in ordinary play at around seven or eight octagon stations.

  **Testing, stated honestly.** The central owner predicate moved to `roomOwner.ts` and is unit-tested at last. Across nine review rounds on these fixes, *every* defect found was in `main.ts` or an untested second-order path and not one was in an extracted module — `main.ts` runs the whole client on import, so vitest cannot load it. The two gates changed here are therefore covered by a source scan that catches the regression that matters (widening them back) and nothing else; extracting these paths properly is a tracked critical-path item.


- **🎰 Slot machine — a three-reel cabinet with its own chair ([#109](https://github.com/Bella-Addormentata/StarStationFurlong/issues/109)):** a new furniture item you sit at and play, staking the same chips as the craps table. The payout engine is PURE — `src/games/slots.ts` touches no document and no DOM, so the odds are console-testable and a Chia commit-reveal fairness upgrade drops in around it untouched, the same shape as `games/craps.ts`. Each reel samples a virtual stop without modulo bias and maps it through a symbol strip, so the strip distribution *is* the house edge; the default returns **91.99%** over that distribution, inside the 85–98% a Vegas three-reeler pays.

  **A room editor can override the odds** (`SlotOddsConfig`, stored in the casino doc) and the machine face shows what they chose — "editor", not "deed holder", because the machine's house controls gate on `canEditRoom()`, which on venture property includes the venture's shareholders. The odds you saw are the odds you get: a play request carries a hash of the paytable the player was looking at, and a round whose hash disagrees with the machine's current odds is failed rather than quietly repriced; the settled record keeps a snapshot of the paytable and funding it was accepted under.

  **Machines are OWNER-FUNDED; shared bankrolls are disabled.** Worth stating plainly, because a shared bankroll was built and then withdrawn rather than shipped behind a caveat: a Yjs lease cannot serialize money across a partition, so it is not a mutex and was never going to be one. New shared funding is refused and the shared reserve, settle and refund paths are closed; the lease record survives only so a pre-gate balance can be inspected and withdrawn. Any machine you fund today is funded from your own chips.

  **The honest boundary.** Everything above is bounded by the same fact as the room-authority entry: nothing authorizes a write to a room document. A peer able to write one can enqueue play requests without limit or re-insert a cleared one — the read scans uncapped and no request id is remembered — so this is griefing that the design does not prevent, and it closes when writes are authorized, not before. Per-mechanism detail lives in the source — `src/games/slots.ts` (the engine), `src/slotCroupier.ts` (acceptance and settlement) and `src/casinoDoc.ts` (the records and their gates) — and in the review thread on [#110](https://github.com/Bella-Addormentata/StarStationFurlong/pull/110), which is where the hardening rounds happened. This entry deliberately does not restate it, having got it wrong repeatedly in review.

- **🏋️ Fitness-coach robot — the class is staged for the screen ([#77](https://github.com/Bella-Addormentata/StarStationFurlong/issues/77)):** a new `coach` routine for dock robots, built for the hangout rather than for the room: the bot walks to the open floor nearest room centre, turns to face **the camera**, and loops squat / jumping-jack / lunge sets — announce, eight counted reps ("One!" … "Eight!"), rest — so the person at the computer can work out along with it. A fox standing in the 6 m circle is escorted to a **side** slot of the row (never in front or behind, and door zones stay exempt so leaving the room always works), turned to the camera, and mirrors the demo in lockstep: the bot supplies its own chibi-scaled pose so the two rigs cannot drift, applied in the same post-state override slot the drink-hold uses and gated on `idle`, so walking away breaks the mirror rather than dragging the fox along.

  The coach rig was rebuilt for it — a V-taper torso with pec and ab plating, visored box head, and a real **knee joint**, so a squat bends like a human rep with the feet planted instead of sinking through the floor. Robot speech now waits until one second after the player actually enters the room, and a greeting dropped in that window re-arms instead of burning its cooldown, so it lands when there is someone to hear it. Robot bubbles anchor above the antenna rather than over the head.

- **Treasury PR C — read-only treasury UI (dev-only, no runtime money path):** a 🏦 TREASURY view opens from 🚀 VENTURES on the phone, reading the room's signed treasury caches. It shows this room's funding, the company's board and threshold, policy version and fingerprint, share classes, a proposal list, and a per-proposal detail with recomputed voting/veto/timelock/expiry clocks, votes held in the room, and board-approval progress. The room terminal gains a **FUNDING** section — funding source, company and treasury identity, funding profile, policy version, bound/end heights, a read-only note on every branch, and a plain list of the rows nothing can supply yet (covered systems and budgets; any pending bind/unbind request) — with `REQUEST COMPANY FUNDING` / `SELECT PROFILE` / `UNBIND` / `REFRESH PROOF` present but disabled. Everything is display only — the phone holds no treasury key, decides no balance, declares nothing executable, and no spend command is enabled. Each cache-backed panel carries a badge for how much checking actually happened (signature verified here, contents matching their own fingerprint, or unverified shape), and the honest states are the point: balances report that no verified source exists yet rather than showing a number the browser cannot verify; a missing funding record reads as a missing record rather than as "funded personally"; proposal clocks are worked out from the acceptance record and the matching policy version instead of a peer's cached copy, and stay marked unverified because both inputs are themselves peer-writable; a proposal's position on its clocks is only claimed when a height exists, always labelled with where that height came from; vote counts are described as what this room holds, with an unconditional note that a vote only counts once it is in a record confirmed on chain — unconditional because a record's confirmation fields sit outside its fingerprint, so a peer could otherwise fabricate reassurance and switch off the warning about it; and board approvals report one round's own progress rather than a figure spliced from two. Display logic lives in the unit-tested `src/treasuryView.ts` (u64-safe amount formatting — mojo strings exceed `Number.MAX_SAFE_INTEGER`); `src/treasuryNetwork.ts` adds the network-pin configuration seam (`VITE_SSF_TREASURY_GENESIS`), which **fails closed**: a build with no network configured has no pin at all, so every treasury read is disabled and nothing is displayed. (An earlier placeholder of 64 zeros looked unusable but was valid `Hex32`, which made it the live network — any peer could then publish records under it.)

  **Hardening (213 tests).** Everything above is read from a Y.Map any player in the room can write to, and most of what follows was found by review rather than by design. Reading a record now costs a bounded amount: each scan carries a traversal guard and a separate **verification** budget, because the expensive half is the per-record signature check, not the walk — 800 planted self-signed proposals took 1189 ms of blocked main thread and now take 39.7 ms. Verdicts are memoized per record class over deep-frozen values, so a cached "valid" cannot outlive the bytes it described or be handed to a different record type. Every list **pages**, and its controls are the point rather than a nicety: a bound with no way past it is a censorship tool, since a peer can plant a page of decoys ahead of the genuine record — that mistake was made three times, each time by moving a cap rather than removing it. Absence is reported as distinct states rather than one silence — held and readable, genuinely absent, held but unreadable, and, for the records that can be refused on size, held but larger than this device will read — because a record this device *refused* is not a record that does not exist, and saying otherwise about a room's own data is the same invented certainty as showing an unverified balance. The chain-view entry, which is the only cache value used as a clock, now carries a genesis and is rejected off-network; it was previously the one record the §17.5 pin could not check. A held-but-unusable funding record withholds the company details entirely, rather than letting the freely-writable policy name the company on its own. Accessibility went with it: every caveat is opaque at 5.5:1 or better against both dark surfaces (measured in-browser; the alpha gold they replaced composited to 2.6:1), the room terminal carries its own arrow-key traversal because Tab is the phone's open/close toggle, and focus survives the repaints that peer writes trigger. The censorship-by-bound lesson kept arriving from new directions and each one is now closed: keys are refused above the longest any record here can be named, because the guard counted keys and never their LENGTH, and a sort over peer-chosen keys sharing a long prefix costs 288 ms per scan against 15 ms at schema size; a pager is shown whenever the reader is anywhere but the whole list, because gating it on "more than one page EXISTS" let a peer DELETE earlier records to strand a reader past them and take PREVIOUS away with the same press; the share-class list pages instead of merely disclosing what it dropped, since a policy anyone can write could otherwise bury the real classes behind two dozen declared ahead of them; and one control driving the two vote key spaces advances both histories per step, or the shorter stopped growing and a single PREVIOUS rewound one side two pages while the other went back one. Absence honesty took two more passes at the very end: the open proposal was the one cache read on the screen not gated on whether a read was possible at all, and the reader answers a lookup it could not perform with the same `absent` it uses for an empty slot — so a disabled read told the player their proposal had been REMOVED, a claim about the room made from a question the device never asked; and the phone's readability gate judged on room id alone, while leaveRoom deliberately keeps the room id as last-room memory after destroying the document, so a stale id read as readable. That one was latent rather than live — an early return further down happens to stop anything being painted from it — but the room terminal had included the document in its own copy of that check all along, and two surfaces disagreeing about what "readable" means is exactly how this screen contradicted itself once before. A signature entry this device throws out is likewise not a signature nobody gathered: unreadable entries under a round's own signature keys were being skipped in silence, letting that round render an exact "2 of 3", and they now qualify the round they belong to. **One limit is disclosed rather than fixed, deliberately:** every page is rebuilt by walking the room's map from the start, so a peer who puts more entries than the traversal guard allows ahead of an honest key keeps it out of reach of paging entirely. That is a horizon, the comment claiming it was not has been corrected, and it is now reported as `discoveryCutShort` separately from ordinary paging truncation — "there is another page" and "the search that builds the pages stopped early" are different claims, and only the second means NEXT may never arrive. It is not repairable at this layer: resuming a walk part-way needs a stable position in a peer-writable insertion order, which is the index-versus-cursor mistake one level down, and a key-based resume point can simply be deleted. No plan scheduled a fix when this shipped; the sound one — a per-record-class key index the reader itself maintains from the map's change events, so no page is ever rebuilt from a walk a peer can crowd — is **PR C.1** in the sovereign plan §11 (proposed 2026-09-05; an index in the reader's own first-seen order, which is the only order a writer cannot push a seen key back through), gating the governance lane (PR F). The horizon itself is accepted there, as implemented, as a disclosed limit of this read-only screen only — the map is the vote transport, not a display cache, so no other reader may inherit the bound — and the code comments say the same rather than pointing at an op-log that pages nothing.


- **Treasury PR C follow-up — who signed the funding record, and what the tests actually pin (234 tests):** the room funding badge now rests on the room-side half of plan §10.1 rather than on any signature at all. The cache layer proves only that a binding was signed under the key it carries, so a peer could mint a key, sign a binding naming any company, overwrite this room's slot and have the phone show that company's board and proposals as this room's under a SIGNED badge. A binding is now this room's funding only when its signer is the room owner's identity key as the room document names it, read live (`gamesDoc.readRoomOwnerKey`, the same chain `roomOwnerInfo` and the deed check already read): **OWNER-SIGNED** anchors the company scope; **OWNER UNKNOWN** and **NO OWNER KEY** display the record as the claim it is and withhold the company details — deliberately never failing open, because once a room has synced the only peer actions that produce an unknown owner are deleting or overwriting the owner's players entry, or rewriting who owns the room, and trusting any signer then would turn censorship into forgery (an overwrite that carries a key substitutes the owner silently instead — the badge is honest that the owner is whoever the room's records currently name); **NOT OWNER-SIGNED** is refused as funding without denying that a record is held, and gets no company billboard. The signer's fingerprint is always shown, the company-side half of §10.1 is printed as **not checked** with the receipt it would rest on, the badge's wording is chosen by the owner key's provenance so it can never claim deed confirmation the reader did not do, and an unbound room's company (named by the policy cache alone) says so. Issue #138 — room modules as Chia NFT deeds — is documented in the code as exactly one seam, `readRoomOwnerKey`: an authority head verified by the player's own node replaces the players-map chain there and nothing downstream changes. Also in this pass: approval rounds and vote records set aside by the per-proposal company filter are counted on screen instead of letting the panel print "No approval round open" as fact; treasury paging state is reset at the room-join seam (a cursor is a key in one document's key space and meant nothing in the next room); the detail screen rewinds a cursor left past the end, as the list already did, and never prints an inverted range; an unreadable cached clock record is no longer described as a mismatch; the CHAIN VIEW panel shows the genesis the records are pinned to and not only its label; and six commit-message claims that no test pinned now have tests that go red when the defect is reintroduced — the separate horizon flag, the signing-session key-length bound, the verdict memo, the rewind condition, the three-panel horizon disclosure, and the late-added UNREAD states. The paging predicates moved out of `main.ts` into the tested module for the same reason. `TODO.md` no longer claims every review thread was resolved: one is left open by decision, and the fix it waits on is PR C.1 in the sovereign plan (proposed 2026-09-05) rather than described as already scheduled by a plan that never mentioned it.


- **Treasury PR B started (dev-only, no runtime behavior):** canonical treasury contracts landed per the [sovereign serverless plan](brainstorming/sovereign-treasury-serverless-plan.md) — `src/treasuryTypes.ts` (SSF deterministic CBOR, domain-tagged hashes, guards, derived proposal windows, checkpoint vote tree) with a vitest suite (the frontend's first test runner, new `npm test` CI lane), its byte-for-byte Rust twin `ssf-p2p-node/src/treasury_codec.rs` under the `chia-lane` feature, and shared golden vectors in `test-vectors/treasury/`. Nothing ships in a release binary path; no version bump.

- **Treasury PR B review hardening (PR 112):** cross-runtime guard parity tightened — the canonical encoder rejects unpaired surrogates and non-plain objects; the Rust codec enforces Hex32 vote ids, full `GovernanceKindRule` validation, and checked window arithmetic, mirroring TypeScript; inclusion-proof verification bounds proof length and rejects unknown step sides; and `TreasuryExecutionRequestBody` gains `stateCoinId`, binding each signed request to the exact state coin it consumes so an authorization cannot replay across state-coin generations. The policy-hash accumulator is `__proto__`-safe: an own `"__proto__"` rule kind from parsed JSON hashes as an ordinary map key in both runtimes instead of hitting the legacy prototype setter. Deserialization-boundary guards `isCompanyTreasuryPolicy` and `isDeviceAllowance` (plus `isShareClassPolicy`/`compareMojoStrings`) validate the authority-bearing cache records — complete 9-kind rule records, distinct signer/class/module sets, `threshold ≤ signers`, bounded mojo fields — for the treasuryDoc.ts cache layer to enforce; the hash helpers stay validation-free pending PR D's typed Rust structs.

- **Treasury PR B completed — `treasuryDoc.ts` signed caches:** the plan §14 treasury Y.Map lands with validation-on-read scaled to what each record can prove. Signed records (proposals, votes, room bindings) get the full ladder — guard → id recomputation → signature verification; content-addressed records (checkpoints, signing-session shells, payloads) get guard → id/hash recomputation; plain caches (policy, allowance, receipt, registration, windows, sync) get shape/network guards only and are **deliberately plain-replace** — invariant 5 keeps them replaceable, and enforcing occupancy over unauthenticated entries would let planted records brick honest re-puts (authenticity is the chain's job via PR D). Vote slots are keyed by the gameSig-authenticated `voterGamePub` with the §7.2 canonical rule per slot (greatest per-voter sequence, ties to the smallest voteId; `pickCanonicalVote` exported for the tally-time puzzle-hash dedup). Session ids are derived (`signingSessionIdOf` hashes the immutable shell) so shell conflicts are structurally impossible, and collected signatures live one-key-per-signer so CRDT sync itself unions partitioned collections. A content-addressed `payload:<payloadHash>` lane covers sovereign §5 data availability. Bound at the room-join seam in main.ts with keypair identity verification and a configured network-genesis pin (§17.5): correctly signed records from any other network are rejected on both write and read. Supporting contract additions in `treasuryTypes.ts`: `voteSignatureBytes` and `payloadHashOf` (golden-vectored and mirrored by the Rust codec), browser-only `roomBindingSignatureBytes`/`signingSessionIdOf`, and record guards for proposals, votes, checkpoints, signing sessions, registrations, windows, bindings, and receipts.

- **Furniture drive handles — one list (absorbs #117):** the six handle kinds a furniture mesh can carry on `userData` (`wallScreen`, `holoSpin`, `trunkLid`, `gameTableTop`, `cloneVat`, `slotMachineVisual`) were filed by two hand-kept copies of the same list — `World.registerFurnitureGroup` and the DEV menu's `registerSpawnedGroup` — and the copies drifted (a DEV-spawned slot machine had no reel drive). `src/furnitureHandles.ts` now owns the list: both paths call `registerFurnitureHandles` against `World.furnitureHandleSinks()`, so a new kind cannot land in one path only, `tsc` fails wherever a sink is missing, and `furnitureHandles.test.ts` fails if either path grows a private copy again or World's one-path removal stops deleting from any sink (#45 F1). No runtime behaviour change beyond the DEV-spawned slot machine now being driven.


- **Board mirroring survives a room that started empty ([#118](https://github.com/Bella-Addormentata/StarStationFurlong/pull/118)):** the #45 game-board mirror subscribed behind a `gameTableTops.size > 0` check sampled while the platform was built, so a room built with no tables never subscribed at all — a table added later, by DEV spawn or by the E4 reconcile a joiner runs, had no mirror until reload. The subscription is now unconditional (its closure walks the live map; over an empty one it is a free no-op) and its unsubscribe is held, so rebuilding the platform swaps the listener instead of stacking a second one. Both registration paths also paint the current document state when a table registers, since the listener only fires on the NEXT change — a table spawned into a room with a game already in progress used to come up blank.

- **Dependency: postcss bumped past the `sourceMappingURL` disclosure advisories ([#113](https://github.com/Bella-Addormentata/StarStationFurlong/pull/113)).**
...
The door overhaul (#103, sixteen commits): every rule that made one door different from another is deleted. A minor-version bump because stored-room formats changed shape (pairing records, wall names) — this is a development build with no backwards compatibility; older clients should not join new rooms.

- **🚪 One kind of door.** The invisible split between the four built-in doors and doors you place is gone: every door is a record — which wall, where along it, what its sign says — and every feature works on every door. The last two gaps closed this release: the keypad's **POSITION** nudge buttons now appear for placed doors too, and a door's full wall run is draggable (the built-in doors had silently lost 3–4 m of travel to an old bookkeeping split).
- **🛰️ Any door docks.** The connection assembly, target address, provisioning and INITIATE appear on every door's terminal, any door can wear a **docking adapter** (with its collar rendered on the hull where the door really is), gangway chains can aim at and auto-fit a placed door, and walking through comes back correctly. Doors must now sit **4 m apart** so two docking envelopes can't overlap — everything already built clears that with room to spare.
- **🧭 Rotate and shift the new module before you commit it.** Provisioning a module shows a live wireframe **ghost** at the end of your connector: **⟳** turns which of its walls carries the door, **◀ ▶** slides that door along the wall, and the module is born with its door exactly there. The pairing carries the far door's true position, so everyone renders the connection correctly before anyone has walked it.
- **🪧 Doors have names, not directions.** Type a sign in the door panel — **CARGO BAY**, **DOCK 3** — and it appears as an engraved plaque over the door and everywhere the door is mentioned. Unnamed doors show as **DOOR 1, DOOR 2, …** counted around the room. No compass word survives anywhere a player looks: pane titles, hints, the wall computer's map (which now draws your real doors, numbered), the far-door picker (which now lists the target module's actual doors). Walls are named by axis internally — **x±/y±**, origin at the room's centre — because a module seen at an angle has no north.
- **🔑 Seen by everyone, entered by permission.** Any module's outside — its size, position and connections — is visible to the whole station, and a room's size now actually travels (peers used to draw unvisited modules at a guessed size). The dial-in address is published only while a door **that actually exists** is set to public passage; before, a room whose only door was owner-locked still broadcast its address, because the check consulted four doors the room didn't have. The door panel says which state you're in: **🔑 published** or **🔒 UNLISTED**.
- **🛰️ New rooms are born right.** **JUMP TO NEW STATION** gives a room with **zero doors** (previously impossible to represent — four doors materialized out of the accounting); a provisioned module is born with **one door**, where you placed it; a claimed room gets **one centred door per wall** instead of the old doubled-up arrangement. Removing a room's last door is allowed with a confirm click instead of refused.
- **🏊 Wayfinding follows reality.** The built-in POOL and CASINO plaques hang on whichever door is actually **paired** to those rooms, wherever it is and whatever it's called — they were pinned to two fixed door names before, and pointed wrong the moment you re-paired. Your own signs always win over the built-ins.
- **Honest limits.** Existing rooms keep their current door arrangement — nothing moved in any already-built room, verified to the centimetre; the doubled-up layout in the older rooms retires only when re-authored. Door numbers renumber if you add or remove doors (names you type never change). The far-door picker offers only "auto" for modules the station hasn't seen yet — the first walk-through fills in the truth.
- **Release line:** version bumped to 0.34.0, all nine locations. **Node binaries UNCHANGED** — frontend-only (TypeScript); no Rust node code changed since v0.33.30.

## v0.33.32 — 2026-08-08

### 🚪🔒 The terminal moves onto the door — and every door gets one (#95)

A door release: the control panel is mounted on the door itself, doors you place yourself finally have controls, and the panel stops running off the bottom of the screen.

- **🎛️ The terminal is on the door now.** The golden control panel and its status light used to sit off to the side, on the wall beside the doorway. They're now centred on the **right-hand door post** — as you face the door from inside the room — standing slightly proud of the frame, so the terminal reads as part of the door instead of a separate fixture next to it. One trade: the keypad sits over the doorway's edge, so the clear patch you click to walk *through* a door is a little smaller than it was.
- **🚪 Doors you place yourself have controls.** They get the same keypad and status light as the four built-in ones — until now only the built-ins had a terminal at all. Click the keypad and a compact **DOOR SETTINGS** panel opens: lock or unlock the door, and under **DOOR POLICY** set who is allowed to walk through it. Locking really does shut the door, turn its light red, and refuse to open when you walk into it — though for now that lock is a **local setting**, not something shared with the other people in the room. The **SECURITY PIN** box is drawn on this panel but doesn't do anything yet on a door you placed: whatever you type is discarded, and the field is blank again next time you open it.
- **🛰️ Still no docking from a placed door.** The docking-only parts — the connection assembly, the target address box, provisioning a new room, and the pairing button — stay hidden on these panels, because a door you placed is a passage, not a docking berth.
- **📏 The panel stops at the edge of the screen.** The door control panel now stops growing at **80% of the window height**. When it holds more than fits — an expanded DOOR POLICY section, or a full connection assembly — its body scrolls, with a slim amber scrollbar that matches the panel, while the title bar and its close button stay pinned at the top. Previously the panel had no height limit at all, so on a short window whatever spilled past the bottom of the screen was simply out of reach.
- **🧭 Doors are named by the wall they stand on.** A door now reads as its physical wall — **"NORTH WALL"** — in the control panel's title and on the tag that appears when you select it in room editing, instead of an internal name like "south" or a long machine id. That's more honest, but it isn't a unique name: the standard room layout stands two doorways on the north wall and two on the west, so an ordinary room shows just **two labels across its four doors**. Messages elsewhere — the note you get walking through a door, for instance — still use the old internal names.
- **♿ The close button reads properly.** Hovering the **X** that closes the door panel now pops up a "Close" tooltip, and screen readers announce it as "Close door control panel" rather than reading out just the letter X. The button itself looks the same as before.
- **Release line:** version bumped to 0.33.32, all nine locations. **Node binaries UNCHANGED** — this release is frontend-only (TypeScript), so no Rust node code changed since v0.33.30.
