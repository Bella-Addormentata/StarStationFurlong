...
- **🏒 Air hockey table (#115):** a new 2×3 game-table furniture piece (dev-menu spawnable) with a stand at each end — walk up to an end and the camera locks into a first-person play position over your goal. Pointer lock steers your mallet; **holding the mouse button presses the mallet down onto the table, letting go lifts it** — a lifted mallet passes over the puck without striking it. First to 7 wins; a pole-mounted dual-face digital scoreboard (plus goal lamps) tracks the match, serves run on a countdown, a solo **PRACTICE** mode opens both goals, and a stale opponent forfeits after 10 s. Real-time play streams over the existing 13-byte tick datagram lane — two new lane kinds (mallet, puck) at 30/20 Hz with u16 serial acceptance and self-echo/side-spoof drops — while score and match lifecycle sync through the shared `games` map with the same shape-guarded whole-value writes as chess/checkers; the physics itself is a pure engine module (float sim, not yet deterministic — see honest limits) with 28 unit tests. **Room owners can set an entry fee in casino chips** (the same balance the slot machines pay out): charged when you READY UP or start a paid practice into a **per-player escrow record** — one transaction debits the payer and holds the fee, un-readying before the start always refunds it in full (the chips never left escrow), the record becomes final immediately after the match-start write (the frame loop remains the crash backstop), and the owner's client sweeps finalized fees into their balance on a slow timer. Every escrow key has exactly one writer per lifecycle phase, so concurrent pays/refunds/sweeps on diverged replicas can neither mint nor destroy chips (proved by a 44-test two-doc Yjs fork/merge convergence suite), and a crash, reset, kick, or table removal strands nothing: each client self-heals its own records from pure state — refund when the match can't be proven to have run, finalize when it can. Fee configs are validated against the *current* room owner on read and write, so a stale or hostile config naming any other recipient reads as "no fee"; the owner plays their own table free, short players are refused with the price shown, and ownerless (or offline) rooms hide the fee knob and charge nothing — a fee needs a recipient. Hardening from the same PR-review pass: (a) `payAirHockeyFee` now only reuses `held` records — a leftover `final` from a completed-but-unswept previous match no longer counts as fresh payment (double-play block, config-bypass block); (b) `doReady` and `doPractice` finalize their own escrow immediately after the match-start write, so a fast END PRACTICE/unmount can no longer refund a fee whose match provably ran; (c) opponent display names in the focused HUD are `textContent`-escaped, closing an XSS route where a peer-writable name would inject markup; and (d) the pointer-lock capture layer now forwards mousedown/mouseup to the mallet-down handler before stopping propagation — the mallet strikes reliably in the locked cursor. A malformed peer write landing an air-hockey record under a *classic* game table's key renders a recovery strip (anyone may RESET) instead of wedging that table. **Honest limits:** pointer-lock mallet feel and a two-browser live match still owe a hands-on pass (engine + all wiring were verified headlessly end to end, and the escrow economics were re-driven live in-app after the redesign: config validation, pay/refund/idempotency, and all three self-heal arms); the physics sim is float-based, wall-clock-`dt`-driven, and simulated by a single client with no signed input transcript retained — so any future real-value settlement (state channel + on-chain referee) has no arbitration path today: before entry fees become real money the sim must either be made deterministic (integer/fixed-point puck state, fixed timestep, seeded serve, both players' mallet inputs signed and retained) or settlement must go ceremonial (both players co-sign the final score, 2-of-2 spend on agreement, disagreement refunds both). Related tiering caveat: the operator writes the score the escrow decides on, which is a T2 shape (two distrusting parties, value at stake) currently wearing T1 clothes — fine while chips are prototype non-money, must be re-tiered before fees are anything real. Un-ready refunds racing the opponent's match-start write across a sync gap can let that one player keep their fee yet still play — bounded at a single fee and biased toward the player, the conservative direction; opponent liveness rides unauthenticated peer ticks, so a hostile peer could stall a forfeit until the M5.5 per-tick authorship lands (RESET recovers); and escrow records sit inside the same map-wide unauthenticated-write boundary as the balances themselves — a forged record can mint chips *into* its named player or owner, which is nothing a direct balance write couldn't already do, is equally visible in the cage ledger, and is pinned by tests as the documented bound until authenticated authorship closes the whole class.
