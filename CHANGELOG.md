...
- **🎰 Slot machine — a three-reel cabinet with its own chair ([#109](https://github.com/Bella-Addormentata/StarStationFurlong/issues/109)):** a new furniture item you sit at and play. The payout engine is PURE — `src/games/slots.ts` touches no document and no DOM, so the odds are console-testable and a Chia commit-reveal fairness upgrade drops in around it untouched, the same shape as `games/craps.ts`. Each reel samples a virtual stop without modulo bias and maps it through a symbol strip, so the strip distribution *is* the house edge; the default returns **91.99%** over that distribution, inside the 85–98% a Vegas three-reeler pays. **The room owner can override the odds** (`SlotOddsConfig`, stored in the casino doc) and the machine face shows what they chose. Chips are the craps chips, stakes are deducted before the spin, and the machine runs `idle → spinning → settled → idle` — the owner's client or the croupier bot writes the settled result and every other client animates its reels independently from it, so nobody watches a spin that has not happened yet.
  ...
  **Hardening, most of it found by review rather than design.** Settlement is authoritative rather than client-claimed; wagers are reserved, settled or refunded rather than deducted hopefully; an operator **lease** stops two croupiers settling the same spin; the shared bankroll is leased and serialized so two machines cannot spend the same chips; the play-request lifecycle is bounded so a peer cannot flood or replay one; escrow is recoverable rather than strandable; the odds are bound into the settled record (a paytable hash) so the table cannot be swapped between commit and reveal; and unsafe shared-machine funding was disabled outright rather than shipped with a caveat. The debug handle is guarded and its teardown exported.
