...
- **🎲 Craps engine + felt UI — full bank craps (#69 G3):** the pure `src/games/craps.ts` engine now covers the whole standard bank‑craps repertoire — pass line, don't pass, come, don't come, pass odds and don't pass lay odds (true‑odds 4/10 2:1, 5/9 3:2, 6/8 6:5; reciprocal lay), come odds and don't come lay odds, field (2/12 double), place 4/5/6/8/9/10, any 7, and any craps — with come/don't come travel handled at the round level (a fresh come bet whose first roll lands on 4/5/6/8/9/10 travels: `resolveCrapsRound` stamps the come point onto the surviving bet, and subsequent rolls settle it against that stamped number). Stack aggregation (place + all odds types) sums stake by `(type, pick)` before rounding, matching a real dealer's payout on stacked chips — six 1‑chip PLACE 6 bets pay 7 (not six per‑chip 0s), and five 1‑chip pass odds on point 6 pay 11 (not 10). The bet‑shape guard `isCrapsBet` now rejects malformed peer writes (stray `pick`/`point` fields, non‑point `pick` on odds bets, unknown types); the come‑out phase enforces the standard "odds are OFF" rule at the engine (a peer‑injected pass/come/don't‑pass/don't‑come odds bet during come‑out simply stays instead of paying, matching the block comment); the felt gains a COME/DON'T COME strip and a PASS ODDS / DON'T PASS ODDS strip that dim when the current point state closes them, with 3x/4x/5x max‑odds cap enforced at placement (helper `crapsMaxOdds` exported). Comprehensive vitest suite pins every payout table, the come‑bet travel lifecycle, the stack‑aggregation semantics, phase gating (`canPlaceBet`), the trust‑boundary guard (including the come‑out odds guard), and the shooter‑hand narration (`rollCall`) — 97 new tests bringing the frontend total to 194, all green alongside `tsc --noEmit`. Honest limits: the come‑odds / don't‑come‑odds UI affordance is engine‑only for now (no per‑come‑point felt hitbox), so those bets are placeable via peer writes and console but not yet by click; the fair‑dice modes are still dev‑simulated locally (block‑beacon and multiparty second parties are wired end‑to‑end but produce dev entropy until the network/chain half lands).
