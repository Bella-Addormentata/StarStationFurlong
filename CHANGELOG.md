...
- **🚪 #66 S2 door-carry safety net: shrink-on-read clamp + shape-parity tests.** The edit-mode STRUCTURE door carry (`editMode.beginDoorDrag` → `commitDoorDrag`) and the keypad slide (`docking.ts` `slide-neg`/`slide-pos`) already write through ONE store after the #18 fold — both call `writeDoorLayout({...rec, lateral: n, placed: true})` on the door-layout Y.Map. This slice locks that "one source of truth" invariant with the tests it lacked and fixes the last stranded-door edge on the S3 grow/shrink path. The wall's slide limit (`doorLateralLimitForWall`) is dynamic — it shrinks with the room — but the reconcile re-poses from records rather than editing them, so a door authored at lateral 4 on a 5×5 wall (limit 4) stayed recorded at 4 after the owner shrunk the room to 1×1 (limit 1) and `reposeDoorTargets` rendered it 3 m past the wall corner. `readAllDoorLayout` in `doorLayoutDoc.ts` now applies the same read-side inward clamp `floorPlanDoc.readDoorPlacement` uses for the retired slide store, so every consumer (reposeDoorTargets, physicalDoorPose, paired vestibules, exterior projections) sees a legal position on the next observe notify — and a grow reveals the ORIGINAL authored lateral again because the stored record was never edited. The un-migrated-room fallback in `defaultDoorLayoutRecords` gets the same clamp so the paired compat kinds' ±3 PAIR_OFFSET can't strand a cardinal on a 1-tile-axis room. Seventeen new vitest cases in `doorLayoutDoc.test.ts` prove: keypad-shape and drag-shape writes round-trip through the doc identically (a spread-then-overwrite preserves the label; the read-side #91 lateral normalisation reaches every writer); clamp-on-shrink is symmetric, elastic (grow restores the original), inward-only, and per-wall on rectangular rooms; hostile / malformed records (non-string wall, NaN/Infinity lateral, missing id, id/key disagreement, primitives) fail safe (skipped, never propagated); labels are sanitised at the read boundary (padded/whitespace-only/over-long); and the shape guard `isDoorLayoutRecord` rejects each malformed shape individually. All 157 tests pass across 5 files (140 baseline + 17 new); `npx tsc --noEmit` clean. Owner-only writes and shape-guarded reads unchanged; the doc layer stays deliberately unaware of pairing (that gate lives in editMode + docking.ts where the write callers are).
- **🧱 Rooms grow and shrink by one 6 m tile at a time (#66 S2/S3 core).** The floor plan's `dims` field (1×1 to 5×5 tiles, envelope-guarded on write AND on read) is now the single source of truth for the room's wall coordinates, so a resize re-derives every dependent piece live — floor, click plane, edit grid, side walls + coping strips, exterior capsule, octagon hull, the door wall coords and their along-wall slide clamps, the walkable-grid bake, obstacles, seats, stands, devices, the skylight mesh list, and the neighbour-through-the-window shells — one owner-driven action, zero teardown/rebuild round trip. Owner-only STRUCTURE controls in the dev menu drive the write (`+ / − COLS` and `+ / − ROWS`, disabled at the envelope limits); a peer's resize refreshes the readout live via the shared floor-plan subscription. The subscription handler cheaply short-circuits when only non-dims fields change (a door slide, a tile edit), so this doesn't cost anything on the common path; a shrink safely beams the local avatar back into the fresh walkable box rather than stranding them outside; and a live edit session's raycast index is invalidated the same way a furniture change already does it. A new `HULL TILE` currency joins FLEX/EXT/ADAPTER (dev-grant row only for now); pre-existing installs upgrade cleanly because the load reads missing counts as 0. Coverage: 26 new vitest cases prove the S2 zero-diff bar (a doc with no `dims` still reads the legacy `{halfX:6, halfZ:6}` room and every door lands exactly on its LEGACY placement), the write- and read-side sanitizers (non-integer, out-of-range, missing axis, non-object, hostile plants), the boundary conditions (1×1, 5×5, rectangles), the `meta.v=2 + minClient` advisory being reserved for non-default dims only, and the dims-driven door geometry (walls in the LOCAL frame stay axis-aligned; a stale door on the old wall is repaired to the fresh default; a 1-tile axis shrinks the door slide limit to 1.0 so a door can't slide off-wall). Behind the shipped bit-identical bar, the sanitize-on-both-sides discipline in `floorPlanDoc.ts` and the addOctagonHull-style dispose-then-rebuild in `world.reconcileRoomDims` are the two invariants everything else rests on.
  **Audit remediation (pass 1).** A follow-up scrutiny pass surfaced six defects, all now fixed with regression coverage: (1) the wood / casino carpet / outdoor stone floor textures baked their `repeat` at the DEFAULT 12 m room, so a grown room drew them stretched and a shrunken room drew them compressed — `reconcileRoomDims` now rescales all three linearly (matching the wood texture's own creation formula), and the lazy-build path in `applyRoomVisuals` refreshes on first theme entry too; (2) paired vestibules / connector chains captured `physicalDoorPose(id)` at group creation and stayed frozen at the old anchor after a door slid along a wall or a resize moved the wall itself — a new `repositionPairedVestibules` on the World, fired from `reconcileDoorPlacements`, re-poses every live group from the fresh door face while preserving each chain's fold shape; (3) `writeRoomDims` now REFUSES a shrink that would strand placed furniture past the new placement bound (checked via a new `anyFurnitureOutsideBounds` helper on `furnitureDoc.ts`), and the return type became a discriminated `WriteRoomDimsResult` so the dev-menu can surface a targeted hint (`stranded-furniture` / `invalid-dims` / `not-bound`) instead of a silent no-op; (4) a shrink BACK to the default 2×2 now downgrades the S3 advisory (`meta.v=2` → `1`, `roomInfo.minClient='0.32.36'` → `0.32.1`) only when THIS release wrote those specific values — a foreign advisory (e.g. a future `meta.v=3`) is left intact and Case-C untouched peers never fabricate one; (5) `reconcileRoomDims` now runs `reconcileRobots(computeRobotPatrol())` after the walkable rebake, mirroring `reconcileFurniture` and the room-template / morph paths so a resize can't strand a robot at a former patrol node; (6) `reconcileRoomDims` returns a boolean (`true` on full rebuild, `false` on short-circuit), letting the `subscribeFloorPlan` handler skip the duplicate `reconcileDoorPlacements` call on a genuine dims write while still firing it on a door slide. Eleven new vitest cases in `floorPlanDoc.test.ts` cover the write-result contract (3), the shrink-would-strand guard with growth and same-size exemptions (4), and the advisory downgrade (4) — including the defensive branch that leaves a foreign meta / minClient alone. All 134 tests pass (123 baseline + 11 new); `npx tsc --noEmit` clean.
  **Audit remediation (pass 2).** A second scrutiny pass caught one live defect and one type-honesty cleanup, both in `repositionPairedVestibules`. The docstring claimed a missing door record was "safely ignored by physicalDoorPose", but `physicalDoorPose` actually falls back to a north-wall centre (with a warn) for any id absent from both `doorRecords` and the four `LEGACY_ID_WALL` cardinals. For a paired free `d:xxx` door whose deletion `drainOrphanVestibules` had to DEFER (avatar mid-walk-through, transit vestibule active) the id stays in `pairedVestibules` until a later frame can dispose it safely — and the reposition would have teleported the live gangway mesh to the north wall for the remainder of that window. `repositionPairedVestibules` now iterates with `physicalDoorPoseOrNull` and SKIPS null results, leaving each still-in-use group at its last known pose until `drainOrphanVestibules` finally disposes it, exactly as the deferred-drain contract already promised. The same edit removes the misleading `id as DoorId` cast (the map is keyed by `string`, not the four-cardinal `PhysicalDoorId` union), so the types now tell the truth about which ids reach the call. Six new vitest cases in `doorLayout.test.ts` lock the contract — `physicalDoorPoseOrNull` returns null for a free id with no live record, resolves through `poseFromWall` when the record IS present, still recognises the four LEGACY cardinals from `LEGACY_ID_WALL`, and lets a live record beat the cardinal fallback; the wrapper `physicalDoorPose` still falls back to the north-wall centre for a completely unknown id (pinned so a future caller reaches for it knowingly), and agrees with the or-null lookup on every id that resolves. All 140 tests pass (134 pass-1 + 6 new); `npx tsc --noEmit` clean. **Honest limits.** The HULL TILE row is a dev grant today; the consumption gate that requires spending a HULL TILE per grown square (and refunds one per shrunk square) is a follow-up so the STRUCTURE UX and its plumbing could land under the current owner-only permission gate. Per-tile floor variation (the `t:i,j` tile records) still renders the room as one uniform floor — the records are seeded and read but no owner action writes them yet, so the S3 fine-grained tile-editing UX ships in a subsequent slice. The three player-safety fallbacks — the beam-clamp on shrink, the edit-session raycast rebuild, and the `lastAppliedDims` idempotency guard reset in `createPlatform` — are exercised by the unit tests transitively (through the floor-plan doc) but a full scene-integration test remains headless-only. `anyFurnitureOutsideBounds` checks every stored record regardless of its `movable` flag because `MOVABLE_KIND_OVERRIDE` keeps stored records at `false` for the wall-anchored kinds — an item's actual mobility is a KIND-level attribute, not a per-record one, so refusing the shrink on ANY stranded item is the honest test even if it never blocks the shipped defaults today. **Audit verification (pass 3, PR #130 reviewer note 2026‑08‑22).** A reviewer asked whether the exterior collar and vestibule pose recomputations are batched under ONE floor-plan observer or each subscribing independently — the "N observers doing N full rebuilds on one placement write" regression. Verified in code that the batching already exists: `bindFloorPlan` in `floorPlanDoc.ts` installs exactly ONE `planMap.observe(...)` — the sole Yjs-level subscription on the floor-plan map — which fans out through `notify()` to registered listeners. There are exactly two active `subscribeFloorPlan` callers, and only one of them touches geometry: `main.ts` runs `world.reconcileRoomDims()` → (on the short-circuit branch) `world.reconcileDoorPlacements()` → `world.dockingSystem?.refreshPolicyUI()` → `refreshExteriorView()`, where `reconcileDoorPlacements` internally calls `repositionPairedVestibules` (line 1624) and `refreshExteriorView` rebuilds the whole exterior group including the dock-adapter collars (`exteriorView.ts` lines 291–377). `devMenu.ts` is the second caller and only refreshes text rows in the STRUCTURE panel — no rebuild. One placement write therefore fires one Yjs observer notification, one main-thread handler, one door-reconcile pass (covers vestibules), and one exterior rebuild (covers collars). `reconcileRoomDims` short-circuits on same-dims callbacks via its `lastAppliedDims` cache, so door-slide notifies pay only the cheap comparison plus the door-reconcile — never the shell teardown. No consolidation change was made because none was warranted.
