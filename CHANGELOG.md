...
- **🎰 Slot machine — a three-reel cabinet with its own chair ([#109](https://github.com/Bella-Addormentata/StarStationFurlong/issues/109)):** a new furniture item you sit at and play, staking the same chips as the craps table. The payout engine is PURE — `src/games/slots.ts` touches no document and no DOM, so the odds are console-testable and a Chia commit-reveal fairness upgrade drops in around it untouched, the same shape as `games/craps.ts`. Each reel samples a virtual stop without modulo bias and maps it through a symbol strip, so the strip distribution *is* the house edge; the default returns **91.99%** over that distribution, inside the 85–98% a Vegas three-reeler pays.
  ...
  **The honest boundary.** Everything above is bounded by the same fact as the room-authority entry: nothing authorizes a write to a room document. A peer able to write one can enqueue play requests without limit or re-insert a cleared one — the read scans uncapped and no request id is remembered — so this is griefing that the design does not prevent, and it closes when writes are authorized, not before. The [#109](https://github.com/Bella-Addormentata/StarStationFurlong/issues/109) thread carries the per-mechanism detail; this entry deliberately does not restate it, having got it wrong repeatedly in review.
