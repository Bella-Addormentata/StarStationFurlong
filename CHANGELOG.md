...
- **Ship doc lands + first flight ships (#30 SH2 + SH3, dev-phase):** the fuel level and flight state machine (`docked` → `undocking` → `in-flight` → `redocking`) become their own record in the room doc — `src/shipDoc.ts` mirrors the furniture / doors / games binding discipline (rebinds per join at the T0 seam, whole-value LWW writes, shape-guarded reads, hostile-write defaults). The helm console is now the primary flight surface: a live fuel gauge whose capacity derives from mounted `fuel-tank` items (removing a tank silently caps effective level per plan §2), a REFUEL button (owner-gated arm/confirm; dev-phase full-fill until parts-economy metering lands, plan §7 SH6), a DESTINATION picker over Furlong Station / High Orbit / L4 Anchorage, and a DEPART button funneled through a pure `canDepart` predicate that surfaces every refusal reason in-panel — `not-flight-capable`, `not-docked`, `chained-berth` (a permanent connector chain refuses flight *by construction*), `insufficient-fuel` (with needed / have), `unknown-destination`, `no-owner`. DEPART casts off every transient berth (the shipped #67 D2 detach lane), debits fuel, and publishes the record; the owner-side helm's tick then auto-advances `in-flight` → `redocking` when writer-clock `etaAt` passes (clock-skew posture, plan §2 item 3), and the commander completes REDOCK to return to `docked`. The port terminal's INITIATE handler now refuses new berths while the local module is not `docked`, so a peer cannot latch onto a moving station-side target. A 37-test vitest suite covers fuel accounting (conservation, negative/Infinity/hostile writes, capacity clamps, tank-removal strand-overflow), the full state-transition table, the `canDepart` refusal ladder, writer-clock progress interpolation + arrival clamping, and two-peer convergence under `Y.applyUpdate`. All 134 tests green, `tsc --noEmit` clean. **Honest limits:** SH3 first flight is state-machine complete but the physical *re-berthing* after REDOCK is left to the player (walk to the port terminal and re-pair using the shipped #67 D2 flow); the exterior view still renders the module's static atlas neighbourhood during a hop (a starfield swap is a later cosmetic slice); owner writes are UI-gated only — signed write enforcement lives with plan §7 SH5; the parts-economy fuel meter is deferred to plan §7 SH6 (dev refuel is a full-fill).
