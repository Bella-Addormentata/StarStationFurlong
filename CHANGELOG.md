...
- **🦾 ROBOT ARM construction gate (#62, first diegetic build gate):** a new *NEW* pairing at a door now requires a station-mounted robot arm reaching the door — mount an arm on the SAME wall as the door, within 4 m of the door centre, before you can INITIATE a fresh dock *or* PROVISION a fresh module at that door. The arm is a placeable exterior-wall furniture piece (mirrors engine-block's mount pattern) with a low-poly articulated build: base plate, shoulder yoke, upper arm, elbow, forearm, gripper. During pairing it *reaches* — shoulder yaws along-wall toward the door and elbow un-folds — and eases back to parked when the pairing resolves. **Compatibility:** the gate is enforced *only* for NEW pairings — already-paired doors, transient adapter guest berths (#67 D2), and pairings the walkthrough auto-accept (own-minted modules under the dev toggle) all skip it, so existing stations keep working exactly as before. A refusal alert names the missing arm ("no ROBOT ARM installed / on this wall / too far from the door"). Dev-menu PARTS section gains a 🦾 ROBOT ARM row with +1 button, alongside FLEX / EXT / DOCK ADAPTER, and the FURNITURE gallery's + button for a fresh arm now **consumes one grant** (a full room refuses BEFORE consuming; a room-inventory re-place stays exempt because that piece was already granted once). PR-131 review remediations landed: PROVISION seam wired up to the same gate (was documented but not enforced); grant→placement coupling closed; and the retraction animation now preserves the arm's *last live door pose* until the shoulder eases back to zero (previously the fallback zeroed the along-wall delta so the shoulder snapped to parked on the first retraction frame while only the elbow eased). **Honest limits.** The predicate is along-wall distance only (2 m outward reach forward is a fixed animation constant) — a later balance pass may want a broader radial check; the pairing ACCEPT seam is unchanged (a joiner still accepts on trust of the initiator's local gate); the LED on the arm's plate stays green whether idle or reaching (motion is the "armed" signal).
