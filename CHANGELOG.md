...
- **⚠️ BREAKING — room authority: the `Local-Clone` wildcard is retired and destructive surfaces move to the deed holder ([#141](https://github.com/Bella-Addormentata/StarStationFurlong/issues/141)–[#144](https://github.com/Bella-Addormentata/StarStationFurlong/issues/144)).** Four ways an unauthenticated peer write turned into room authority, all found by reading the code during the [#138](https://github.com/Bella-Addormentata/StarStationFurlong/issues/138) ownership analysis, none of them reachable without the capability already known and documented: **there is no write authorization on room documents at all**, so a modified client writes any Yjs record and every gate named here is client-side.
  ...
  **The rest, in brief.** A planted office record is no longer permanent: it can be deregistered from the venture panel behind a two-step confirm, and the repair drops the local ledger entry too, so a forged cap table cannot re-propagate on the next visit. Peer-written `snapshotAt` (venture links) and `updatedAt` (station atlas) are bounded, so a far-future stamp can no longer pin a record against every honest correction; the atlas additionally repairs and **persists** stamps poisoned before the upgrade, clamps what it publishes so a writer never emits what its own reader would refuse, and evicts on a two-tier rule where a room you actually stood in outranks anything gossiped. `stationAtlas`'s cap was a capacity bug as much as an attack — one join absorbs a whole station, so 64 entries is reached in ordinary play at around seven or eight octagon stations.
