...
- **🔒 House commit‑reveal fairness — roulette + craps (#69 G5):** the elected croupier/stickman now publishes a SHA‑256 COMMITMENT to a secret seed on the round record BEFORE bets close, and REVEALS the seed at settle. Every client re‑derives the outcome from the revealed seed via a pure deterministic function (`derivePocket` for roulette, `deriveDicePair` for craps — rejection‑sampled off SHA‑256(seed || tableId || round || counter) block chain so there is no modulo bias for either 0‑36 pockets or 1‑6 dice), and refuses to render the round FAIR unless (a) the reveal hashes back to the commit AND (b) the derived outcome matches the published one. A round with a mismatched commit or a derived outcome that disagrees with the published outcome renders UNVERIFIED in plain‑language ("Do not trust this round/roll") on every viewer's screen; a round with no proof (pre‑G5 rollout on this table, or the documented legacy fallback when the operator hot‑swapped mid‑round) renders LEGACY, deliberately visible so it can never be confused with a G5‑verified outcome. The pure fairness core lives in `src/games/fairness.ts` (domain‑tagged commit + derive hashes, constant‑time hex compare on the commit check, single‑writer operator seed store keyed by `(kind, tableId, round)` that consumes the entry at reveal so a seed can never be revealed twice, `clearFairnessForTable` on `closeTable`/`closeCrapsTable` so a removed table leaves no operator‑side secrets behind), with 48 tests pinning hex round‑trips, the rejection‑sampler's unbiased distribution, the domain‑separated commit and derivation (a roulette commit will not verify against a craps outcome), reveal‑vs‑commit and derivation checks (each failure mode individually), the whole‑pipeline `verifyRoulette`/`verifyCraps` verdicts, and the operator seed store's consume‑once + clear‑by‑table semantics. The auto‑croupier and auto‑stickman are single‑writer per key; the `openBetting`/`openCrapsBetting` and `rollAndSettle`/`rollAndSettleCraps` paths were rewritten fire‑and‑forget with re‑entry `Set` guards keyed by `(tableId, round)` so the 60Hz operator tick cannot stampede the async hash. `LocalCrapsBackend.rollAndSettle` branches on a pre‑committed proof: if the operator holds one for this round it derives the dice from the revealed seed and publishes `houseFairness: {commit, reveal}` on the settled state; otherwise it falls through to the existing multi‑mode `produceRoll` path and the round renders LEGACY. Trust boundary: `isFairnessProof` shape‑guards the peer‑written proof field (well‑formed lower‑hex commit; optional lower‑hex reveal) and both `isRouletteTableState` and `isCrapsTableState` extend to drop a whole state whose `fairness`/`houseFairness` field is malformed — fail SAFE so a hostile peer cannot freeze the verifier. Vitest: 250/250 passing (was 202); `npx tsc --noEmit` green. Honest limits: G4 (per‑bet escrow claims) and G6 (Chia state‑channel settlement) remain blocked on external work — the local backend is now provably fair end‑to‑end, but the Chia backend stays a stub that safely falls back to local until the node wallet + chia‑gaming bridge land.
- **🎲 Craps engine + felt UI — full bank craps (#69 G3):** the pure `src/games/craps.ts` engine now covers the whole standard bank‑craps repertoire — pass line, don't pass, come, don't come, pass odds and don't pass lay odds (true‑odds 4/10 2:1, 5/9 3:2, 6/8 6:5; reciprocal lay), come odds and don't come lay odds, field (2/12 double), place 4/5/6/8/9/10, any 7, and any craps — with come/don't come travel handled at the round level (a fresh come bet whose first roll lands on 4/5/6/8/9/10 travels: `resolveCrapsRound` stamps the come point onto the surviving bet, and subsequent rolls settle it against that stamped number). Stack aggregation (place + all odds types) sums stake by `(type, pick)` before rounding, matching a real dealer's payout on stacked chips — six 1‑chip PLACE 6 bets pay 7 (not six per‑chip 0s), and five 1‑chip pass odds on point 6 pay 11 (not 10). The bet‑shape guard `isCrapsBet` rejects malformed peer writes (stray `pick`/`point` fields, non‑point `pick` on odds bets, unknown types); the felt gains a COME/DON'T COME strip and a PASS ODDS / DON'T PASS ODDS strip that dim when the current point state closes them, with the 3x/4x/5x max‑odds cap enforced at placement — the pass‑side cap uses `crapsMaxOdds` (3F/4F/5F) and the don't side uses the newly‑exported `crapsMaxLayOdds` (uniform 6F under the modern schedule; the win — not the laid amount — is what the schedule caps). **Round‑2 peer‑write hardening (audit remediation for PR #127 review):** `passodds` and `dontpassodds` now BIND to the current pass‑line point at settle time (`bet.pick` must equal `pointBefore`) and FAIL CLOSED — any peer‑injected odds bet with a mismatched pick, including any pass‑side odds bet during a come‑out, loses immediately so a hostile peer cannot bypass `canPlaceBet` and siphon odds payouts against the wrong number; `comeodds` and `dontcomeodds` correctly settle against their OWN come point independent of pass‑line phase (traveled come/don't‑come bets are always working, and so are their odds — a `dontcome` backing 6 with `dontcomeodds` now wins both on a pass‑line come‑out 7, matching real casino rules); the don't‑side UI cap in `devices.ts` switched from `crapsMaxOdds` to `crapsMaxLayOdds` so the felt allows the full advertised lay (previously undercut by 2×); and the 4/10 true‑odds explanation is corrected to state the actual math (six ways to roll 7 vs three ways to roll 4/10 → 6/3 = 2) instead of the earlier misleading gloss. Comprehensive vitest suite pins every payout table, the come‑bet travel lifecycle, the stack‑aggregation semantics, phase gating (`canPlaceBet`), the trust‑boundary guard (including the round‑2 fail‑closed cases for passodds/dontpassodds mismatch and the come‑out come‑side working proofs), the `crapsMaxLayOdds` helper, and the shooter‑hand narration (`rollCall`) — 105 tests in `craps.test.ts` (was 97), 202 total across the frontend suite (was 194), all green alongside `tsc --noEmit`. Honest limits: the come‑odds / don't‑come‑odds UI affordance is engine‑only for now (no per‑come‑point felt hitbox), so those bets are placeable via peer writes and console but not yet by click; the fair‑dice modes are still dev‑simulated locally (block‑beacon and multiparty second parties are wired end‑to‑end but produce dev entropy until the network/chain half lands).
