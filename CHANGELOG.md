...
- **BANK send/receive — in-world chips between two players (issue #20 slice — client-side only, game-first):** the BANK app grows a `SEND CHIPS` button inside the room's CHIPS section and a `TRANSFERS` audit block (Received / Sent) beneath it. Recipient list is the current room's players map minus yourself and minus anyone on your local block list — legacy players without a `keyB64` are hidden from the picker (a signed transfer needs a pub for both parties). Two-step confirmation (first click on `SEND` arms, second click on `CONFIRM SEND` writes) mirrors the deed-handover pattern; any change to picker/amount re-arms. Refusals speak plain-language: `"Not enough chips — you have N, tried to send M."`, `"Pick a recipient from the list."`, `"Enter a whole number of chips greater than zero."`, and (on a race with a concurrent bet burning the balance between arm and send) `"Send refused — either your chips just moved, or the room is full of transfers. Try again."`. A new module `src/casinoTransfers.ts` is the pure engine (no Yjs, no `@noble` import — verifier injected): domain-tagged deterministic sha512 id (`ssf-chip-xfer-id:v1`), shape guard (`isChipTransfer` — kind + version + every field type/range, self-transfer rejected, id must recompute from the candidate fields), entry guard (`isValidChipTransferEntry` — chains the value guard AND requires KEY === value.id so a hostile peer's mismatched-key entry cannot smuggle a phantom row past the audit log), Ed25519 signature over domain-tagged canonical bytes (`ssf-chip-xfer:v1`), deterministic over-drain replay (`partitionValidTransfers` — sorts by (ts asc, id asc) and refuses any transfer whose sender's lifetime outgoing would exceed their sender-side issuance `bought − cashed`; conservative rule — received transfers do NOT credit the sender's budget, so the audit log's floor of honesty is that a mint-attempting hostile write is dropped by every reader without coordination), plus block-list partition helpers that return `{visible, hidden}` (incoming from a blocked identity is HIDDEN from the audit UI but the physical `bal:<to_pid>` credit that already landed at write time is still counted — conservation preserved, matching the #20 audit r2 block-follows-rotation sibling rule). Bound to the room casino Y.Map by `casinoDoc.ts` at `xfer:<id>` records — `writeChipTransfer(t)` moves `bal:<from_pid>` DOWN and `bal:<to_pid>` UP inside the SAME `boundDoc.transact` as the record, so `Σ bal:*` is instantly unchanged by any transfer (physical conservation invariant, test-locked). Own-write checks reject over-drain, double-send (deterministic id ⇒ existing key collision), unsafe-integer overflow on the recipient's next balance, and a `MAX_CHIP_TRANSFERS = 512` flood cap. The BANK repaints on any casino-map change (`subscribeCasino` wired next to `subscribeVentures` at boot); block-list changes clear any half-armed send state so a just-blocked recipient can't survive as an armed confirmation; room switch clears send state so a stale arm from the previous room can't fire against a coincidentally-matching pid. Registry / Chia settlement stays blocked-external (the current per-room casino doc is the ledger of record for this slice). Test coverage: `src/casinoTransfers.test.ts` (37 tests — id derivation determinism, shape/entry guards accept + reject, sig verification with throwing-verifier defence, deterministic replay under any input order, per-sender-independence, conservation invariant Σ deltas = 0, conservative sender-budget rule, filters, partitions, factory round-trip); `src/casinoDocTransfers.test.ts` (11 tests — atomic debit + credit + record, Σ bal:* conservation across mixed transfers, cage counters unchanged, over-drain refusal, insufficient-balance refusal, double-send refusal on deterministic id, malformed-transfer refusal at write time, flood-cap refusal, hostile peer's mismatched-key row dropped on read, well-formed rows survive, end-to-end conservation across buyIn → transfer → cashOut with `houseNet === 0`). 48 new tests + 234/234 total across 8 vitest modules. Frontend-only; no node behavior, no version bump, no new dependencies.
