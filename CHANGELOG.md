...
- **Ship doc lands + first flight ships (#30 SH2 + SH3, dev-phase):** the fuel level and flight state machine become their own record in the room doc — `src/shipDoc.ts` mirrors the furniture / doors / games binding discipline (rebinds per join at the T0 seam, whole-value LWW writes, shape-guarded reads, hostile-write defaults). Two legal DEPART paths land in the state machine — the SH3 **fast path** `docked → in-flight` shipped here, and the reserved **slow path** `docked → undocking → in-flight` kept in `isLegalFlightTransition` so a future preflight-animation slice can restore the visible cast-off beat without touching the state room. Every `writeFlightRecord` call now enforces BOTH the shape guard AND the legal-transition table against the current record (defense in depth on top of the shape-guarded read; a rejected write is louder than a silently-reverted one, and honest-client posture keeps this a local-writer sanity check pending SH5 signed enforcement). The helm console is now the primary flight surface: a live fuel gauge whose capacity derives from mounted `fuel-tank` items (removing a tank silently caps effective level per plan §2), a REFUEL button (owner-gated arm/confirm; dev-phase full-fill until parts-economy metering lands, plan §7 SH6), a DESTINATION picker over Furlong Station / High Orbit / L4 Anchorage, and a DEPART button funneled through a pure `canDepart` predicate that surfaces every refusal reason in-panel — `not-flight-capable`, `not-docked`, `chained-berth` (a permanent connector chain refuses flight *by construction*), `insufficient-fuel` (with needed / have), `unknown-destination`, `no-owner`. DEPART casts off every transient berth (the shipped #67 D2 detach lane), debits fuel, and publishes the record; the owner-side helm's tick then auto-advances `in-flight` → `redocking` when writer-clock `etaAt` passes (clock-skew posture, plan §2 item 3), and the commander completes REDOCK to return to `docked`. **Every pairing completion is flight-gated at the shared seam:** a new pure `pairingAllowedByFlight()` predicate in `shipDoc.ts` refuses berth completion in every non-`docked` status, wired into `completePairing`'s accept branch AND both the INITIATE and ACCEPT UX handlers in `docking.ts` — a remote peer's pending request cannot land on an in-flight module even when the local user clicks ACCEPT (round-2 audit MAJOR remediation; the previous fix only gated INITIATE). The **fuel-money boundary** is now named in the plan doc (`brainstorming/spaceship-conversion-plan.md` §1.2 + §8.2): LWW on a clamped integer is fine only while fuel has no exchange path; the moment fuel becomes purchasable with an offer slip / deed / treasury payout, the DEPART debit becomes a transfer between distrusting parties and the schema changes (paid-decrement receipts). The helm's derivation walk over `functions`-tagged furniture is **verified NOT per-frame** — grep-audited to run only from doc observers, the mount() paint, discrete user gestures, and a 4 Hz HELM_TICK_MS setInterval that re-renders only while in-flight; the function's docstring records the audit trail so a future move requires a deliberate cache decision. A 47-test vitest suite covers fuel accounting (conservation, negative/Infinity/hostile writes, capacity clamps, tank-removal strand-overflow), the full state-transition table (fast + slow DEPART paths), the writer-side transition-legality gate (illegal edges rejected, healing over hostile current records, idempotent self-republish under commander contention), the `canDepart` refusal ladder, the **new pairing-completion gate** (docked accepts, undocking/in-flight/redocking each refuse with the exact status carried back for alert copy, hostile flight record heals to accept), writer-clock progress interpolation + arrival clamping, and two-peer convergence under `Y.applyUpdate`. All 144 tests green, `tsc --noEmit` clean, `vite build` succeeds. **Honest limits:** SH3 first flight is state-machine complete but the physical *re-berthing* after REDOCK is left to the player (walk to the port terminal and re-pair using the shipped #67 D2 flow); DEPART's transient-berth cast-off calls `deleteDoorPairing` (local-only detach) so an observer viewing the previously-adjacent room from another peer sees a phantom berth to the departed ship until they take an action that heals it — the transient-berth contract legalizes either-side detach (#67 D2); the exterior view still renders the module's static atlas neighbourhood during a hop (a starfield swap is a later cosmetic slice); owner writes are UI-gated only — signed write enforcement lives with plan §7 SH5; the parts-economy fuel meter is deferred to plan §7 SH6 (dev refuel is a full-fill).
