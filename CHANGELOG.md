...
- **🪙 Coin pusher remediation (#135, pass 5).** One r4 MINOR audit finding against the pure engine's wall-clock anchor landed with regression tests. **(MINOR 1) `state.pusherAtMs` drifted ahead of wall clock per insert, freezing the render.** `processInsert` step 5 always ran `advanceSim(cur, SETTLE_MS, playerId)`, whose substeps accumulate `dtMs` into `pusherAtMs` — on return, pusherAtMs sat at `caller-nowMs + SETTLE_MS`. Step 1 then SKIPPED the between-advance on the next insert (nowMs' <= pusherAtMs), so the SETTLE_MS offset never got consumed; every insert stacked ANOTHER SETTLE_MS on top, and after 10 inserts pusherAtMs was 24400 ms ahead of wall clock. The renderer's `currentPusherPhase(state, Date.now())` computes `dt = max(0, nowMs - state.pusherAtMs)` and returns bare `state.pusherPhase` whenever dt clamps to 0, so the cabinet in `furniture.ts` froze between the operator's 4 Hz publishes instead of interpolating per render frame — defeating the whole point of `currentPusherPhase`. Fix re-anchors `pusherAtMs = nowMs` after step 5 in `processInsert` (pusherPhase kept at its post-settle value so physics continuity is preserved; only the wall-clock timestamp is corrected). Companion change in `devices.ts::tickCoinPusherOperator`: the free-running physics dt now derives from `state.pusherAtMs` instead of a private `lastTickAt` cursor, self-correcting when a drain-time insert just re-anchored (dt = 0, no double-simulation) and catching up cleanly when the state has been idle. Conservation, tick monotonicity, determinism, and the atomic settle behaviour all hold; the pure engine still never reads Date.now(). Seven new vitest cases pin the invariant (pusherAtMs === caller nowMs after each insert; render interpolates for wall-clock > nowMs; phase continuity across a settle; conservation across a 50-insert burst; nowMs=null escape hatch still yields legacy SETTLE_MS advance; long-idle-then-insert snaps to nowMs; multi-insert same-tick drain all end at pusherAtMs === nowMs). Gates: `npx tsc --noEmit` = 0 errors; `npx vitest run` = 198/198 passing (70 engine + 31 wiring + 67 treasuryTypes + 30 treasuryDoc — 7 new engine cases, no regressions).
- **🪙 Coin pusher remediation (#135, pass 4).** Four r4 audit findings against the wiring layer landed with tests. **(MINOR 1) `escrowedAt` was client-copied, not server-stamped.** `submitCoinPusherInsert` copied `request.requestedAt` verbatim into the escrow record even though the r3 comment claimed a server stamp. Fixed by giving the function an optional `nowMs` parameter (default `Date.now()`) and stamping `escrow.escrowedAt = nowMs` inside the same transact that debits balance and writes the request; tests demonstrate `requestedAt = MAX_SAFE_INTEGER` no longer locks the submitter's own escrow and `requestedAt = 0` no longer lets them bypass the TTL. **(NOTE 2) Two-transact settle window.** The operator tick published state (transact 1) and then cleared the request record (transact 2) as separate atomic units — an interleaving failure between them would leave the request live while the chip was already in machine state, letting a next-tick re-drain mint the chip AGAIN. New `publishAndClearCoinPusherInsert` wraps the state set, request delete, and escrow delete in ONE Yjs transact; a subscriber sees a SINGLE `update` event covering all three effects, and a bad `nextState` shape rejects without touching the map. **(NOTE 3) Ownership lockout by hostile ownerId overwrite.** `state.ownerId` was peer-writable, so an attacker could stamp a bogus id and permanently prevent the true room owner's operator loop from starting. `ensureCoinPusherInitialized(mid, isHouse=true)` now HEALS on foreign-ownerId observation — it re-writes state with the current owner's id while preserving piles, `pendingCredit`, and every counter (money conservation intact across the heal), and bumps `tick` so any peer watching the state sees a monotonic transition. Same code path also covers the legitimate "new room owner takes over from an offline predecessor" scenario. **(NOTE 4) Placeholder operator try/catch test.** The r3 test suite included a placeholder that admitted it did not exercise the operator's try/catch. Added an end-to-end integration test that faithfully emulates the drain loop and forces `processInsert` to throw on one request — asserting the throwing request's escrow is refunded, the poison record is dropped, and later requests in the same drain still settle cleanly (no chips minted from the failed insert). Gates: `npx tsc --noEmit` = 0 errors; `npx vitest run` = 191/191 passing (63 engine + 31 wiring + 67 treasuryTypes + 30 treasuryDoc — 9 new wiring cases, no regressions).
- **🪙 Coin pusher remediation (#135, pass 3).** Eight audit findings against the wiring layer — the seam between `casinoDoc.ts` and the operator loop in `devices.ts` — landed here as fixes with a new 22-case vitest suite that exercises the mint/burn/lock trust boundary directly. **(BLOCKER 1) Operator loop minted chips on unbacked requests:** `tickCoinPusherOperator` read every `pusher-req:<mid>:<pid>` record and called `processInsert` unconditionally. A hostile peer with write access to the casino map could bypass `submitCoinPusherInsert` and stamp a synthetic request with any ante — the operator credited `state.pendingCredit[attacker]` from thin air, and a follow-up `claimCoinPusherPayout` turned the credit into real `bal:<attacker>` chips. The drain now READS the matching `pusher-esc:<mid>:<pid>:<reqId>` escrow first via a new `readCoinPusherEscrow`; on missing/mismatched escrow it calls a new `operatorRefundCoinPusherRequest` teardown (which deletes the poison request without minting) and continues. A `simulated operator drain — mint-attack PoC` test writes a forged request into a fresh doc, spins the drain, and asserts zero chip movement. **(BLOCKER 2) `clearCoinPusherKeys` burned foreign escrows on removal:** the teardown deleted every `pusher-req:*` and `pusher-esc:*` key unconditionally; `world.ts` only refunded escrows the LOCAL peer had written, so any escrow authored by another peer was silently destroyed on cabinet removal. `clearCoinPusherKeys` now walks the escrow prefix BEFORE the delete, aggregates refunds per player (guarded against key/value pid mismatch and MAX_SAFE_INTEGER overflow), and issues a single atomic `transact()` that credits every affected `bal:<pid>` alongside the delete — so a remote peer's replay sees an already-credited balance. **(MAJOR 3) Seed derivable from wall-clock second:** the old seed was `Math.floor(now/1000) ^ (req.requestedAt >>> 0)` — an owner-operator could delay their tick to a favourable second to pre-compute an unfavourable trajectory. Replaced with a pure `seedFromRequestId(req.requestId)` (FNV-1a 32-bit over the UUID-bearing string); the physics is identical, but the seed no longer varies with the tick's wall-clock arrival. **(MAJOR 4) One poison request halted the queue forever:** `readCoinPusherRequests` sorts by requestId, so a request that made `processInsert` throw (e.g. hostile out-of-range ante) blocked every later request behind it in perpetuity. Per-request `try/catch` now refunds the escrow via `operatorRefundCoinPusherRequest` and drops the request so the drain continues. **(MAJOR 5) 4 Hz per-machine O(all-keys) idle poll:** each tick called `readCoinPusherRequests(machineId)`, which scans every key of the shared casino map. Added a prefix-scoped write-generation counter — `casinoWriteGeneration()` and `casinoPrefixWriteGeneration(prefix)` — bumped by `notify()` when a key matching the prefix changes. The operator caches its last-observed `pusher-req:<mid>:` generation and skips the scan entirely when nothing has moved (the free-running physics substep still runs, so idle attract-mode sweep animation stays live). **(MINOR 6) TTL gated on client-provided `requestedAt`:** a hostile peer could stamp `requestedAt = Number.MAX_SAFE_INTEGER` and lock their own escrow past `2^53`. `refundExpiredCoinPusherRequest` now ages on `escrow.escrowedAt` (server-side, stamped inside `submitCoinPusherInsert`'s transact), which no peer other than the escrow's submitter can influence. **(MINOR 7) Non-monotonic same-ms publishes:** a fast wall-clock (or two publishes in one tick from the drain + physics arms) could write two states with the same `tick` number. Added `publishCoinPusherState(machineId, state, op)` that no-ops when `state.tick <= op.lastTick`, so peers using tick number as a naive deduper always see strictly increasing values. **(MINOR 8) Zero wiring-layer test coverage:** the engine suite (63 cases) tested only the pure module — no test held the map-level trust boundary. New `src/casinoDoc.test.ts` (22 cases) covers `submitCoinPusherInsert` atomicity, the mint-attack PoC, the atomic-refund-on-teardown case, TTL-from-escrowedAt, the safe teardown of the operator refund path, `commitCoinPusherEmpty` owner gating, the prefix-generation counter's edge cases, and an end-to-end conservation identity across the wiring layer (Σ `bal:` + Σ `chipsInMachine` + Σ `pendingCredit` == Σ `buyInChips`). Gates: `npx tsc --noEmit` = 0 errors; `npx vitest run` = 182/182 passing (63 engine tests + 22 wiring tests + 67 treasuryTypes + 30 treasuryDoc — no regressions).
- **🪙 Coin pusher remediation (#135, pass 2).** Three audit findings from the initial PR landed here as fixes with regression tests. **(1) Double-credit of between-insert fallouts:** `processInsert` step 6 re-applied the step-1 `paid[]` accumulator (chips that `advanceSim` had already credited to `pendingCredit[player]` and `totalPaid`) as a second credit — silently minting chips into the player's pending ledger and inflating `totalPaid` past the conservation identity. The two aggregates are now separate: `betweenPaid` (step-1, already credited) is threaded ONLY into the returned `paidChipIds` for the caller's log; step 6 credits ONLY `instantPaid` (step 3's direct writes to `upper`/`lower` that no other layer accounted for). Three new vitest cases construct the exact upper-past-front + full-lower-chain scenario the audit called out — they fail on the pre-fix code (`totalPaid == 8` where 4 chips actually fell) and pass on the fix. **(2) Idle physics attribution:** the operator's ambient-tick call passed `payoutTo=null` to `advanceSim`, so any chip that tipped off between inserts bumped `totalPaid` with NO player credited — a wiring-layer chip vaporised. The operator now passes `state.ownerId`; idle sweep accrues to the machine owner (matching the owner-empty precedent). The engine header for `advanceSim` now spells out the `null` contract as "dev-inspection only — the wiring layer MUST NOT pass null." A regression test in `advanceSim — audit #2` pins the routing. **(3) Selection-mesh mixup:** `setSelectedHole` and `triggerDropFx` in `furniture.ts` used a single `holeMeshes[]` array whose ownership was ambiguous — the current visitor was highlighting the wrong sub-mesh under some hole layouts. The array is split into `holeRimMeshes` (for selection tint) and `holeDiscMeshes` (for drop-fx animation) so each visual is addressed by its explicit role. Gates: `npx tsc --noEmit` = 0 errors; `npx vitest run` = 160/160 passing (63 coin-pusher tests including 4 new regression cases).
- **🪙 Coin pusher cabinet (#135) — a spawnable arcade coin pusher.** Drop the cabinet from the dev menu; walk up, pick one of three drop holes and time the release, and a chip lands on the upper platform. A pure deterministic engine (`src/games/coinPusher.ts`, seedable RNG, 40 ms substeps, 59-case vitest suite) advances two platforms of stacked chips — a rocking pusher bar spills the front row forward, the upper platform tips onto the lower, and the lower tips off the front edge into the tray as your payout. Every insert is escrow-transacted through `casinoDoc.ts` so a debit + queued request land atomically; the machine's owner is the sole operator, drains queued inserts at 4 Hz, and republishes the reduced state via a shape-guarded whole-value LWW key. Chips STAY IN THE MACHINE until they physically tip off — the owner has an OPEN DOOR button (owner-only; the pure engine's `emptyMachine` is a no-op for anyone else) that credits the accumulated pile to the owner's chip balance in one transact. Payouts to non-owners settle atomically off `state.pendingCredit[player]`. The engine enforces `totalInserted == chipsInMachine + totalPaid + totalEmptied` at every step; the DeviceUI shows the current delta so drift is visible. A stuck request expires after 90 s and any client entitled to the escrow (its own player) can refund itself — nothing is auto-siphoned to anyone else. Plan: [brainstorming/coin-pusher-plan.md](brainstorming/coin-pusher-plan.md).
