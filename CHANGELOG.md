...
- **Treasury PR C — read-only treasury UI (dev-only, no runtime money path):** a 🏦 TREASURY view opens from 🚀 VENTURES on the phone, reading the room's signed treasury caches. It shows this room's funding, the company's board and threshold, policy version and fingerprint, share classes, a proposal list, and a per-proposal detail with recomputed voting/veto/timelock/expiry clocks, votes held in the room, and board-approval progress. The room terminal gains a **FUNDING** section (personal vs company, company identity, policy version, explicit read-only state) with `REQUEST COMPANY FUNDING` / `SELECT PROFILE` / `UNBIND` / `REFRESH PROOF` present but disabled. Everything is display only — the phone holds no treasury key, decides no balance, and declares nothing executable. Each panel carries a badge saying how much checking actually happened (signature verified, self-checked against its own fingerprint, or unverified shape), balances report that no verified source exists yet rather than showing a number the browser cannot verify, proposal clocks are recomputed from the acceptance record rather than trusted from a peer's copy, and a phase is only claimed when a height is available — labelled with where that height came from. Display logic lives in the unit-tested `src/treasuryView.ts` (u64-safe amount formatting — mojo strings exceed `Number.MAX_SAFE_INTEGER`); `src/treasuryNetwork.ts` adds the network-pin configuration seam (`VITE_SSF_TREASURY_GENESIS`), which stays on a placeholder that matches no real chain until a build is configured.
