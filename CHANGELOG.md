...
- **Treasury PR B completed — `treasuryDoc.ts` signed caches:** the plan §14 treasury Y.Map lands with verify-on-read for every slot (guards → id recomputation → signature verification; hostile peer writes — junk, misfiled decoys, slot squatters, encoder-hostile strings — are skipped or overwritten, never trusted), vote slots keyed by the gameSig-authenticated `voterGamePub` with the §7.2 canonical rule per slot (greatest per-voter sequence, equal sequences resolve to the smallest voteId; `pickCanonicalVote` exported for the tally-time puzzle-hash dedup), commutative signature-set merging for zero-authority signing sessions, a content-addressed `payload:<payloadHash>` replication lane (sovereign §5 data availability), policy-version monotonicity on the policy cache, and offers-style fail-loud behavior when no room doc is bound. Supporting contract additions in `treasuryTypes.ts`: `voteSignatureBytes` and `payloadHashOf` (both golden-vectored and mirrored by the Rust codec), browser-only `roomBindingSignatureBytes`, and record guards for proposals, votes, checkpoints, signing sessions, registrations, windows, bindings, and receipts. Bound at the room-join seam in main.ts with keypair identity verification and a configured network-genesis pin (§17.5): correctly signed records from any other network are rejected everywhere. Session ids are derived (`signingSessionIdOf` hashes the immutable shell) so shell conflicts are structurally impossible, and collected signatures live one-key-per-signer so CRDT sync itself unions partitioned collections; receipt bodies are immutable under their id (only confirmation status may change).
