...
### 7.7 — The ChiaHub floor beneath the mesh (rung 6) *(cross-link, added post-Slice-3)*

> Companions: [REVIEW-20260710-ChiaHub.md](REVIEWS/REVIEW-20260710-ChiaHub.md) (the full design) · [STUDY-Architecture v007 §4](AI%20BRAINSTORMING/STUDY-Architecture%20v007.md) (the reachability ladder) · `ssf-p2p-node/src/chia_lane.rs` (the C0 scaffold: record types + sign + room-key seal + epoch hints, unit-tested, **no chain IO yet — gated on spike B-7**).

The mesh (M1–M4) and ChiaHub are **two rungs of one ladder, not alternatives.** M4 is **rung 4** — a *live* relay/introduce through a reachable friend's node (fast, carries traffic, needs a friend online *now*). ChiaHub is **rung 6** — the always-on **discovery + punch-coordination floor** (slow, permanent, needs nothing online but the chain the economy already assumes). Between them sits rung 5 (a volunteer player beacon, the only *other* traffic fallback). The one-line boundary from the ChiaHub review governs everything here: **the chain can be the swarm's address book and its metronome, but never its pipe** (~19 s blocks vs 20 Hz datagrams).

They compose at three concrete seams — this is where the Phase-4 code hands off to the chain lane:

1. **`hintsFor(pub) → null` is the fall-through trigger.** The M1 peer store holds a friend's *identity key* but their *route* can go dead (they changed networks; the card hints are stale). When no live mutual is reachable to introduce (rung 4 exhausted), resolve the friend's **current** presence record from the chain and feed those addrs back into the normal dial path. ChiaHub is precisely what makes M1's "identity known, route unknown" state recoverable *with zero live infrastructure*.
2. **Block-height is the sovereign punch broker** when no live friend-broker exists. M4's gossip-dial/punch needs a live third party for simultaneity; ChiaHub §4 replaces it with the chain's peak height as a shared clock (publish punch-intent/ack keyed to a future block, fire on each new peak). Connects firewalled-but-not-symmetric pairs — IPv6 especially — with **no reachable friend needed at all**.
3. **Identity-keyed presence — the evolution the ChiaHub review predates.** That review (2026-07-10) keys presence records by **room key + iroh node_id** ("the room is the key extends to the chain"), because at the time there was no durable *person* identity. Slice 1 changes that: every person now has a durable Ed25519 pubkey, and the node already holds it (the `players`-map self-cert). So presence can be addressed by a person's **identity key** — "find where *Bob* is right now" independent of any shared room. The peer store's friend identity keys *become* the on-chain lookup keys; the contact card's hints become a **cache** of what the chain holds authoritatively. This also cleanly answers `chia_lane.rs`'s open **[C1-DESIGN]** question ("the node never sees room *keys*, only room ids"): identity-keyed presence sidesteps the room-key-plumbing problem entirely for the *person-discovery* use case (room-keyed presence remains for room rendezvous).

**Net effect on the reachability dependency** (extends the M4 analysis): ChiaHub drives *discovery + coordination* reliance toward **zero** — you can always find each other and always attempt a punch, even when every relay/DHT/DNS/friend is down. It does **not** remove *traffic-relay* reliance: a symmetric-NAT-both-sides pair that can't punch still needs a live relay (rung 4 friend-relay or rung 5 beacon), i.e. *someone* reachable in the trust graph — just never the specific peer you're dialing. So the honest summary across M4 + ChiaHub: **"you'll always find them and always try; you'll connect unless physics forbids a direct path *and* no reachable node exists to carry bytes."**
