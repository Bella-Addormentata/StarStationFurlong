...
- **Rooms stay cardinal; the connector takes the turn** — generalized: *every wall run of an axis-aligned tile set faces a cardinal, and a door's id IS its facing.* `DoorId` stays the 4-value union (~160 references untouched); camera detents and the facing-fade dot products (`docking.ts:74-79`) hold at every stage.
  - *Superseded 2026‑08‑20 (#102), not deleted:* the invariant is unchanged; what's newly stricter is the pose-helper API. `physicalDoorPose` / `physicalDoorPoseOrNull` / `physicalPortPose` / `projectionPoseForDoor` now take the room's door layout as an explicit `DoorRecordsMap` parameter (`doorLayout.ts`, `adapter.ts`), so a caller resolving the ARRIVAL room's doors never reads the departure room's module-global (the ordering hazard: `setDoorRecords` runs AFTER `completeAdapterArrival`). This is the exact seam this plan needs anyway — the derived-registry work of S1 (§5.1 stage 3) now has a per-room slot layout to feed the helpers explicitly, rather than a swap-in-place global.
...
- **Invariant audit:** "rooms stay cardinal; the connector takes the turn" — **preserved by construction**: a slot may only sit on an exterior run facing its own cardinal, so every door normal stays cardinal; bends stay in the connector. The *secondary, previously implicit* assumption — "door at the axis midpoint of a ±5.9 wall" — is consciously demoted from constant to per-door data, versioned additively via the registry (near) and the two new fields (far).
  - *2026‑08‑20 update (#102):* the per-door-data demotion is now enforced at the pose-helper API boundary, not just at the writer. `physicalDoorPose` / `physicalDoorPoseOrNull` / `physicalPortPose` / `projectionPoseForDoor` accept an explicit `DoorRecordsMap` parameter that wins over the module-global (the fallback that returns `LEGACY_ID_WALL[id]` when the layout says nothing about `id` stays — that's the "empty ⇒ legacy" contract). Additionally, `projectionPoseFromWall`'s chained-connector branch returns `rotY: 0` on an unknown far wall (matching the straight-gangway branch), so the R2/§4 far-door pose is authored data or zero — never a heading-derived guess. The three corrections vs the pre-#102 text: (i) helpers are no longer bare-argument reads of a global; (ii) unknown far wall ⇒ no rotation, not a heading fallback; (iii) callers that resolve arrival poses must pass the arrival room's snapshot explicitly (the `applyRoomVisuals` → `setDoorRecords` ordering hazard).
