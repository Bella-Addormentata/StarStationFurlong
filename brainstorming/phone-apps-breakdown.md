...
**Follow-up edge — block-follows-rotation (recorded per PR #136 reviewer note 2026‑08‑21):** the block set stores base64url Ed25519 identity pubs (blockList.ts) because that is the durable cross-room stable id today. When issue **#124's** keyed-identity rotation path lands (authority-head `seq` bump), a rotated key mints a NEW pub, so an existing block will not follow it. The rotation slice must record the pre-image → post-image pub link and rewrite the local block set on rotation-observed (walking `ssf-blocked-pubs`, replacing any pub whose authority head advanced) — otherwise a rotation silently un-blocks. Same edge applies to the local contacts roster and any DM history keyed by pub. Land it inside the #124 rotation slice, not here.
