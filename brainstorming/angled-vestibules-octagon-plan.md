...
- **Rooms stay cardinal; the connector takes the turn.** `DoorId` stays the 4-value union (~160 references untouched); no room ever grows a non-cardinal door. The art agrees.
  - *Superseded 2026‑08‑20 (#102), not deleted:* the invariant holds, but the pose-helper API is now stricter about which room's layout is being read. `physicalDoorPose` / `physicalDoorPoseOrNull` / `physicalPortPose` and the projection helper `projectionPoseForDoor` in `doorLayout.ts`/`adapter.ts` now take the room's door layout as an explicit `DoorRecordsMap` parameter that wins over the module-global — a caller reasoning about the ARRIVAL room's doors gets the arrival answer instead of whatever the module-global happened to hold. `world.completeAdapterArrival` captures the arrival snapshot at entry and threads it through `resolveArrivalDoor` and the R2 station-bias math. The "no room ever grows a non-cardinal door" clause is unchanged (DoorWall is still N/S/E/W); the change closes the ordering hazard that `applyRoomVisuals`'s `setDoorRecords` runs AFTER the arrival hook — implicit reads of the departure room's global while resolving arrival poses would produce silent-180 arrival headings.
...
| `resolveArrivalDoor` (`world.ts:1563-1570`) | Hardcoded opposite-cardinal map | **Breaks**: departing east through a 90°-total-bend chain can arrive at the target's *east* door. Prefer the record's `farDoor`, keep opposite-cardinal as legacy fallback. **2026‑08‑20 update (#102):** `resolveArrivalDoor` now additionally takes an `arrivalLayout?: DoorRecordsMap` parameter so its internal `wallOfDoor` reads the arrival room's slot layout — not the module-global (which still holds the departure room's records at that call site). Also: `projectionPoseFromWall`'s chained-connector branch returns `rotY: 0` on an unknown far wall (matching the straight-gangway branch); a guessed rotation is a silent R2 violation. |
