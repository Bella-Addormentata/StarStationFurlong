...
- **Fuel is NOT money in v1 — but the boundary is named.** LWW writes to a clamped integer are fine ONLY while fuel has no exchange path with anything real. The debit at DEPART is a shared decrement every client observes; today it is a game resource with no offer/deed backing, so `writeFuelLevel(level, capacity)` clamps to derived capacity and calls it done. The moment fuel becomes purchasable with an offer slip (#68), a deed (`deeds.ts`), a company treasury payout (`company-treasury-governance-plan.md`), or any other asset that carries authority, the DEPART debit becomes a TRANSFER between distrusting parties and the trust tier changes: LWW on the integer is no longer sufficient, the write side needs an atomic paid-decrement receipt on the ship map (or the offer slip / treasury proposal that debits it lands atomically with the fuel credit on the seller side), and the readers must verify the receipt against the signing key. **Recorded here so the boundary is chosen deliberately, not discovered at rollout** — the transition point is not in the v1 slices (SH1–SH5 all remain LWW-on-clamped-integer); it lives at the SH6 parts-economy / refuel-economy slice named in §8 item 2.
...
2. **Deep-space content — and the refuel-economy money boundary.** Destinations are flavor entries; mining/encounters/markets unscoped. Refuel economy likely rides #68 offer slips — and this is where the trust tier for fuel flips (§1.2 invariant): the moment fuel is purchasable with an asset that carries authority (an offer slip, a deed, a treasury payout), the DEPART debit becomes a transfer between distrusting parties, LWW on a clamped integer is no longer enough, and the ship map needs a paid-decrement receipt (or an atomic dual write against the seller's asset doc) that readers verify against a signing key. Name that transition the "fuel-money boundary" so a future refuel-economy slice knows it is inheriting a schema change, not a cosmetic follow-up.
