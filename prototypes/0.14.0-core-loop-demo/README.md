...
```bash
cd /path/to/StarStationFurlong/prototypes/0.14.0-core-loop-demo
```
...
│   ├── map.ts           # 2D Solar System map (elliptical, Kepler, Lagrange)
│   ├── zoom.ts          # multi-scale 8 zoom levels + first person perspective camera + pointer locked mouse look + eye blinking
│   └── network/         # WebTransport + Yjs sync networking (Sprint 3)
├── ssf-p2p-node/        # Standalone Rust direct Iroh Swarm P2P node
...
Check the following:
1. Make sure you are inside the `0.14.0-core-loop-demo` folder.
