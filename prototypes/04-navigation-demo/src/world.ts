 ...
 * Demo 04 adds a click-navigation plane for point-and-click pathfinding.
 */
...
export class World {
  ...
  /**
   * Returns the invisible click plane mesh so main.ts can add it to the
   * raycaster's target list.  Returns null before the morph has started.
   */
  getClickPlane(): THREE.Mesh | null {
    return this.clickPlane;
  }

  /**
   * Navigate the player to a world-space destination (x, z) via A* pathfinding.
   * Called from main.ts when the user clicks on the floor.
   */
