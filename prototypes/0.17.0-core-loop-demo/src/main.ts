      ...
      if (env.kind === 'bridge' && typeof env.payload === 'string') {
        try {
          ...
          if (status.status === 'dialing') {
            ...
            logToPhoneSystem(`⚠️ P2P dial to ${shortTarget}… failed: ${status.detail ?? 'unknown error'}. If both stations are behind home routers, one side needs a port-forward (SSF_IROH_PORT) or a relay (SSF_RELAYS).`);
          }
        } catch (e) {
