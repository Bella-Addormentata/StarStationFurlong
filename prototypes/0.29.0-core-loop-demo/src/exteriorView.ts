...
const gr = (): GameRenderer =>
  (window as unknown as { gameRenderer?: GameRenderer }).gameRenderer ?? {};
type WorldRef = {
  dockingSystem?: {
    editChainSegment?: (
      doorId: string,
      index: number,
      patch: { bendDeg?: number },
    ) => boolean;
  } | null;
...
function applyExteriorZoom(): void {
  const camera = gr().camera as
    | (THREE.OrthographicCamera & { zoom: number })
    | undefined;
...
function box(
  ...
  const m = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color, roughness: 0.7, metalness: 0.45 }),
  );
...
function buildGroup(): THREE.Group {
  ...
  for (const [slot] of readExterior()) {
    ...
    for (const gx of [-0.85, 0, 0.85]) {
      const rib = new THREE.Mesh(
        new THREE.BoxGeometry(0.04, 0.1, 1.62),
        new THREE.MeshStandardMaterial({ color: HULL_DARK }),
      );
  ...
  // renders from its ONE builder at its true pose, so what you mounted in
  // edit mode is exactly what you see from space. Materials are forced to
  // their resting opacity (buildItemGroup starts them at 0 for the room
  // morph fade, which never runs out here). Interior-only fittings no longer
  // paint fake bells/saddles on the hull — the hull shows the truth.
  for (const item of FURNITURE) {
    if (!isExteriorItem(item)) continue;
    const ig = buildItemGroup(item);
    ig.position.set(item.pos.x, 0, item.pos.z);
    ig.rotation.y = item.rot * (Math.PI / 2);
    ig.traverse((obj) => {
...
function openBendEditor(
  ...
  editor.innerHTML = `
    ...
    <div id="exterior-bend-row">${bends
      .map(
        (b) =>
          `<button type="button" data-bend="${b}" class="exterior-bend-btn${b === currentBend ? " current" : ""}">${b > 0 ? "+" : ""}${b}°</button>`,
      )
      .join("")}</div>
  ...
  editor
    .querySelector("#exterior-bend-close")
    ?.addEventListener("click", closeEditor);
  editor
    .querySelectorAll<HTMLButtonElement>(".exterior-bend-btn")
    .forEach((btn) => {
      btn.addEventListener("click", () => {
        const bend = Number(btn.dataset.bend);
        const ok =
          worldRef().dockingSystem?.editChainSegment?.(doorId, index, {
            bendDeg: bend,
          }) ?? false;
        const note = editor?.querySelector("#exterior-bend-note");
        if (note)
          note.textContent = ok
            ? `bend set to ${bend}° — synced to everyone`
            : "no construction rights on this port";
        if (ok) {
          editor
            ?.querySelectorAll(".exterior-bend-btn")
...
function onClickCapture(e: MouseEvent): void {
  ...
  while (node) {
    if (node.userData?.isAtlasModule) {
      ...
      const paneOpen =
        (document.getElementById("docking-control-pane")?.style.display ??
          "none") !== "none";
    ...
    if (
      node.userData?.isConnectorPart &&
      node.userData.kind === "flex" &&
      !flexPart
    )
      flexPart = node;
    if (node.userData?.isConnectorChain) {
      chainRoot = node;
      break;
    }
    node = node.parent;
  ...
  if (flexPart && chainRoot) {
    ...
    if (index >= 0) {
      openBendEditor(
        doorId,
        index,
        Number(flexPart.userData.bendDeg ?? 0),
        e.clientX,
        e.clientY,
      );
      return;
...
export function showEnterRoomBubble(onEnter: () => void): void {
  ...
  if (!active) {
    onEnter();
    return;
  } // exterior unavailable → straight inside
