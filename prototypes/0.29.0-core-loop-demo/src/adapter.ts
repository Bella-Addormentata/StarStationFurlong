...
function flexArcFrame(
  ...
  return {
    x: (1 - Math.cos(bendRad * t)) * r,
    z: Math.sin(bendRad * t) * r,
    yaw: bendRad * t,
  };
}
...
export function buildFlexJoint(bendDeg: number, stretch = 0): THREE.Group {
  ...
  group.userData = {
    ...
    kind: "flex",
    bendDeg: bend,
    stretch: clampFlexStretch(stretch),
  };
  const mats = partMats();
...
export function buildExtension(
  ...
  group.userData = {
    ...
    bays: nBays,
    skin,
    stretch: clampExtStretch(stretch),
  };
 ...
 *  exactly as solved. */
function segmentExit(seg: ConnectorSegment): {
  x: number;
  z: number;
  yawRad: number;
} {
  ...
  return {
    x: 0,
    z:
      clampExtBays(seg.bays ?? EXT_BAYS_MIN) * EXT_BAY_LEN +
      clampExtStretch(seg.stretch ?? 0),
    yawRad: 0,
  };
}
...
export function solveChain(
  ...
  const flexIdx = segs
    .map((s, i) => (s.kind === "flex" ? i : -1))
    .filter((i) => i >= 0);
  const extIdx = segs
    .map((s, i) => (s.kind === "ext" ? i : -1))
    .filter((i) => i >= 0);
  ...
  for (let iter = 0; iter < 60; iter++) {
    ...
    if (flexIdx.length > 1) {
      const mean =
        flexIdx.reduce((s, i) => s + (segs[i].bendDeg ?? 0), 0) /
        flexIdx.length;
    ...
    write(
      p.map(
        (v, j) =>
          v - ((step * grad[j]) / norm) * (j < flexIdx.length ? 8 : 0.4),
      ),
    );
  }
 ...
 */
export function foldChainEnd(segments: ConnectorSegment[]): {
  x: number;
  z: number;
  yawRad: number;
} {
  let x = 0,
    z = CHAIN_PORTAL_MARGIN,
    yaw = 0;
 ...
 */
export function setVestibuleLightState(
  group: THREE.Group,
  state: VestibuleLightState,
): void {
