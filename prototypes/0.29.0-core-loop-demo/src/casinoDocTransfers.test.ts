...
// flood, and preserve the physical-conservation invariant (Σ bal:* unchanged
// by any successful transfer). The READ path (readAllChipTransfers) must drop
// hostile / malformed rows so a peer that hijacks the map cannot crash BANK
// render or smuggle a mis-keyed record into the audit log.
//
 ...
 *  path does NOT verify signatures itself — verification lives at the render
 *  seam — so any string keeps the shape guard happy. */
...
describe('casinoDoc · writeChipTransfer (happy path)', () => {
  it('atomically debits sender + credits recipient + writes xfer row', () => {
    ...
    expect(writeChipTransfer(t)).toBe(true);
  ...
  it('preserves Σ bal:* across every transfer (conservation invariant)', () => {
    ...
    expect(writeChipTransfer(newTransfer(ALICE_PUB, BOB_PUB, ALICE_PID, BOB_PID, 100, 'n2', ts0))).toBe(true);
    expect(writeChipTransfer(newTransfer(BOB_PUB, ALICE_PUB, BOB_PID, ALICE_PID, 40, 'n3', ts0 + 1))).toBe(true);
    expect(writeChipTransfer(newTransfer(ALICE_PUB, BOB_PUB, ALICE_PID, BOB_PID, 60, 'n4', ts0 + 2))).toBe(true);
...
describe('casinoDoc · writeChipTransfer (refusals)', () => {
  it('refuses when the sender has no chips', () => {
    ...
    expect(writeChipTransfer(t)).toBe(false);
    expect(readChips(BOB_PID)).toBe(0);
  ...
  it('refuses when the transfer object is malformed (guard on own writes)', () => {
    ...
    expect(writeChipTransfer(bad as unknown as ChipTransfer)).toBe(false);
    // Balances untouched.
    expect(readChips(ALICE_PID)).toBe(500);
    expect(readChips(BOB_PID)).toBe(0);
  });

  it('refuses when the transfer map has hit MAX_CHIP_TRANSFERS (flood defence)', () => {
    // Seed a fake flood — write MAX_CHIP_TRANSFERS records via the low-level
    // Y.Map directly (a hostile peer's writes look like this on our doc). The
    // cheapest way to reach the bound is to loop writeChipTransfer, but that's
    // slow at 512; poke the map directly and verify writeChipTransfer refuses.
    buyInChips(ALICE_PID, 100_000);
    // Use the doc handle directly to plant well-formed transfers up to the cap.
    for (let i = 0; i < MAX_CHIP_TRANSFERS; i++) {
      // Small, valid transfers of 1 chip each; each id is distinct via nonce.
      const t = newTransfer(ALICE_PUB, BOB_PUB, ALICE_PID, BOB_PID, 1, `flood-${i}`, 1_700_100_000_000 + i);
      const ok = writeChipTransfer(t);
      if (!ok) {
        // Bound reached earlier than MAX (shouldn't happen for well-formed
        // sequential writes); the invariant we care about is the NEXT write.
        break;
      }
    }
    // Now try one more — must be refused.
    const overflow = newTransfer(ALICE_PUB, BOB_PUB, ALICE_PID, BOB_PID, 1, 'overflow-1', 1_700_200_000_000);
    expect(writeChipTransfer(overflow)).toBe(false);
    // The audit-log reader also caps at MAX_CHIP_TRANSFERS.
    expect(readAllChipTransfers().length).toBeLessThanOrEqual(MAX_CHIP_TRANSFERS);
  });
});
...
describe('casinoDoc · readAllChipTransfers (hostile posture)', () => {
  it('drops xfer:* rows whose key does not match value.id (hijack defence)', () => {
    ...
    expect(writeChipTransfer(good)).toBe(true);
  ...
  it('returns every well-formed row (caller sorts) — deterministic content', () => {
    ...
    expect(writeChipTransfer(a)).toBe(true);
    expect(writeChipTransfer(b)).toBe(true);
    const rows = readAllChipTransfers();
