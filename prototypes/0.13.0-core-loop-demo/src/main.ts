...
async function mintBootstrapLink(rawAddress: string): Promise<{ link?: string; error?: string; scope?: ReturnType<typeof classifyAddress> }> {
  ...
  const parsed = parseHostAddress(rawAddress, fingerprint.port);
  if (!parsed) {
    return { error: 'Enter the address friends can reach you at (LAN IP, public IP, or DNS name).' };
  }

  try {
    localStorage.setItem(BOOTSTRAP_ADDRESS_STORAGE_KEY, rawAddress.trim());
  } catch {
  ...
  const boot = buildOutgoingBootstrap(parsed.wtUrl, fingerprint);
  return {
    link: `${window.location.origin}${window.location.pathname}?seed=${encodeURIComponent(encodeBootstrapSeed(boot))}`,
    scope,
  };
}
...
export async function generateBootstrapLink(): Promise<{ link?: string; error?: string }> {
  ...
  return mintBootstrapLink(addressInput?.value ?? '');
}
...
async function buildShareLinkForHostname(hostname: string): Promise<string | null> {
  const minted = await mintBootstrapLink(hostname);
  return minted.link ?? null;
}
...
async function syncShareLink(): Promise<void> {
  ...
  let defaultPort = 4443;
  if (activeBootstrap) {
    try {
      const activeUrl = new URL(activeBootstrap.wtUrl);
      defaultPort = Number(activeUrl.port || 4443);
    } catch {
      defaultPort = 4443;
    }
  }
  ...
  if (rawAddress) {
    const parsed = parseHostAddress(rawAddress, defaultPort);
    if (parsed) {
      const scope = classifyAddress(new URL(parsed.wtUrl).hostname);
      if (scope === 'private') {
        bestLan = new URL(parsed.wtUrl).hostname;
      } else if (scope === 'public') {
        bestWan = new URL(parsed.wtUrl).hostname;
      }
    }
  }
  ...
  const lanLink = bestLan
    ? await buildShareLinkForHostname(bestLan)
    : null;
  const wanLink = bestWan
    ? await buildShareLinkForHostname(bestWan)
    : null;

  if (shareElLan) {
    shareElLan.value = lanLink ?? 'Enter your LAN address above & click Bootstrap';
  }
  if (shareElWan) {
    shareElWan.value = wanLink ?? 'Enter your Public WAN/Internet address & click Bootstrap';
  }
}
