...
* [ ] **Company Treasury & Governance — Testnet:** Implement the [company treasury architecture](brainstorming/company-treasury-governance-plan.md) as amended by the [sovereign serverless plan](brainstorming/sovereign-treasury-serverless-plan.md) on Chia testnet: stable singleton identity, rotatable threshold board, per-proposal governance policy, on-chain proposal registration and vote checkpoints, signed votes and receipts, room funding bindings, and authenticated operation-specific allowances — with no treasury service anywhere: Chialisp puzzles are the only money authority and every player's own node verifies. Keep Yjs display-only; each node pauses its own spending whenever its verified chain state is unavailable (chain access via Chia peer-protocol cross-check is the amendment's **proposed** posture, pending maintainer ratification — plan §6/§14).
